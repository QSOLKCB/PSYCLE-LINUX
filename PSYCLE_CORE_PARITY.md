...
| Delayed / retrigger / extended commands | [`original-delayed-retrigger.json`](phase6c/evidence/sequencer-delayed-retrigger/original-delayed-retrigger.json): pinned 1.12.0 accepts the exact FD/FB/FA/FE04 fixture with 38 stable marker polls and clean runtime/UI identity; [`original-source-delayed-retrigger.json`](phase6c/evidence/sequencer-delayed-retrigger/original-source-delayed-retrigger.json) separately binds the pinned original command IDs/formulas, but `runtime_execution_trace` remains `not-observed` | [`candidate-delayed-retrigger.json`](phase6c/evidence/sequencer-delayed-retrigger/candidate-delayed-retrigger.json): frozen-core runtime probe observes FD note-delay scheduling, complete FB/FA retrigger series and FE04 marker geometry with the exact final PR #69 fixture | fixture construction only | UNKNOWN | additive original-render diagnostics now narrow the boundary in three stages: PR #72 shows FD/FB/FA/FE are not the differentiator; PR #73 workflow `35730075034` shows Master-only, empty-Sampler and sample/instrument-state controls retain Psycle with the same finalized silent PCM output while one ordinary note reproduces `0xC0000005`; workflow `35734490424` then binds the startup ladder to pinned original `Sampler.cpp` / `Song.cpp` source and shows the no-previous-instrument and missing-sample controls survive with the same finalized silent WAV, while both enabled-sample variants (constructor-default original Instrument and serialized instrument state) exit with `0xC0000005` and zero-byte output. The boundary is enabled-sample voice startup, not serialized instrument state. Isolate `Voice::Tick` initialization versus first `Voice::Work`, then obtain a command-bearing original output and collect same-witness candidate render evidence before timing classification. The frozen PR #69 callback schedule remains preservation evidence, not the comparison partner; [`comparison.json`](phase6c/evidence/sequencer-delayed-retrigger/comparison.json) remains frozen. |
...
7. preserve the versioned deferred comparison: source semantics plus native load acceptance are not promoted into original runtime execution. PR #72 excludes FD/FB/FA/FE as the crash differentiator; PR #73 narrows the exit to ordinary-note-present Sampler execution; and workflow `35734490424` at evidence head `c4c4b55900d1444cbe644345e559067d3996c532` shows source-pinned no-previous-instrument and missing-sample early-return controls survive while both enabled-sample notes (constructor-default original Instrument and serialized instrument state) exit with `0xC0000005`. The boundary is therefore enabled-sample voice startup, not serialized instrument state. Isolate `Voice::Tick` initialization versus first `Voice::Work` next; only after obtaining a command-bearing original output should candidate render evidence be collected from the same witness for like-for-like timing. Do not compare that sampled output directly with the frozen PR #69 one-beat callback schedule;
