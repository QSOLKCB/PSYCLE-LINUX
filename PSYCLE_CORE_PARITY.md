...
| Delayed / retrigger / extended commands | [`original-delayed-retrigger.json`](phase6c/evidence/sequencer-delayed-retrigger/original-delayed-retrigger.json): pinned 1.12.0 accepts the exact FD/FB/FA/FE04 fixture with 38 stable marker polls and clean runtime/UI identity; [`original-source-delayed-retrigger.json`](phase6c/evidence/sequencer-delayed-retrigger/original-source-delayed-retrigger.json) separately binds the pinned original command IDs/formulas, but `runtime_execution_trace` remains `not-observed` | [`candidate-delayed-retrigger.json`](phase6c/evidence/sequencer-delayed-retrigger/candidate-delayed-retrigger.json): frozen-core runtime probe observes FD note-delay scheduling, complete FB/FA retrigger series and FE04 marker geometry with the exact final PR #69 fixture | fixture construction only | UNKNOWN | additive original-render diagnostics now narrow the boundary in four stages: PR #72 shows FD/FB/FA/FE are not the differentiator; PR #73 workflow `35730075034` reaches ordinary-note-present Sampler execution; workflow `35734490424` shows the no-previous-instrument and missing-sample controls survive while both enabled-sample variants (constructor-default original Instrument and serialized instrument state) exit with `0xC0000005` and zero-byte output. Serialized instrument state is not the differentiator; workflow `35752301481` then shows the enabled-sample release/no-active-voice control survives with a finalized silent WAV, while one-row `E-DF`, four-beat `E-DF`, and one-row ordinary-note fixtures all exit with `0xC0000005` and zero-byte output. Because source-pinned Sampler-local `E-DF` is a 2.5-row delay and the short fixture is only one row, it cannot reach `controller.Work`; this excludes normal controller sample processing as a prerequisite, but it does not establish whether `Voice::Work` was entered. Voice selection/setup, `Voice::Tick` initialization, and pre-`controller.Work` `Voice::Work` entry remain unresolved. The hash-bound diagnostic projection is frozen at [`observation.json`](phase6c/evidence/sequencer-sampler-work-boundary/observation.json). The opt-in runtime fault-location lane has now produced a versioned hosted witness: PR #78 workflow `35772562875` captures second-chance `0xC0000005` at pinned `psycle.exe+0x5c377`, with corroborating workflow `35770412423` reproducing the same module-relative offset under a different ASLR base. The committed [`observation.json`](phase6c/evidence/sequencer-sampler-fault-location/observation.json) binds both runs, artifacts, receipts, debugger identity and module snapshots. This establishes a stable binary-relative crash position, not a source-function identity; `function_location` remains unresolved. The next like-for-like evidence is a command-bearing original output and same-witness candidate render before timing classification. The frozen PR #69 callback schedule remains preservation evidence, not the comparison partner; [`comparison.json`](phase6c/evidence/sequencer-delayed-retrigger/comparison.json) remains frozen. |
...
7. preserve the versioned deferred comparison: source semantics plus native load acceptance are not promoted into original runtime execution. PR #72 excludes FD/FB/FA/FE as the crash differentiator; PR #73 and workflow `35734490424` narrow the exit to enabled-sample voice startup; workflow `35752301481` at evidence head `4cf71eba6f7b5f6ba14be6089cf37422b16b5ca6` advances the boundary inside `Voice::Tick`. Its enabled-sample release/no-active-voice control survives, but one-row `E-DF`, four-beat `E-DF`, and one-row ordinary-note fixtures all exit with exact `0xC0000005`. Since Sampler-local `E-DF` delays for 2.5 rows, the one-row fixture cannot reach `controller.Work`, excluding normal controller sample processing as a prerequisite for the crash. The evidence does not establish whether `Voice::Work` was entered. Voice selection/setup, `Voice::Tick` initialization, and pre-`controller.Work` `Voice::Work` entry remain unresolved. The fault-location rung is now versioned: PR #78 workflow `35772562875` captures pinned `psycle.exe+0x5c377`, and corroborating workflow `35770412423` reproduces the same offset under a different ASLR base. This remains diagnostic and cannot name a source function without independent symbol/source binding. The next unresolved rung is a command-bearing original output, after which candidate render evidence should be collected from the same witness for like-for-like timing. Do not compare that sampled output directly with the frozen PR #69 one-beat callback schedule;
