...
7. preserve the versioned deferred comparison: source semantics plus native load acceptance are not promoted into original runtime execution, so the next tracker-command evidence step is a stronger original execution observer rather than another source-plus-load comparison;
