...
| Delayed / retrigger / extended commands | [`original-delayed-retrigger.json`](phase6c/evidence/sequencer-delayed-retrigger/original-delayed-retrigger.json): pinned 1.12.0 accepts the exact FD/FB/FA/FE04 fixture with 38 stable marker polls and clean runtime/UI identity; [`original-source-delayed-retrigger.json`](phase6c/evidence/sequencer-delayed-retrigger/original-source-delayed-retrigger.json) separately binds the pinned original command IDs/formulas, but `runtime_execution_trace` remains `not-observed` | [`candidate-delayed-retrigger.json`](phase6c/evidence/sequencer-delayed-retrigger/candidate-delayed-retrigger.json): frozen-core runtime probe observes FD note-delay scheduling, complete FB/FA retrigger series and FE04 marker geometry with the exact final PR #69 fixture | fixture construction only | UNKNOWN | the stronger original execution-attempt observer is implemented on a separate four-beat sampled witness; obtain and version a command-bearing original runtime output, then collect candidate execution evidence using that same witness and a comparable rendered-onset procedure before any like-for-like timing classification. The frozen PR #69 callback schedule remains preservation evidence, not the comparison partner for this sampled witness; the versioned evidence-role decision remains frozen in [`comparison.json`](phase6c/evidence/sequencer-delayed-retrigger/comparison.json) |
...
7. preserve the versioned deferred comparison: source semantics plus native load acceptance are not promoted into original runtime execution; the stronger execution-attempt observer is now implemented on a separate four-beat sampled witness, so first obtain and version a command-bearing original runtime output, then collect candidate execution evidence from the same witness with a comparable rendered-onset procedure before any like-for-like timing comparison. Do not compare that sampled output directly with the frozen PR #69 one-beat callback schedule;
