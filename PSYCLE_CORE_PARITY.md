...
7. preserve the versioned deferred comparison: source semantics plus native load acceptance are not promoted into original runtime execution; the stronger execution-attempt observer is now implemented, so the remaining tracker-command evidence step is to obtain and version a command-bearing original runtime output before any like-for-like timing comparison;
