...
The current classified scope is three `PASS` contracts (PSY2 parse/load, PSY3 parse/load, and single-sequence order), one serialization/save `MISSING` contract, and one BPM/LPB/tick `DIFFERENT` contract; 15 contracts remain `UNKNOWN`. The first Phase 7 backlog items are therefore the exact-fixture PSY3 save capability and a distinct legacy LPB/TPB timing model. The delayed/retrigger Sampler diagnostics remain evidence work: the one-row delayed witness excludes normal `controller.Work` sample processing as a prerequisite for the observed crash, but voice selection/setup, `Voice::Tick` initialization, and pre-`controller.Work` `Voice::Work` entry remain unresolved. An opt-in manual Phase 6C lane can now attach a preinstalled, hash-bound x86 CDB to a fresh copy of that same short `E-DF` witness and retain an exception/module-offset observation; tool absence or incomplete location is inconclusive, and no module offset is promoted directly to a source-function claim.
