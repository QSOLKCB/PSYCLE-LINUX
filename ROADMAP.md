...
- [ ] tracker commands and delayed/retrigger events;
  ...
  - [x] Add a stronger original-Psycle execution observer without mutating the frozen comparison fixture: an additive sampled witness drives pinned Psycle 1.12.0's own offline `Render as Wav` path twice at mono 44.1 kHz / 16-bit / dither-off, and validation requires byte-identical renders plus multiple impulses in both the `FB` and `FA` witness windows before calling runtime execution observed. Exact timing parity remains deliberately unclassified.
  - [ ] Version a clean execution-witness run and bind the original runtime receipt before interpreting exact onset timing against the frozen candidate schedule; keep `sequencer-delayed-retrigger` `UNKNOWN` until that like-for-like comparison is versioned.
