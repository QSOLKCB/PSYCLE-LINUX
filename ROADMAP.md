...
- [ ] tracker commands and delayed/retrigger events;
  ...
  - [x] Split the enabled-sample boundary through source-pinned `Voice::Tick` / `Voice::Work` gates. Workflow `35752301481` at evidence head `4cf71eba6f7b5f6ba14be6089cf37422b16b5ca6` shows the one-row enabled-sample release/no-active-voice control retaining Psycle and a finalized 4,868-byte silent WAV (`f40e6f9f280c993b8c0ecdf6a54ba8314462803b7075e46d7db0c4a2bd9dfbae`), while one-row `E-DF`, four-beat `E-DF`, and one-row ordinary-note fixtures all exit with exact `0xC0000005` and zero-byte WAVs. Because original Sampler-local `E-DF` is a 2.5-row delay and the one-row fixture ends before that delay can expire, `Voice::Work` cannot reach `controller.Work`; the historical generated receipt records `voice-tick-initialization-associated-exit`, but that label is narrower than the observation proves. The qualified result is `enabled-note-startup-pre-controller-work-associated-exit`: normal `controller.Work` sample processing is excluded as a prerequisite, while voice selection/setup, `Voice::Tick` initialization, and pre-`controller.Work` `Voice::Work` entry remain unresolved. The hash-bound projection is committed at `phase6c/evidence/sequencer-sampler-work-boundary/observation.json`; obtain a runtime fault-location witness before naming a function-level location.
