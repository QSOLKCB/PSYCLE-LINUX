...
- [ ] tracker commands and delayed/retrigger events;
  ...
  - [x] Version final PR #69 run `35457374404` at head `9310b08bae744093c068a4f557bbd1b80bd499c8` and commit the evidence-role comparison. The comparison mechanically concludes that original source semantics plus native fixture acceptance are not like-for-like with candidate runtime scheduling, so `sequencer-delayed-retrigger` remains `UNKNOWN` rather than forcing a `PASS` / `DIFFERENT` / `MISSING` verdict.
  - [x] Add a stronger original-Psycle execution-attempt observer without mutating the frozen comparison fixture: an additive sampled witness reaches the existing clean-load gate before invoking pinned Psycle 1.12.0's exact offline `Render as Wav` UI at mono 44.1 kHz / 16-bit / dither-off. Successful output still requires two byte-identical renders plus multiple impulses in both the `FB` and `FA` windows; a reference-process exit is retained separately as failure evidence and cannot be promoted to command execution.
  - [x] Add an additive fresh-process render-isolation matrix for the PR #71 access violation: ordinary-note control plus isolated `FD 7F`, `FB 3F`, `FA 42` and `FE 04` sampled witnesses, all using the same sample/routing/timing family and exact original-render UI. Workflow `35723698458` localizes the observed `0xC0000005` failure to the shared sampled-fixture/original-render path because the control and every single-command variant fail identically after verified Save Wave dispatch; this remains diagnostic only and cannot classify parity.
  - [x] Isolate that shared render failure with the PR #73 cumulative substrate ladder. Workflow `35730075034` at head `0a99e8a7af3aa2d714e1c85480d16ddaf62c9ab0` shows Master-only, empty-Sampler, and Sampler+embedded-sample/instrument fixtures each retain the reference process and produce the same finalized 154,412-byte silent RIFF/WAV (`bfcdb0b89773f22484650ad80fc52354ee7c584f89c272d0e8f67ee5546a6e8c`), while adding the first ordinary note exits with `0xC0000005` and leaves a zero-byte WAV.
  - [x] Split that ordinary-note boundary through the source-pinned Sampler startup gates. Workflow `35734490424` at evidence head `c4c4b55900d1444cbe644345e559067d3996c532` shows instrument `FF` with no previous instrument and explicit instrument 1 with missing sample 1 both retain Psycle and the same finalized silent WAV, while enabled sample 0 crashes with `0xC0000005` whether original Psycle uses its constructor-default legacy Instrument or the fixture serializes instrument state. The derived result is `enabled-sample-voice-startup-associated-exit`: serialized instrument state is not the differentiator; the next diagnostic boundary is inside enabled-sample voice startup (`GetFreeVoice` / `Voice::Tick` initialization versus first `Voice::Work`).
  - [x] Split the enabled-sample boundary through source-pinned `Voice::Tick` / `Voice::Work` gates. Workflow `35752301481` at evidence head `4cf71eba6f7b5f6ba14be6089cf37422b16b5ca6` shows the one-row enabled-sample release/no-active-voice control retaining Psycle and a finalized 4,868-byte silent WAV (`f40e6f9f280c993b8c0ecdf6a54ba8314462803b7075e46d7db0c4a2bd9dfbae`), while one-row `E-DF`, four-beat `E-DF`, and one-row ordinary-note fixtures all exit with exact `0xC0000005` and zero-byte WAVs. Because original Sampler-local `E-DF` is a 2.5-row delay and the one-row fixture ends before that delay can expire, `Voice::Work` cannot reach `controller.Work`; the historical generated receipt records `voice-tick-initialization-associated-exit`, but that label is narrower than the observation proves. The qualified result is `enabled-note-startup-pre-controller-work-associated-exit`: normal `controller.Work` sample processing is excluded as a prerequisite, while voice selection/setup, `Voice::Tick` initialization, and pre-`controller.Work` `Voice::Work` entry remain unresolved. The hash-bound projection is committed at `phase6c/evidence/sequencer-sampler-work-boundary/observation.json`.
  - [x] Add an opt-in pinned-reference fault-location collector for the exact short `E-DF` witness. The normal workflow remains the uninstrumented control. Manual dispatch with `sampler_fault_location=true` adds a fresh clean-load run that uses only a preinstalled hash-bound x86 `cdb.exe`, snapshots loaded module ranges before render, and retains a second-chance `0xC0000005` exception address plus module-relative offset when available. Missing tooling, attach failure, missing address, or an unmapped address remain `inconclusive`; symbol/function location stays unresolved and parity remains `UNKNOWN`.
  - [x] Run and version the hosted fault-location observation. PR #78 workflow `35772562875` at evidence head `ed1d2ad38700021d88141e9d5310367f6e7753d9` validates the exact short `E-DF` control with `0xC0000005` and captures a second-chance exception at `0x00e6c377` inside pinned `psycle.exe` loaded at `0x00e10000`, yielding module-relative offset `0x5c377`. Corroborating workflow `35770412423` captured the same `0x5c377` offset with a different ASLR base (`0x00520000` -> `0x0057c377`). The repeated offset narrows the failure to a stable binary position, but `function_location` remains `unresolved` because no independent symbol/source mapping has been established. See `phase6c/evidence/sequencer-sampler-fault-location/observation.json`.
  - [ ] Obtain and version a command-bearing original runtime execution output from the separate four-beat sampled witness, then collect candidate execution evidence using that same witness and a comparable rendered-onset procedure before interpreting exact onset timing. The frozen PR #69 one-beat callback schedule remains preservation evidence and is not a like-for-like comparison partner for this sampled witness. The first pinned-reference render attempt exits after verified Save Wave dispatch before producing a valid waveform, so `sequencer-delayed-retrigger` remains `UNKNOWN`.
...
**Phase 6C status: active.** PSY2 and PSY3 parse/load acceptance plus sequence/pattern order are scoped `PASS` results for exact project-authored fixtures, serialization/save capability is scoped `MISSING`, and BPM/LPB/tick timing is scoped `DIFFERENT`; 15 contracts remain `UNKNOWN`. The timing verdict is limited to the exact BPM 137 / LPB 8 / TPB 24 fixture: original Psycle preserves TPB 24 while the candidate feeds LPB 8 into its legacy tick-speed path with `is_ticks=true`. It does not classify delayed/retrigger commands, sampler tick processing, playback duration, tempo changes, multi-sequence behaviour or UI editing. The confirmed `MISSING` serializer gap and `DIFFERENT` tick-semantics gap now both justify narrow Phase 7 convergence backlog items. The final PR #69 delayed/retrigger run is versioned and compared; that comparison remains `UNKNOWN` because the original side lacks a like-for-like runtime command-execution trace. Later additive diagnostics keep the frozen PR #69/#70 comparison bytes untouched: PR #72 excludes FD/FB/FA/FE as the crash differentiator; PR #73 narrows the exit to ordinary-note-present Sampler execution; workflow `35734490424` shows the exit begins only after an enabled sample advances into voice startup; and workflow `35752301481` advances one gate further. Its enabled-sample release/no-active-voice control survives, while even the one-row `E-DF` fixture exits with `0xC0000005`. Since source-pinned `E-DF` delays the voice for 2.5 rows, that one-row song cannot reach `controller.Work`, so normal controller sample processing is not a prerequisite for the observed exit. The evidence does not establish whether `Voice::Work` was entered or whether the fault occurred in `Voice::Tick`; voice selection/setup, `Voice::Tick` initialization, and pre-`controller.Work` `Voice::Work` entry remain unresolved. The opt-in runtime fault-location collector is implemented and PR #78 workflow `35772562875` has now versioned a successful hosted witness: x86 CDB captured `0xC0000005` at `psycle.exe+0x5c377`; an earlier independent run captured the same offset under a different ASLR base. This establishes a repeatable binary-relative crash location while leaving `function_location` unresolved. The remaining delayed/retrigger rung is now a command-bearing original output plus same-witness candidate render before any timing classification.
