...
- [ ] tracker commands and delayed/retrigger events;
  ...
  - [x] Isolate that shared render failure with a cumulative fresh-process substrate ladder. Workflow `35730075034` at head `0a99e8a7af3aa2d714e1c85480d16ddaf62c9ab0` shows Master-only, empty-Sampler, and Sampler+embedded-sample/instrument fixtures each retain the reference process and produce the same finalized 154,412-byte silent RIFF/WAV (`bfcdb0b89773f22484650ad80fc52354ee7c584f89c272d0e8f67ee5546a6e8c`), although the render dialog does not reach the observer's normal terminal-close condition. Adding the first ordinary note is the first rung that exits with `0xC0000005` and leaves a zero-byte WAV, narrowing the failure to ordinary-note/Sampler playback execution rather than renderer setup, Sampler presence, or serialized sample/instrument state.
  - [x] Add a cumulative render-substrate ladder after PR #72: Master-only → empty Sampler → embedded sample/instrument state → one ordinary note. Workflow `35730075034` at evidence head `0a99e8a7af3aa2d714e1c85480d16ddaf62c9ab0` shows the first three rungs produce the same finalized 154412-byte silent mono PCM output while Psycle remains alive, whereas adding the ordinary note reproduces the verified Save Wave `0xC0000005` exit with a zero-byte output. This narrows the observed failure to an ordinary-note-present Sampler execution path without claiming a unique root cause or changing parity.
