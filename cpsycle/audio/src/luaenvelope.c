...
int luaenvelope_create(lua_State* L)
{
	...
	psyclescript_createuserdata(L, 1, luaenvelope_meta, env,
		(psy_fp_gcptr_dispose)lenvelope_dispose);	
	return 1;
}

int luaenvelope_gc(lua_State* L)
{
	psyclescript_deallocate_userdata(L, luaenvelope_meta);	
	return 0;
}
...
int work(lua_State* L)
{
	...
	if (n == 2) {
		...
		ptr = psyclescript_check(L, 1, luaenvelope_meta);
		self = (LEnvelope*)(ptr->ud);
