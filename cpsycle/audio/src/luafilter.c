...
{		
	...
	filter_init(filter);	
	psyclescript_createuserdata(L, 1, luafilter_meta, filter,
		(psy_fp_gcptr_dispose)filter_dispose);	
	return 1;
}

int luafilter_gc(lua_State* L)
{	
	psyclescript_deallocate_userdata(L, luafilter_meta);	
	return 0;
}
...
int luafilter_work(lua_State* L)
{
	...
	if (n == 2 || n == 3 || n == 4) {
		...
		ptr = psyclescript_check(L, 1, luafilter_meta);
		filter = (Filter*)(ptr->ud);
