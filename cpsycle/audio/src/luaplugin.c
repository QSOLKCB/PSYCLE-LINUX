...
int luapluginparam_label(psy_audio_LuaPluginMachineParam* self, char* text)
{
	...
	if (self->index >= 0 && self->index < self->plugin->client->numparameters_) {
		...
		psy_luaimport_init(&in, self->plugin->script.L, self->plugin->client, NULL);
...
static int luamachine_setviewport(lua_State*);
...
{	
	if (!psyclescript_empty(&self->script)) {
		...
		dispose(self);
		psy_audio_luaplugin_init(self, mcb, path, self->script.logger, self->hostview);
	}	
}
...
{		
	...
	if (err != 0) {
		...
		psy_audio_luaplugin_init(&plugin, NULL, path, logger, NULL);
...
void generateaudio(psy_audio_LuaPlugin* self, psy_audio_BufferContext* bc)
{
	if (psy_audio_buffercontext_num_samples(bc) > 0) {
		...
		psy_luaimport_init(&in, self->script.L, self->client, NULL);
...
int luamachine_open(lua_State *L)
{
	static const luaL_Reg methods[] = {
		...
		{"setviewport", luamachine_setviewport},
		{NULL, NULL}
	  };
...
int luamachine_setparameters(lua_State* L)
{
	...
	ptr = psyclescript_check(L, 1, luamachine_meta);
	self = (psy_audio_LuaMachine*)(ptr->ud);
...
int luamachine_setviewport(lua_State* L)
{
	psy_GCPtr* ptr;
	psy_audio_LuaMachine* self;	
	...
	/*psy_ui_Component* ptr;
		...
		ptr = *(psy_ui_Component**)luaL_checkudata(L, 2, psy_luaui_component_meta);
		if (ptr) {			
				psy_ui_component_set_parent(ptr, plugin->hostview);
		}*/
//	}
	return 0;
}
