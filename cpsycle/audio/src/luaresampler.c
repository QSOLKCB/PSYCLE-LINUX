...
{		
	...
	*/
	...
	psyclescript_createuserdata(L, 1, luaresampler_meta, rwrap,
		(psy_fp_gcptr_dispose)luaresampler_dispose);
	lua_pushnil(L);
...
int gc(lua_State* L)
{
	psyclescript_deallocate_userdata(L, luaresampler_meta);
	return 0;
}
...
int work(lua_State* L)
{
	...
	ptr = psyclescript_check(L, 1, luaresampler_meta);
	rwrap = (LuaResampler*)(ptr->ud);	
