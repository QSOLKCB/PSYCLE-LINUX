...
int create(lua_State* L)
{
    ...
    psyclescript_createuserdata(L, 1, luawavedata_meta, sample,
        (psy_fp_gcptr_dispose)psy_audio_sample_dispose);    
    return 1;
}

int gc(lua_State* L)
{
    psyclescript_deallocate_userdata(L, luawavedata_meta);    
    return 0;    
}
...
int copy(lua_State* L) {
	...
	ptr = psyclescript_check(L, 1, luawavedata_meta);
	wave = (psy_audio_Sample*)(ptr->ud);
