...
{	
	...
	if (n != 2 && n != 3) {
		...
		psyclescript_createuserdata(L, 1, luawaveosc_meta, osc,
			(psy_fp_gcptr_dispose)psy_audio_waveosc_dispose);
		return 1;
	}
	return 0;
}
...
{
	psyclescript_deallocate_userdata(L, luawaveosc_meta);	
	return 0;
}
...
int luawaveosc_work(lua_State* L)
{
	...
	if (n == 2 || n == 3 || n == 4 || n == 5) {
		...
		ptr = psyclescript_check(L, 1, luawaveosc_meta);
		self = (psy_audio_WaveOsc*)(ptr->ud);
