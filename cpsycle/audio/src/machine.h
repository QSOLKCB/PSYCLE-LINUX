...
typedef	bool (*fp_mcb_addcapture)(void*, intptr_t index);
typedef	bool (*fp_mcb_removecapture)(void*, intptr_t index);
typedef void (*fp_mcb_readbuffers)(void*, intptr_t index, float** pleft,
	float** pright, uintptr_t numsamples);
typedef const char* (*fp_mcb_capturename)(void*, intptr_t index);
typedef uintptr_t (*fp_mcb_numcaptures)(const void*);
typedef const char* (*fp_mcb_playbackname)(void*, intptr_t index);
typedef uintptr_t (*fp_mcb_numplaybacks)(const void*);
...
typedef	void (*psy_audio_fp_machine)(struct psy_audio_Machine*);
...
typedef	psy_List* (*fp_machine_sequencerinsert)(struct psy_audio_Machine*,
	psy_List* events);
typedef	void (*fp_machine_set_panning)(struct psy_audio_Machine*,
	double);
...
typedef	int (*fp_machine_has_standby)(struct psy_audio_Machine*);
...
typedef	void (*fp_machine_updatesamplerate)(struct psy_audio_Machine*,
	double samplerate);
typedef	uintptr_t (*fp_machine_numinputs)(const struct psy_audio_Machine*);
typedef	uintptr_t (*fp_machine_numoutputs)(const struct psy_audio_Machine*);
typedef	uintptr_t (*fp_machine_slot)(struct psy_audio_Machine*);
typedef	void (*fp_machine_set_slot)(struct psy_audio_Machine*, uintptr_t);
...
typedef	psy_audio_MachineParam* (*fp_machine_tweakparameter)(struct
	psy_audio_Machine*, uintptr_t param);
typedef	psy_audio_ParamTranslator* (*fp_machine_inst_param_translator)(struct
	psy_audio_Machine*);
...
typedef uintptr_t(*fp_machine_param_selected)(struct psy_audio_Machine*);
typedef void (*fp_machine_select_param)(struct psy_audio_Machine*, uintptr_t index);
...
typedef	void (*fp_machine_set_editname)(struct psy_audio_Machine*, const char*
	name);
typedef void (*fp_machine_set_position)(struct psy_audio_Machine*, double x,
	double y);
typedef void (*fp_machine_position)(struct psy_audio_Machine*, double* x,
	double* y);
typedef	struct psy_audio_Buffer* (*fp_machine_buffer_memory)(struct
	psy_audio_Machine*);
typedef	uintptr_t (*fp_machine_buffer_memory_size)(struct psy_audio_Machine*);
typedef	void (*fp_machine_set_buffer_memory_size)(struct psy_audio_Machine*,
	uintptr_t);
typedef	psy_dsp_amp_range_t (*fp_machine_amp_range)(struct psy_audio_Machine*);
...
typedef uintptr_t(*fp_machine_num_aux_columns)(struct psy_audio_Machine*);
typedef uintptr_t (*fp_machine_aux_column_selected)(struct psy_audio_Machine*);
typedef void (*fp_machine_select_aux_column)(struct psy_audio_Machine*, uintptr_t index);
/* presets */
...
typedef	struct psy_audio_MachineFactory* (*fp_machine_machinefactory)(struct
	psy_audio_Machine*);
typedef void (*fp_machine_output)(const struct psy_audio_Machine*, const char* text);
typedef	bool(*fp_machine_editresize)(struct psy_audio_Machine*, intptr_t width, intptr_t height);
typedef	void (*fp_machine_buschanged)(struct psy_audio_Machine*);
typedef bool (*fp_machine_addcapture)(struct psy_audio_Machine*, intptr_t index);
typedef bool (*fp_machine_removecapture)(struct psy_audio_Machine*, intptr_t index);
typedef void (*fp_machine_readbuffers)(struct psy_audio_Machine*, intptr_t index,
	float** pleft, float** pright, uintptr_t numsamples);
typedef const char* (*fp_machine_capturename)(const struct psy_audio_Machine*,
	intptr_t index);
typedef uintptr_t (*fp_machine_numcaptures)(const struct psy_audio_Machine*);
typedef const char* (*fp_machine_playbackname)(const struct psy_audio_Machine*,
	intptr_t index);
typedef uintptr_t (*fp_machine_numplaybacks)(const struct psy_audio_Machine*);
typedef const char* (*fp_machine_language)(const struct psy_audio_Machine*);

typedef struct {
	/* the life cycle of a machine */
	psy_audio_fp_machine init;
	fp_machine_setcallback setcallback;
	psy_audio_fp_machine dispose;
	...
	psy_audio_fp_machine reload;
	/* connections */
	fp_machine_numinputs numinputs;
	fp_machine_numoutputs numoutputs;
	fp_machine_slot slot;
	fp_machine_set_slot setslot;
	fp_machine_amp_range amprange;
	/* panning */
	fp_machine_set_panning setpanning;
	fp_machine_panning panning;
	/* general information */
	fp_machine_info info;
	fp_machine_set_editname seteditname;
	...
	fp_machine_param_selected paramselected;
	fp_machine_select_param selectparam;
	...
	fp_machine_inst_param_translator instparamtranslator;
	/* data */
	...
	fp_machine_num_aux_columns numauxcolumns;
	fp_machine_aux_column_selected auxcolumnselected;
	fp_machine_select_aux_column selectauxcolumn;
	/* gui stuff */
	...
	fp_machine_set_position setposition;
	fp_machine_position position;
	fp_machine_sethostview sethostview;
	/* states */
	fp_machine_bypassed bypassed;
	psy_audio_fp_machine bypass;
	psy_audio_fp_machine unbypass;
	fp_machine_muted muted;
	psy_audio_fp_machine mute;
	psy_audio_fp_machine unmute;
	psy_audio_fp_machine standby;
	psy_audio_fp_machine deactivatestandby;
	fp_machine_has_standby hasstandby;
	fp_machine_isbus isbus;
	psy_audio_fp_machine setbus;
	psy_audio_fp_machine unsetbus;
	/* buffer memory */
	fp_machine_buffer_memory buffermemory;
	fp_machine_buffer_memory_size buffermemorysize;
	fp_machine_set_buffer_memory_size setbuffermemorysize;
	/* machine callbacks */
...
INLINE uintptr_t psy_audio_machine_num_outputs(const psy_audio_Machine* self)
{
...
INLINE bool psy_audio_machine_add_capture(psy_audio_Machine* self, intptr_t index)
{
	return self->vtable->addcapture(self, index);
}

INLINE bool psy_audio_machine_remove_capture(psy_audio_Machine* self, intptr_t index)
{
	return self->vtable->removecapture(self, index);
}

INLINE void psy_audio_machine_read_buffers(psy_audio_Machine* self, intptr_t index,
	float** pleft, float** pright, uintptr_t numsamples)
{
	self->vtable->readbuffers(self, index, pleft, pright, numsamples);
}

INLINE const char* psy_audio_machine_capture_name(const psy_audio_Machine* self, intptr_t
	index)
{
...
INLINE const char* psy_audio_machine_playback_name(const psy_audio_Machine* self, intptr_t
	index)
{
	return self->vtable->playbackname(self, index);
}

INLINE uintptr_t psy_audio_machine_num_playbacks(const psy_audio_Machine* self)
{
...
INLINE void psy_audio_machine_output(const psy_audio_Machine* self, const char* text)
{
