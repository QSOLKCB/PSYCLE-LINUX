	...
	psy_audio_MachineType type, const char* plugincatchername, uintptr_t instindex)
{
	...
	return psy_audio_machinefactory_make_machine_from_path(self, type,
		psy_audio_plugincatcher_module_path(self->catcher, type,
		self->loadnewgamefxblitz,
		plugincatchername, fullpath),
		psy_audio_plugincatcher_extract_shell_idx(plugincatchername),
		instindex);
}
...
{
	return psy_audio_machinefactory_make_machine_from_path(self,
		machineinfo->type,
...
psy_audio_Machine* psy_audio_machinefactory_make_machine_from_path(
	...
	case psy_audio_LUA: {
		...
		if (plugin) {
			...
			status = psy_audio_luaplugin_init(plugin, self->machinecallback,
				path, self->logger, (struct psy_ui_Component*)instindex);				
			if (status == PSY_OK || psy_audio_machine_info(
