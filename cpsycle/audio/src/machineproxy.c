...
#if defined DIVERSALIS__OS__MICROSOFT
static int FilterException(const psy_audio_MachineProxy* proxy, const char* msg, int code,
	struct _EXCEPTION_POINTERS* ep)
{
	char text[512];
	((psy_audio_MachineProxy*)proxy)->crashed = 1;
	...
	psy_audio_machine_output(psy_audio_machineproxy_base_const(proxy), text);
...
static void machineproxy_output(const psy_audio_MachineProxy*, const char* text);
...
static bool machineproxy_addcapture(psy_audio_MachineProxy*, intptr_t index);
static bool machineproxy_removecapture(psy_audio_MachineProxy*, intptr_t index);
static void machineproxy_readbuffers(psy_audio_MachineProxy*, intptr_t index,
	float** pleft, float** pright, uintptr_t numsamples);
static const char* machineproxy_capturename(psy_audio_MachineProxy*, intptr_t index);
static uintptr_t machineproxy_numcaptures(const psy_audio_MachineProxy*);
static const char* machineproxy_playbackname(const psy_audio_MachineProxy*, intptr_t index);
static uintptr_t machineproxy_numplaybacks(const psy_audio_MachineProxy*);
...
static void vtable_init(psy_audio_MachineProxy* self)
{
	if (!vtable_initialized) {
		...
		vtable.stop = (psy_audio_fp_machine)machineproxy_stop;
		vtable.dispose = (psy_audio_fp_machine)machineproxy_dispose;
		vtable.reload = (psy_audio_fp_machine)machineproxy_reload;
		...
		vtable.setpanning = (fp_machine_set_panning)machineproxy_setpanning;
		vtable.panning = (fp_machine_panning)machineproxy_panning;
		vtable.mute = (psy_audio_fp_machine)machineproxy_mute;
		vtable.unmute = (psy_audio_fp_machine)machineproxy_unmute;
		vtable.muted = (fp_machine_muted)machineproxy_muted;
		vtable.setbus = (psy_audio_fp_machine)machineproxy_setbus;
		vtable.unsetbus = (psy_audio_fp_machine)machineproxy_unsetbus;
		vtable.isbus = (fp_machine_isbus)machineproxy_isbus;
		vtable.bypass = (psy_audio_fp_machine)machineproxy_bypass;
		vtable.unbypass = (psy_audio_fp_machine)machineproxy_unbypass;
		vtable.bypassed = (fp_machine_bypassed)machineproxy_bypassed;
		vtable.standby = (psy_audio_fp_machine)machineproxy_standby;
		vtable.deactivatestandby = (psy_audio_fp_machine)machineproxy_deactivatestandby;
		vtable.hasstandby = (fp_machine_has_standby)machineproxy_hasstandby;
		...
		vtable.instparamtranslator = (fp_machine_inst_param_translator)
		...
		vtable.paramselected = (fp_machine_param_selected)
			paramselected;
		vtable.selectparam = (fp_machine_select_param)
			selectparam;
		...
		vtable.setslot = (fp_machine_set_slot)machineproxy_setslot;
		...
		vtable.setposition = (fp_machine_set_position)machineproxy_setposition;
		vtable.position = (fp_machine_position)machineproxy_position;
		vtable.seteditname = (fp_machine_set_editname)machineproxy_seteditname;
		vtable.editname = (fp_machine_editname)machineproxy_editname;
		vtable.buffermemory = (fp_machine_buffer_memory)machineproxy_buffermemory;
		vtable.buffermemorysize = (fp_machine_buffer_memory_size)
			machineproxy_buffermemorysize;
		vtable.setbuffermemorysize = (fp_machine_set_buffer_memory_size)
			machineproxy_setbuffermemorysize;
		vtable.amprange = (fp_machine_amp_range)machineproxy_amprange;
		...
		vtable.numauxcolumns = (fp_machine_num_aux_columns)numauxcolumns;
		vtable.selectauxcolumn = (fp_machine_select_aux_column)selectauxcolumn;
		vtable.auxcolumnselected = (fp_machine_aux_column_selected)auxcolumnselected;
...
void machineproxy_output(const psy_audio_MachineProxy* self, const char* text)
{	
#if defined DIVERSALIS__OS__MICROSOFT        
		__try
#endif		
...
bool machineproxy_addcapture(psy_audio_MachineProxy* self, intptr_t index)
{
	bool rv = 0;
...
bool machineproxy_removecapture(psy_audio_MachineProxy* self, intptr_t index)
{
	bool rv = 0;
...
void machineproxy_readbuffers(psy_audio_MachineProxy* self, intptr_t index,
	float**pleft, float** pright, uintptr_t numsamples)
{	
	if (self->crashed == 0) {
#if defined DIVERSALIS__OS__MICROSOFT        
...
const char* machineproxy_capturename(psy_audio_MachineProxy* self, intptr_t index)
{
	const char* rv = 0;
...
const char* machineproxy_playbackname(const psy_audio_MachineProxy* self, intptr_t index)
{
	const char* rv = 0;
...
uintptr_t machineproxy_numplaybacks(const psy_audio_MachineProxy* self)
{
	uintptr_t rv = 0;
