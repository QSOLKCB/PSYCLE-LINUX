...
typedef struct psy_audio_MachineProxy {
	...
	psy_audio_fp_machine machinedispose;
...
INLINE const psy_audio_Machine* psy_audio_machineproxy_base_const(
	const psy_audio_MachineProxy* self)
{
	return (const psy_audio_Machine*)&self->machine;
}
