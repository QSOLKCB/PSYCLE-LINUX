...
static int psy_audio_psy3loader_xm_inst_load(psy_audio_PSY3Loader*,
	psy_audio_Instrument*, bool islegacy, uint32_t legacyversion);
static int psy_audio_psy3loader_xm_inst_env_load(
	psy_audio_PSY3Loader*, psy_dsp_Envelope* envelope,
	bool legacy, uint32_t legacyversion);
static int psy_audio_psy3loader_xm_wav_load(psy_audio_PSY3Loader*,
	psy_audio_Sample* rv);
...
static int psy_audio_psy3loader_load_param_mapping(psy_audio_PSY3Loader*,
	psy_audio_Machine*);
static int psy_audio_psy3loader_load_is_machine_bus(psy_audio_PSY3Loader*,
	psy_audio_Machine*);
	...
	*/
	...
	for(i = 0; i < numInstruments && filepos < self->fp->currchunk.begins +
		...
		if (strcmp(temp,"INST")== 0) {
			...
			/* XMInstrument inst; */
			if ((status = psy_audio_psy3loader_xm_inst_load(self, 0, TRUE,
					lowversion))) {
				return status;
			}
			/*
		...
		self->fp->currchunk.begins + self->fp->currchunk.size;i++)
	{
		...
		if (strcmp(temp,"SMPD")== 0)
		{
			...
			if ((status = psy_audio_psy3loader_xm_wav_load(self, wave))) {
				return status;
			}
			if (!wave) {
				return PSY_ERRFILE;
...
{		
	...
	if ((status = psy_audio_psy3loader_xm_inst_load(self, instrument, 0,
...
int psy_audio_psy3loader_xm_wav_load(psy_audio_PSY3Loader* self, psy_audio_Sample* wave)
{
	int status;
	uint32_t size1;
	uint32_t size2;
...
int psy_audio_psy3loader_xm_inst_load(psy_audio_PSY3Loader* self,
...
{		
	...
	*/
	...
	assert(instrument);
	if ((status = psy_audio_psy3loader_xm_inst_env_load(self,
...
int psy_audio_psy3loader_xm_inst_env_load(psy_audio_PSY3Loader* self,
	...
	psy_audio_PSY3Loader* self, int32_t index)
{
	...
	if (psyfile_currchunkversion(self->fp) >= 2) {
		psy_audio_psy3loader_load_param_mapping(self, machine);
	}
	if (psyfile_currchunkversion(self->fp) >= 3) {
		psy_audio_psy3loader_load_is_machine_bus(self, machine);
	}
	return PSY_OK;
}
...
int psy_audio_psy3loader_load_param_mapping(psy_audio_PSY3Loader* self, 
	psy_audio_Machine* machine)
{
	uint8_t nummaps;
	uint8_t i;
...
int psy_audio_psy3loader_load_is_machine_bus(psy_audio_PSY3Loader* self,
	psy_audio_Machine* machine)
{
	uint8_t isbus;
	int status;
