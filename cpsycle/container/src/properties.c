	...
	source)
{
	...
	self->id = source->id;	
	psy_strreset(&self->icon_svg, source->icon_svg);
...
psy_Property* psy_property_set_svg(psy_Property* self, const char* text)
{
	assert(self);

	psy_strreset(&self->item.icon_svg, text);
	return self;
}

const char* psy_property_svg(const psy_Property* self)
{	
	assert(self);

	return (psy_strlen(self->item.icon_svg) != 0)
		? self->item.icon_svg		
		: "";
}
