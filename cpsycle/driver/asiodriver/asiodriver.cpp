...
bool ASIOInterface::AddCapturePort(uintptr_t idx)
{
	...
	if (_portMapping.size() <= idx) {
		uintptr_t oldsize = _portMapping.size();
...
static intptr_t numplaybacks(const psy_AudioDriver*);
static intptr_t addcaptureport(const AsioDriver* self, intptr_t idx);
static intptr_t removecaptureport(const AsioDriver* self, intptr_t idx);
...
static void readbuffers(AsioDriver* self, intptr_t idx,
...
void readbuffers(AsioDriver* self, intptr_t idx, float** left, float** right,
...
{
	const AsioDriver* self;
	
	self = (const AsioDriver*)driver;	
...
intptr_t numplaybacks(const psy_AudioDriver* driver)
{
	AsioDriver* self = (AsioDriver*)driver;
	std::vector<std::string> ports;
	self->asioif->GetPlaybackPorts(ports);
...
intptr_t removecaptureport(const AsioDriver* self, intptr_t idx)
{
	return self->asioif->RemoveCapturePort(idx);
}

intptr_t addcaptureport(const AsioDriver* self, intptr_t idx)
{
