...
{	
	...
	self->bitdepth_ = (validbitdepth == 24)
		? 32
		: validbitdepth;
	self->framebytes_ = (self->channelmode_ == psy_AUDIODRIVERCHANNELMODE_STEREO)
		? (self->bitdepth_ >> 2)
		: (self->bitdepth_ >> 3);
}
...
INLINE void psy_audiodriversettings_set_block_bytes(psy_AudioDriverSettings* self,
	uintptr_t value)
{
	assert(self);

	psy_audiodriversettings_setblockframes(self, value / self->framebytes_);
}
...
INLINE void psy_audiodriversettings_enable_dither(psy_AudioDriverSettings* self)
{
	self->dither_ = TRUE;
}

INLINE void psy_audiodriversettings_disable_dither(psy_AudioDriverSettings* self)
{
	self->dither_ = FALSE;
}
