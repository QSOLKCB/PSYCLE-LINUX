...
static void output_error(psy_EventDriver*, const char* msg);
...
void output_error(psy_EventDriver* self, const char* msg)
{
	assert(self);
...
{		
	...
	for (i = 0; i < n; ++i) {
		...
		utf8str = psy_dos_to_utf8(text, NULL);
		psy_property_append_int(devices, utf8str, i, 0, 0);
		free(utf8str);
		utf8str = NULL;
	}
}
...
int driver_open(psy_EventDriver* driver)
{
	...
	// if (self->deviceid != 0) {
		if (midiInOpen (&self->hMidiIn, self->deviceid, (DWORD_PTR)MidiCallback,
			...
			output_error(driver, "Cannot open MIDI device");
			success = 0;
		} else if (midiInStart(self->hMidiIn)) {
			output_error(driver, "Cannot start MIDI device");
			success = 0;
		}
	// }	         
	if (success == 1) {
		if (self->driver.logger) {
			psy_logger_output(self->driver.logger, "Mme Mididriver started\n");
		}
	}
	return success;
}
...
int driver_close(psy_EventDriver* driver)
{
	...
	if (self->deviceid != 0 && self->hMidiIn && midiInClose(self->hMidiIn)) {
		output_error(driver, "Cannot close MIDI device");
		success = 0;
	}
	self->hMidiIn = NULL;
	return success;
