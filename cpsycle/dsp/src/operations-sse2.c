...
void dsp_movmul(float *src, float *dst, intptr_t num, float multi)
{	
#if defined DIVERSALIS__CPU__X86__SSE && defined DIVERSALIS__COMPILER__FEATURE__XMM_INTRINSICS
	const __m128 volps = _mm_set_ps1(multi);
	const __m128* psrc = (__m128*)src;
	if ((ptrdiff_t)(dst) & 0xF) {
		while (num > 3)
		{
		...
		if (num > 0) {
			for (int i = 0; i < num; ++i) dst[i] = src[i] * multi;
		}
	}
	else {
		while (num > 0)
		{
			_mm_store_ps(dst, _mm_mul_ps(*psrc, volps));
			psrc++;
			dst += 4;
			num -= 4;
		}
