...
void psy_dsp_quantize16withdither(float *pin, int *piout, int c)
{
	...
	do
	{
		double res = ((double)pin[1] + psy_dsp_frand()) + d2i;
		int r = *(int*)&res;
		...
		res = ((double)pin[0] + psy_dsp_frand()) + d2i;
		int l = *(int*)&res;
...
void psy_dsp_quantize24in32bit(float *pin, int *piout, int c)
{
	...
	maxval = ((1u << ((sizeof(int32_t) << 3) - 1)) - 100);
	minval = (-maxval - 1);
	for (i = 0; i < c; ++i) {		
		*piout++ = (int)psy_dsp_clip_float((float)(minval), (*pin++) * 65536.0f, (float)(maxval));
		*piout++ = (int)psy_dsp_clip_float((float)(minval), (*pin++) * 65536.0f, (float)(maxval));		
	}	
}
...
void psy_dsp_quantize24be(float *pin, int *piout, int c)
{
	...
	for(int i = 0; i < c; ++i) {
		...
		outval = psy_dsp_rint32_clip_bits((*pin++) * 256.0f, 24);
