...
{
	memset(&self->st, 0, sizeof(struct stat));
...
void psy_path_init_file(psy_Path* self, const char* path)
{
	memset(&self->st, 0, sizeof(struct stat));
	self->st.st_mode = S_IFREG;
	self->path = strdup(path ? path : "");
	self->name = strdup("");
	self->prefix = strdup("");
	self->ext = strdup("");
...
void psy_path_init_dir(psy_Path* self, const char* path)
{
	memset(&self->st, 0, sizeof(struct stat));
	self->st.st_mode = S_IFDIR;
	self->name = strdup("");
	self->prefix = strdup("");
	self->ext = strdup("");
	self->filename = strdup("");
...
void psy_path_clear(psy_Path* self)
{
	memset(&self->st, 0, sizeof(struct stat));
	psy_strreset(&self->path, "");
	psy_strreset(&self->name, "");
	psy_strreset(&self->prefix, "");
	psy_strreset(&self->ext, "");
	psy_strreset(&self->filename, "");
}

void psy_path_copy(psy_Path* self, const psy_Path* other)
{
	assert(self);
	assert(other);

	self->st = other->st;
	psy_strreset(&self->path, other->path);
	psy_strreset(&self->name, other->name);
	psy_strreset(&self->prefix, other->prefix);
	psy_strreset(&self->ext, other->ext);
	psy_strreset(&self->filename, other->filename);
}
...
void psy_dir_find_file(const char* searchpath, const char* wildcard,
	char* filepath)
{
	filepath[0] = '\0';
...
int psy_remove(const char* path)
{
#if defined _MSC_VER
	return remove(path);
#elif defined __GNUC__
...
bool psy_dir_exists(const char* path)
{
#if defined _MSC_VER
...
bool psy_file_readable(const char* fname)
{
	FILE* file;
