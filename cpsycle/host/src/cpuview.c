...
static void cpuview_on_timer(CPUView*, psy_ui_Component* sender,
	uintptr_t timerid);
static void cpuview_on_cpu_performance(CPUView*, psy_ui_Button* sender);
...
{	
	...
	psy_ui_component_set_default_align(&self->performance, psy_ui_ALIGN_TOP,
		self->topmargin);	
	psy_ui_button_init_check_connect(&self->cpucheck, &self->performance,
		"cpu.performance", self, cpuview_on_cpu_performance);
	if (psy_audio_player_measuring_cpu_usage(workspace_player(self->workspace))) {		
		psy_ui_button_highlight(&self->cpucheck);
	}
...
void cpuview_on_cpu_performance(CPUView* self, psy_ui_Button* sender)
{
	assert(self);
