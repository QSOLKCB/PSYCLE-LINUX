...
static void envelopebox_on_draw(EnvelopeBox*, psy_ui_Graphics*);
static void envelopebox_draw_grid(EnvelopeBox*, psy_ui_Graphics*);
static void envelopebox_draw_points(EnvelopeBox*, psy_ui_Graphics*);
static void envelopebox_draw_lines(EnvelopeBox*, psy_ui_Graphics*);
static void envelopebox_draw_ruler(EnvelopeBox*, psy_ui_Graphics*);
static void envelopebox_on_size(EnvelopeBox*);
static void envelopebox_on_mouse_down(EnvelopeBox*, psy_ui_MouseEvent*);
static void envelopebox_on_mouse_move(EnvelopeBox*, psy_ui_MouseEvent*);
...
void envelopebox_on_draw(EnvelopeBox* self, psy_ui_Graphics* g)
{	
	assert(self);

	envelopebox_draw_grid(self, g);
	envelopebox_draw_ruler(self, g);
	envelopebox_draw_lines(self, g);	
	envelopebox_draw_points(self, g);	
}

void envelopebox_draw_grid(EnvelopeBox* self, psy_ui_Graphics* g)
{
	psy_dsp_seconds_t i;
...
void envelopebox_draw_points(EnvelopeBox* self, psy_ui_Graphics* g)
{
	psy_List* p;
...
void envelopebox_draw_lines(EnvelopeBox* self, psy_ui_Graphics* g)
{
	psy_List* p;
...
void envelopebox_draw_ruler(EnvelopeBox* self, psy_ui_Graphics* g)
{
	psy_dsp_seconds_t t;
...
void envelopebox_on_size(EnvelopeBox* self)
{
...
void envelopebox_on_mouse_move(EnvelopeBox* self, psy_ui_MouseEvent* ev)
{	
	assert(self);
...
/* prototypes */
void envelopebar_enable_milli_secs(EnvelopeBar*);
void envelopebar_enableticks(EnvelopeBar*);
void envelopebar_on_milli_secs(EnvelopeBar*, psy_ui_Component* sender);
...
void envelopebar_init(EnvelopeBar* self, psy_ui_Component* parent)
{
	...
	psy_ui_button_init_check(&self->enabled, &self->component, "Envelope");	
	psy_ui_component_set_margin(psy_ui_button_base(&self->enabled), tab);
	psy_ui_component_set_tab_index(psy_ui_button_base(&self->enabled), 0);
	psy_ui_button_init_check(&self->carry, &self->component, "Carry (continue)");	
	psy_ui_component_set_margin(psy_ui_button_base(&self->carry), tab);
	psy_ui_component_set_tab_index(psy_ui_button_base(&self->carry), 0);
	psy_ui_button_init_text_connect(&self->millisec, &self->component, "Millisecs",
		self, envelopebar_on_milli_secs);	
	psy_ui_component_set_tab_index(psy_ui_button_base(&self->millisec), 0);
	...
	psy_ui_component_set_tab_index(psy_ui_button_base(&self->ticks), 0);
	psy_ui_button_init_text(&self->adsr, &self->component, "ADSR");
	psy_ui_button_allow_right_click(&self->adsr);
	psy_ui_component_set_tab_index(psy_ui_button_base(&self->adsr), 0);
	envelopebar_enable_milli_secs(self);
}

void envelopebar_settext(EnvelopeBar* self, const char* text)
{
	psy_ui_button_set_text(&self->enabled, text);	
}

void envelopebar_enable_milli_secs(EnvelopeBar* self)
{
...
void envelopebar_on_milli_secs(EnvelopeBar* self, psy_ui_Component* sender)
{
	envelopebar_enable_milli_secs(self);
}
...
static void envelopeview_on_zoom(EnvelopeView*, ScrollZoom* sender);
static void envelopeview_on_predefs(EnvelopeView* self, psy_ui_Button* sender);
static void envelopeview_on_enable(EnvelopeView*, psy_ui_Button* sender);
static void envelopeview_on_carry(EnvelopeView*, psy_ui_Button* sender);
static void envelopeview_on_millisecs(EnvelopeView*, psy_ui_Button* sender);
static void envelopeview_on_ticks(EnvelopeView*, psy_ui_Button* sender);
static void envelopeview_on_tweaked(EnvelopeView*, EnvelopeBox* sender,
	...
	psy_dsp_Envelope* settings)
{
	...
	if (settings && psy_dsp_envelope_is_enabled(settings)) {
		psy_ui_button_highlight(&self->bar.enabled);
	} else {
		psy_ui_button_disable_highlight(&self->bar.enabled);
	}
	if (settings && psy_dsp_envelope_is_carry(settings)) {
		psy_ui_button_highlight(&self->bar.carry);
	} else {
		psy_ui_button_disable_highlight(&self->bar.carry);
	}
...
void envelopeview_on_zoom(EnvelopeView* self, ScrollZoom* sender)
{
	envelopebox_setzoom(&self->envelopebox, scrollzoom_start(sender),
		scrollzoom_end(sender));
}

void envelopeview_on_enable(EnvelopeView* self, psy_ui_Button* sender)
{
	if (self->envelopebox.settings) {		
		psy_dsp_envelope_set_enabled(self->envelopebox.settings,
			!psy_ui_button_highlighted(sender));
		if (self->envelopebox.settings->enabled) {
...
void envelopeview_on_carry(EnvelopeView* self, psy_ui_Button* sender)
{
	if (self->envelopebox.settings) {
		psy_dsp_envelope_set_carry(self->envelopebox.settings,
			!psy_ui_button_highlighted(sender));
		if (self->envelopebox.settings->carry) {
			psy_ui_button_highlight(sender);
		}
		else {
			psy_ui_button_disable_highlight(sender);
...
void envelopeview_on_millisecs(EnvelopeView* self, psy_ui_Button* sender)
{
	if (self->envelopebox.settings) {
...
void envelopeview_on_ticks(EnvelopeView* self, psy_ui_Button* sender)
{
	if (self->envelopebox.settings) {
...
void envelopeview_on_predefs(EnvelopeView* self, psy_ui_Button* sender)
{		
	/*if (self->envelopebox.settings && psy_ui_button_click_state(sender) == 1) {
...
void envelopeview_on_tweaked(EnvelopeView* self, EnvelopeBox* sender,
	uintptr_t pointindex)
{
