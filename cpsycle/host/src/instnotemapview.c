...
{
	psy_ui_edit_init(&self->input, parent);
	self->entry = entry;
	self->column = column;
	self->state = state;
	psy_ui_edit_set_char_number(&self->input, COLWIDTH);	
	psy_ui_edit_enable_input_field(&self->input);
...
void instrumententryedit_read(InstrumentEntryEdit* self)
{
	...
	psy_ui_edit_set_text(&self->input, text);
}
	...
	psy_ui_Component* sender)
{
	...
	case 2:
		...
		break;
	case 3:		
		if (strcmp(psy_ui_edit_text(&self->input), "-") == 0) {
...
{
	return (intptr_t)atoi(psy_ui_edit_text(&self->input));
}
