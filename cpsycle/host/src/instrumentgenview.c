...
{	
	...
	psy_ui_component_set_default_align(&self->fitrow, psy_ui_ALIGN_LEFT,
		psy_ui_defaults_hmargin(psy_ui_defaults()));
	psy_ui_button_init_check_connect(&self->fitrowcheck, & self->fitrow,
		"instview.play-sample-to-fit",
		self, instrumentgeneralview_onfitrow);
	psy_ui_edit_init(&self->fitrowedit, &self->fitrow);
	psy_ui_edit_set_char_number(&self->fitrowedit, 4);
...
{	
	...
	if (instrument) {
		...
		if (self->instrument->loop) {
			psy_ui_button_highlight(&self->fitrowcheck);
		} else {
			psy_ui_button_disable_highlight(&self->fitrowcheck);
		}
		psy_snprintf(text, 128, "%d", (int)self->instrument->lines);
		psy_ui_edit_set_text(&self->fitrowedit, text);
...
{
	if (self->instrument) {
		if (psy_ui_button_highlighted(&self->fitrowcheck)) {
			psy_ui_button_disable_highlight(&self->fitrowcheck);
