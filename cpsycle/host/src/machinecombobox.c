...
{	
	if (self->sync_machines_select) {
		...
		slot = (size_t)psy_table_at(&self->comboboxslots, psy_ui_combobox_cur_sel(sender));
...
void machinecombobox_select(MachineComboBox* self, uintptr_t slot)
{
	...
	if (psy_table_exists(&self->slotscombobox, slot)) {
		comboboxindex = (uintptr_t)psy_table_at(&self->slotscombobox, slot);
	} else {
		if (psy_ui_combobox_count(&self->machinebox) > 0) {
			comboboxindex = psy_ui_combobox_count(&self->machinebox) - 1;
		} else {
			comboboxindex = psy_INDEX_INVALID;
		}
	}	
