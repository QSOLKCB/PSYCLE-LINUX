	...
	MachineStackView* view, MachineStackState* state)
{
	...
	psy_ui_component_set_align(&self->effects.component,
		psy_ui_ALIGN_TOP);
	psy_ui_button_set_svg(&self->effects,
	...
	psy_ui_button_set_svg(&self->volumes, psy_ui_app_svg(psy_ui_app(), "img.icon-less"));
	...
	psy_ui_button_set_svg(&self->outputs, psy_ui_app_svg(psy_ui_app(), "img.icon-less"));
	...
	psy_ui_Button* sender)
{
	...
	if (psy_ui_component_visible(&self->view->outputs.component)) {		
		...
		psy_ui_button_set_svg(&self->outputs, psy_ui_app_svg(psy_ui_app(), "img.icon-more"));
	}
}
	...
	psy_ui_Button* sender)
{
	...
	if (psy_ui_component_visible(&self->view->volumes.component)) {
		...
		psy_ui_button_set_svg(&self->volumes, psy_ui_app_svg(psy_ui_app(), "img.icon-more"));
	}
}
...
/* prototypes */
static void machinestackinputs_on_preferred_size(MachineStackInputs*,
...
static void machinestackinputs_on_mouse_up(MachineStackInputs*,
	psy_ui_MouseEvent*);
static bool machinestackinputs_on_input(MachineStackInputs*, InputHandler*);
...
static void machinestackinputs_vtable_init(MachineStackInputs* self)
{
	if (!machinestackinputs_vtable_initialized) {
		...
		machinestackinputs_vtable.onpreferredsize =
			...
			machinestackinputs_on_preferred_size;
	...
	psy_ui_Component* parent, MachineStackState* state, Workspace* workspace)
{
	...
	self->state = state;
	inputhandler_connect(psy_ui_app_input_handler(psy_ui_app()), INPUTHANDLER_FOCUS,
		psy_EVENTDRIVER_CMD, "tracker", psy_INDEX_INVALID,
		self, NULL, (fp_inputhandler_input)machinestackinputs_on_input);
}

void machinestackinputs_on_preferred_size(MachineStackInputs* self,
	const psy_ui_Size* limit, psy_ui_Size* rv)
{
...
bool machinestackinputs_on_input(MachineStackInputs* self, InputHandler* sender)
{
	assert(self);

	switch (sender->cmd_.id) {
	case CMD_NAVSELECT:
		workspace_select_view(self->workspace, viewindex_make_section(VIEW_ID_MACHINES,
...
void machinestackoutputs_build(MachineStackOutputs* self)
{
	...
	if (self->state->machines) {
		...
		for (i = 0; i < maxnumcolumns; ++i) {
			...
			column = machinestackstate_column(self->state, i);
			knobui = knobui_alloc_init(&self->component,
