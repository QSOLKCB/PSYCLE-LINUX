	...
	psy_ui_Component* parent)
{
	...
	psy_ui_component_set_align(psy_ui_button_base(&self->ok), psy_ui_ALIGN_LEFT);
	psy_ui_component_set_tab_index(psy_ui_button_base(&self->ok), 0);
	...
	psy_ui_component_set_tab_index(psy_ui_button_base(&self->cancel), 0);
}
...
static void machineconnectionsmenu_append(MachineConnectionsMenu*, const char* str,
...
void machineconnectionsmenu_append(MachineConnectionsMenu* self, const char* str,
	uintptr_t idx, psy_audio_Wire wire)
{
	...
	button = psy_ui_button_alloc_init(&self->pane);
...
static void machineconnecttomenu_append(MachineConnectToMenu*, const char* str,
...
void machineconnecttomenu_append(MachineConnectToMenu* self, const char* str,
	uintptr_t dst)
{
	psy_ui_Button* button;
	...
	MachineMenuState* state)
{
	...
	psy_ui_button_set_svg(&self->hide, psy_ui_app_svg(psy_ui_app(), "img.icon-close")); 
	...
	psy_ui_edit_init(&self->title, &self->client);
	psy_ui_edit_set_char_number(&self->title, 32);
	...
	psy_ui_edit_enable_input_field(&self->title);
}
...
{	
	...
	if (machine) {
		psy_ui_edit_set_text(&self->title,
			psy_audio_machine_edit_name(machine));
	...
	WireFrames* wireframes)
{
	...
	psy_ui_component_set_tab_index(psy_ui_button_base(&self->bank), 0);
	...
	psy_ui_component_set_tab_index(psy_ui_button_base(&self->connect), 0);
	psy_ui_button_init_text_connect(&self->connections, &self->pane,
		"mvmenu.connections",
		self, machinemenu_on_connections);
	psy_ui_component_set_tab_index(psy_ui_button_base(&self->connections), 0);
	...
	psy_ui_component_set_tab_index(psy_ui_button_base(&self->replace), 0);
	...
	psy_ui_component_set_tab_index(psy_ui_button_base(&self->insertbefore), 0);
	psy_ui_button_init_text_connect(&self->insertafter, &self->pane,
		"mvmenu.insertafter", self, machinemenu_on_machine_insert_after);
	psy_ui_component_set_tab_index(psy_ui_button_base(&self->insertafter), 0);
