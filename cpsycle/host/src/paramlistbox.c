...
static void parameterlistbox_on_listbox_selected(ParameterListBox*,
...
{	
	...
	psy_signal_connect(&self->listbox.signal_selchanged, self,
		parameterlistbox_on_listbox_selected);		
...
void parameterlistbox_on_listbox_selected(ParameterListBox* self,
	psy_ui_ListBox* sender)
{
	uintptr_t slot;
	uintptr_t paramindex;
			
	slot = psy_ui_listbox_cur_sel(sender);
	if (self->machine && slot < psy_audio_machine_num_tweak_parameters(
			self->machine)) {
		paramindex = slot;
	} else {
		paramindex = psy_INDEX_INVALID;
...
uintptr_t parameterlistbox_selected(ParameterListBox* self)
{
