...
/* prototypes */
static void pianogrid_connect_input_handler(Pianogrid*);
...
static void pianogrid_vtable_init(Pianogrid* self)
{
	...
	if (!pianogrid_vtable_initialized) {
		...
		pianogrid_vtable.on_focus =
			(psy_ui_fp_component)
			pianogrid_on_focus;
		pianogrid_vtable.on_focus_lost =
			(psy_ui_fp_component)
	...
	psy_DiskOp* disk_op)
{
	...
	pianogrid_connect_input_handler(self);
...
void pianogrid_connect_input_handler(Pianogrid* self)
{
	...
	inputhandler_connect(psy_ui_app_input_handler(psy_ui_app()),
		...
		self, NULL, (fp_inputhandler_input)pianogrid_on_edt_cmds);
	inputhandler_connect(psy_ui_app_input_handler(psy_ui_app()),
		INPUTHANDLER_FOCUS, psy_EVENTDRIVER_CMD, "pianoroll", psy_INDEX_INVALID,
		self, NULL, (fp_inputhandler_input)pianogrid_on_roll_cmds);
	inputhandler_connect(psy_ui_app_input_handler(psy_ui_app()),
		INPUTHANDLER_FOCUS, psy_EVENTDRIVER_CMD, "notes", psy_INDEX_INVALID,
		self, NULL, (fp_inputhandler_input)pianogrid_on_note_cmds);
}
