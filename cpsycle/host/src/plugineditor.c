	...
	psy_ui_Component* parent, Workspace* workspace)
{
	...
	psy_ui_edit_set_text(&self->name, "newplugin");
...
static void plugineditor_init_view_buttons(PluginEditor*);
...
static void plugineditor_on_plugin_selected(PluginEditor*, psy_ui_ComboBox* sender);
static void plugineditor_on_file_selected(PluginEditor*, psy_ui_ComboBox* sender);
...
static void plugineditor_on_float(PluginEditor*, psy_ui_Button* sender);
...
{	
	...
	self->workspace = workspace;
	self->basepath = NULL;	
	self->instance = psy_INDEX_INVALID;
	self->zoom = 1.0;
...
{
	titlebar_init(&self->title_bar_, &self->component, "machinebar.editor");
	...
	plugineditor_init_view_buttons(self);
}

void plugineditor_init_view_buttons(PluginEditor* self)
{
	assert(self);
	...
	psy_ui_button_init_resource_connect(&self->view_float, &self->view_buttons_client,
		"img.float", self, plugineditor_on_float);
}
	...
	psy_audio_Machines* machines, uintptr_t slot)
{
	...
	if (machine && psy_audio_machine_info(machine) &&
		...
		if (path) {
			if (self->basepath == NULL || (strcmp(path, self->basepath) != 0)) {
				psy_ui_editor_load(&self->editor, path);
				psy_strreset(&self->basepath, path);
			}
			self->instance = slot;
		}		
	}
...
{
	if (workspace_song(self->workspace) && self->instance) {
		psy_audio_Machine* machine;
...
{	
	psy_audio_Plugins* plugins;
	psy_List* p;
	uintptr_t i;

	plugins = &workspace->player_.plugincatcher.plugins_;
	psy_ui_combobox_clear(&self->pluginselector);				
	for (i = 0, p = psy_audio_plugins_begin(plugins); p != NULL; ++i, p = p->next) {
		const psy_audio_MachineInfo* machine_info;

		machine_info = (const psy_audio_MachineInfo*)psy_list_entry_const(p);
		assert(machine_info);
		if (machine_info->type == psy_audio_LUA) {
			if (psy_strlen(machine_info->modulepath)) {								
				psy_ui_combobox_set_item_id(&self->pluginselector,
					psy_ui_combobox_add_text(&self->pluginselector, machine_info->name),
					i);
			} else {				
				psy_ui_combobox_set_item_id(&self->pluginselector,
					psy_ui_combobox_add_text(&self->pluginselector, "path missing"),
					psy_INDEX_INVALID);
			}				
		}
	}	
...
void plugineditor_on_plugin_selected(PluginEditor* self, psy_ui_ComboBox*
	sender)
{
	const psy_audio_MachineInfo* machine_info;

	machine_info = psy_audio_plugins_at_const(&self->workspace->player_.plugincatcher.plugins_,
		psy_ui_combobox_item_id(sender, psy_ui_combobox_cur_sel(sender)));		
	if (machine_info) {
		if (machineinfo_module_path(machine_info)) {
			if (self->basepath == 0 || (strcmp(machineinfo_module_path(
					machine_info), self->basepath) != 0)) {				
				psy_ui_editor_clear(&self->editor);
				psy_ui_editor_load(&self->editor, machineinfo_module_path(
					machine_info));
				psy_strreset(&self->basepath, machineinfo_module_path(
					machine_info));
				self->instance = psy_INDEX_INVALID;
				plugineditor_build_file_list(self);
				psy_ui_combobox_select(&self->fileselector, 1);
				plugineditor_on_file_selected(self, &self->fileselector);
			}
		}
	}	
}

void plugineditor_on_file_selected(PluginEditor* self, psy_ui_ComboBox* sender)
{
	char path[4096];
...
void plugineditor_build_file_list(PluginEditor* self)
{
	...
	if (self->basepath) {
		...
		psy_path_init(&extract, self->basepath);
		psy_ui_combobox_clear(&self->fileselector);
		psy_ui_combobox_add_text(&self->fileselector, self->basepath);		
		psy_snprintf(path, 4096, "%s\\%s", psy_path_prefix(&extract),
			psy_path_name(&extract));
		psy_dir_enumerate_recursive(self, path, "*.lua", psy_audio_PLUGIN,
			(psy_fp_findfile)plugineditor_on_enum_dir);
	...
	psy_ui_Component* sender)
{
	...
	if (!psy_dir_exists(psy_path_full(&plugindir))) {
...
void plugineditor_on_float(PluginEditor* self, psy_ui_Button* sender)
{
	assert(self);

	plugineditor_base(self)->splitter_ = self->splitter_;
	plugineditor_base(self)->frame_icons_ = titlebar_base(&self->title_bar_);	
	plugineditor_base(self)->kbd_ = workspace_kbd_driver(self->workspace);	
	psy_ui_component_float(plugineditor_base(self));	
}
