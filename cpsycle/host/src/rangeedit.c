...
void rangeedit_init(RangeEdit* self, psy_ui_Component* parent)
{
	...
	psy_ui_edit_init(&self->edit_from, rangeedit_base(self));	
	psy_ui_edit_set_char_number(&self->edit_from, 5);
	psy_ui_edit_enable_input_field(&self->edit_from);
	psy_ui_label_init(&self->to, rangeedit_base(self));
	psy_ui_label_set_text(&self->to, "to");
	psy_ui_edit_init(&self->edit_to, rangeedit_base(self));
	psy_ui_edit_set_char_number(&self->edit_to, 5);
	psy_ui_edit_enable_input_field(&self->edit_to);
}
	...
	psy_Property* to)
{
	...
	psy_ui_edit_exchange(&self->edit_from, from);
	psy_ui_edit_exchange(&self->edit_to, to);
}
