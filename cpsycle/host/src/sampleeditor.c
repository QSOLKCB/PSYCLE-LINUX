	...
	uintptr_t selectionstart, uintptr_t selectionend)
{
	...
	psy_ui_edit_enable(&self->selstartedit);
	psy_ui_edit_enable(&self->selendedit);
	psy_snprintf(text, 128, "%u", (unsigned int)selectionstart);
	psy_ui_edit_set_text(&self->selstartedit, text);
	psy_snprintf(text, 128, "%u", (unsigned int)selectionend);
	psy_ui_edit_set_text(&self->selendedit, text);
}

void sampleeditorbar_clearselection(SampleEditorBar* self)
{
	psy_ui_edit_prevent(&self->selstartedit);
	psy_ui_edit_prevent(&self->selendedit);
	psy_ui_edit_set_text(&self->selstartedit, "");
	psy_ui_edit_set_text(&self->selendedit, "");
}
...
void sampleeditoroperations_initalign(SampleEditorOperations* self)
{
	...
	psy_list_free(psy_ui_components_setalign(
		psy_ui_component_children(&self->component, psy_ui_NONE_RECURSIVE, FALSE),
...
static void sampleeditorprocessview_onprocessorselected(
	SampleEditorProcessView*, psy_ui_ListBox* sender);
...
{
	psy_ui_notebook_select(&self->notebook, psy_ui_listbox_cur_sel(sender));
}
...
void samplebox_setquality(SampleBox* self, psy_dsp_ResamplerQuality quality)
{
	...
	for (it = psy_table_begin(&self->waveboxes);
		...
		wavebox = (WaveBox*)psy_tableiterator_value(&it);
		wavebox_set_quality(wavebox, quality);
	}	
}
