	...
	SeqEditState* state)
{
	...
	psy_ui_edit_set_char_number(&self->trackedit, 30);
	psy_ui_edit_enable_input_field(&self->trackedit);	
	...
	psy_ui_button_set_svg(&self->view_float, psy_ui_app_svg(psy_ui_app(), "img.float"));
	psy_ui_component_set_align(psy_ui_button_base(&self->view_float),
		psy_ui_ALIGN_LEFT);
	/* expand */
	psy_ui_button_init(&self->expand, &self->alignbar);
	psy_ui_button_set_svg(&self->expand, psy_ui_app_svg(psy_ui_app(), "img.expand"));
	...
	psy_ui_button_set_svg(&self->timesig, psy_ui_app_svg(psy_ui_app(), "img.icon-more"));
	/* loop */
	psy_ui_button_init_text(&self->loop, &self->component, "seqedit.repetitions");
	psy_ui_button_set_svg(&self->loop, psy_ui_app_svg(psy_ui_app(), "img.icon-more"));
...
void seqeditortoolbar_update_track_name(SeqEditToolBar* self)
{
	...
	if (track) {
		psy_ui_edit_set_text(&self->trackedit, psy_audio_sequencetrack_name(
			track));
	} else {
		psy_ui_edit_set_text(&self->trackedit, "");
	}
}
	...
	psy_ui_Edit* sender)
{
	...
	track = psy_audio_sequence_track_at(sequence, editposition.track);
	if (track) {
		psy_audio_sequencetrack_set_name(track, psy_ui_edit_text(sender));
