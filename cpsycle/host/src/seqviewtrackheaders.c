...
void seqviewtrackheaders_build(SeqviewTrackHeaders* self)
{
	...
	if (self->state->cmds->sequence) {
		...
		newtrack = psy_ui_button_alloc_init(&self->client);
		if (newtrack) {
...
{	
	switch (self->state->cmd.cmd) {
	case SEQLVCMD_NEWTRACK:
		sequencecmds_append_track(self->state->cmds);
		break;
	case SEQLVCMD_DELTRACK:
		sequencecmds_remove_track(self->state->cmds, self->state->cmd.track);
		break;
	default:
		break;
	}
	self->state->cmd = seqviewcmd_make(SEQLVCMD_NONE, psy_INDEX_INVALID);
}

void seqviewtrackheaders_on_new_track(SeqviewTrackHeaders* self,
	psy_ui_Button* sender)
{	
	self->state->cmd = seqviewcmd_make(SEQLVCMD_NEWTRACK, psy_INDEX_INVALID);
}

void seqviewtrackheaders_on_del_track(SeqviewTrackHeaders* self,
	TrackBox* sender)
{
	self->state->cmd = seqviewcmd_make(SEQLVCMD_DELTRACK, trackbox_trackindex(
		sender));	
}
