...
void waveboxcontext_set_zoom(WaveBoxContext* self, double zoomleft,
	double zoomright)
{
...
static void wavebox_on_size(WaveBox*);
...
{
	waveboxcontext_set_zoom(&self->context, zoomleft, zoomright);		
	psy_ui_component_invalidate(&self->component);	
}
...
void wavebox_on_size(WaveBox* self)
{	
	psy_ui_Size size;
...
void wavebox_set_quality(WaveBox* self, psy_dsp_ResamplerQuality quality)
{
	psy_dsp_multiresampler_set_quality(&self->context.resampler, quality);
	/* inits (sinc) resampler data */
	/*! @todo: sinc looks like zero hold */
	psy_dsp_resampler_setspeed(psy_dsp_multiresampler_base(
		&self->context.resampler), 1.0);
	psy_ui_component_invalidate(&self->component);
}
