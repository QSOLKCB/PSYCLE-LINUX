...
static psy_List* dev_children(psy_ui_win_ComponentImp*, int recursive, int visible);
...
psy_List* dev_children(psy_ui_win_ComponentImp* self, int recursive, int visible)
{
	psy_List* rv = 0;
	psy_List* p = 0;
		...
		if (hwnd) {
			...
			if (child) {				
				...
				if (recursive == psy_ui_RECURSIVE) {
					...
					q = psy_ui_component_children(child, recursive, visible);
