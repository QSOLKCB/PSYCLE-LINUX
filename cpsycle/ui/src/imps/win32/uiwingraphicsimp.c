...
static void psy_ui_win_g_imp_drawsolidpolygon(psy_ui_win_GraphicsImp*, psy_ui_RealPoint*,
	uintptr_t numpoints, psy_ui_Colour inner, psy_ui_Colour outter);
...
void psy_ui_win_g_imp_drawsolidpolygon(psy_ui_win_GraphicsImp* self,
	psy_ui_RealPoint* pts, uintptr_t numpoints, psy_ui_Colour inner,
	psy_ui_Colour outter)
{
	if (numpoints > 0) {
		HBRUSH hBrush;
		HBRUSH hBrushPrev;		
		...
		if (wpts) {
			for (i = 0; i < numpoints; ++i) {
				...
				x = (int)pts[i].x;
				y = (int)pts[i].y;
				wpts[i].x = x - (int)(self->org.x);
				wpts[i].y = y - (int)(self->org.y);
			}
			hBrush = CreateSolidBrush(psy_ui_colour_colorref(&inner));
