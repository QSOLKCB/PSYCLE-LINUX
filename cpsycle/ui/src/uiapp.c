...
{	
	...
	psy_ui_resources_init(&self->resources);	
	psy_ui_clipboard_init(&self->clipboard);	
	psy_ui_icons_make();
	inputhandler_init(&self->input_handler_);
	psy_ui_navhandler_init(&self->nav_handler_);
}
...
#elif PSYCLE_USE_TK == PSYCLE_TK_TEXT
	self->impfactory = (psy_ui_ImpFactory*)
		psy_ui_edit_impfactory_alloc_init();
#elif PSYCLE_USE_TK == PSYCLE_TK_EMPTY
	self->impfactory = (psy_ui_ImpFactory*)
...
{	
	...
	psy_ui_resources_dispose(&self->resources);	
	psy_ui_clipboard_dispose(&self->clipboard);
	psy_ui_navhandler_dispose(&self->nav_handler_);
...
InputHandler* psy_ui_app_input_handler(psy_ui_App* self)
{
	assert(self);
...
int psy_ui_app_run(psy_ui_App* self) 
{
	...
	psy_ui_navhandler_start(&self->nav_handler_);
	if (self->imp) {
	...
	translator)
{
	...
	for (; r != NULL; psy_list_next(&r)) {
		...
		p = q = psy_ui_component_children(curr_toplevel, psy_ui_RECURSIVE, FALSE);
...
const psy_ui_SVG* psy_ui_app_svg(psy_ui_App* self, const char* key)
{
	const psy_ui_Resource* resource;

	assert(self);

	resource = psy_ui_resources_at(&self->resources, key);
	if (resource) {
		if (resource->type == psy_ui_RESOURCE_TYPE_SVG) {
			return (const psy_ui_SVG*)resource->data;
		}
	}
	return NULL;
}
