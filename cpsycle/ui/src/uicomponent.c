...
static psy_List* children(psy_ui_Component*, int recursive, int only_visible);
...
static void on_align(psy_ui_Component* self)
{
	...
	q = psy_ui_component_children(self, psy_ui_NONE_RECURSIVE, FALSE /* todo */);
	for (p = q; p != NULL; p = p->next) {
		...
		if (component->align == psy_ui_ALIGN_NONE) {
			style = psy_ui_componentstyle_curr_style(&component->style);
...
static void on_language_changed(psy_ui_Component* self) { }

static void on_focus(psy_ui_Component* self)
{
	psy_ui_component_add_style_state(self, psy_ui_STYLESTATE_FOCUS);
	TRACE_INT((int)self->dbg);
}

static void on_focus_lost(psy_ui_Component* self)
{
...
static void onupdatestyles(psy_ui_Component* self)
{
	...
	style = psy_ui_componentstyle_curr_style(&self->style);
...
{
	if (font) {
		psy_ui_font_copy(&psy_ui_componentstyle_curr_style(&self->style)->font, font);
	}
	else {
		psy_ui_font_dispose(&psy_ui_componentstyle_curr_style(&self->style)->font);
	}
	self->imp->vtable->dev_setfont(self->imp, psy_ui_component_font(self));
}

psy_List* children(psy_ui_Component* self, int recursive, int visible)
{
	return self->imp->vtable->dev_children(self->imp, recursive, visible);
}
...
void psy_ui_component_init_base(psy_ui_Component* self) {
	...
	self->close_ = NULL;
	self->sizer_ = NULL;
	self->frame_icons_ = NULL;
	self->kbd_ = NULL;
	self->frame_size_ = psy_ui_size_make_em(80.0, 40.0);
...
void psy_ui_component_activate_resize(psy_ui_Component* self, int recursive)
{
	if (recursive == psy_ui_RECURSIVE) {
		...
		q = psy_ui_component_children(self, recursive, FALSE);
		for (p = q; p != NULL; p = p->next) {
...
psy_List* psy_ui_component_children(psy_ui_Component* self, int recursive, int visible)
{
	return self->vtable->children(self, recursive, visible);
}
	...
	psy_ui_AlignType align)
{
	...
	if (align == psy_ui_ALIGN_CLIENT) {
		self->aligner->align_has_client = TRUE;
		return;
	}	
	if (self->aligner && self->aligner->alignsorted != align) {
		...
		q = psy_ui_component_children(self, psy_ui_NONE_RECURSIVE, FALSE);
		for (p = q; p != NULL; p = p->next) {
			psy_ui_Component* component;
...
void enableinput_internal(psy_ui_Component* self, int enable, int recursive)
{
	...
	if (recursive) {
		...
		for (p = q = psy_ui_component_children(self, recursive, FALSE); p != NULL;
			p = p->next) {
...
void psy_ui_component_clear(psy_ui_Component* self)
{
	...
	c = psy_ui_component_children(self, psy_ui_NONE_RECURSIVE, FALSE);
	for (p = c; p != NULL; p = q) {
		psy_ui_Component* component;
		bool deallocate;
...
static psy_List* dev_children(psy_ui_ComponentImp* self, int recursive, int visible) { return 0; }
...
psy_ui_Component* psy_ui_component_at(psy_ui_Component* self, uintptr_t index)
{
	...
	p = q = psy_ui_component_children(self, 0, FALSE);
	while (p) {
		if (c == index) {
	...
	psy_ui_RealPoint pt, uintptr_t* index)
{
	...
	p = q = psy_ui_component_children(self, 0, FALSE /* todo */);
	while (p) {
		psy_ui_RealRectangle position;
		psy_ui_Component* component;
...
void psy_ui_component_focus_next(psy_ui_Component* self)
{
	if (self->tabindex != -1) {
		...
		if (parent) {
			...
			focus = NULL;
			for (q = p = psy_ui_component_children(parent, 0, FALSE); p != NULL;
				psy_list_next(&p)) {
				psy_ui_Component* component;
...
void psy_ui_component_update_language(psy_ui_Component* self)
{
	...
	self->vtable->on_language_changed(self);
...
void psy_ui_notify_style_update(psy_ui_Component* main)
{
	if (main) {
		...
		for (p = q = psy_ui_component_children(main, psy_ui_RECURSIVE, FALSE);
			p != NULL; p = p->next) {
			psy_ui_Component* child;
...
void psy_ui_component_draw(psy_ui_Component* self, psy_ui_Graphics* g)
{
	if (!(psy_ui_component_has_flag(self, psy_ui_COMPONENTFLAGS_PREVENT_DRAW)))
	{
		...
		if (self->vtable->ondraw || !psy_signal_empty(&self->signal_draw)) {
			psy_ui_Margin padding;
	...
	if (q) {
		...
		for (p = q; p != NULL; p = p->next) {
			...
			if (psy_ui_component_visible(component) &&
				...
				position = psy_ui_component_position(component);
				if ((self->aligner) && !self->aligner->align_has_client &&
	...
	uintptr_t value)
{
	...
	for (p = q = psy_ui_component_children(self, psy_ui_NONE_RECURSIVE, FALSE);
		p != NULL; p = p->next) {
...
void psy_ui_component_prevent_app_focus_out_recursive(psy_ui_Component* self)
{
	...
	q = psy_ui_component_children(self, TRUE, FALSE);
	for (p = q; p != NULL; p = p->next) {
		psy_ui_Component* curr;
...
void psy_ui_component_float(psy_ui_Component* self)
{
	assert(self);

	psy_ui_viewframe_alloc_init(psy_ui_app_main(psy_ui_app()),
		self, self->splitter_,
		self->frame_icons_, self->close_, self->sizer_, self->kbd_, 		
		self->frame_size_, TRUE /* center */);
}

static bool search_started;
static psy_ui_Component* search_start;
uintptr_t search_index;

static psy_ui_Component* psy_ui_component_search(psy_ui_Component* self)
{
	psy_ui_Component* rv;
	psy_List* q;
	psy_List* p;
	...
	rv = NULL;
	q = psy_ui_component_children(self, TRUE, TRUE);
	for (p = q; p != NULL; p = p->next) {
		psy_ui_Component* curr;
		...
		if (search_started && (curr->tabindex == search_index)) {
			rv = curr;
			break;
		}
		if (curr == search_start) {
			search_started = TRUE;
		}		
	}
	psy_list_free(q);
...
psy_ui_Component* psy_ui_component_next_tab_index(psy_ui_Component* self)
{
	search_started = FALSE;
	search_start = self;
	search_index = 0;
	return psy_ui_component_search(psy_ui_app_main(psy_ui_app()));
}

static psy_ui_Component* psy_ui_component_rev_search(psy_ui_Component* self)
{
	psy_ui_Component* rv;
	psy_List* q;
	psy_List* p;
...
psy_ui_Component* psy_ui_component_prev_tab_index(psy_ui_Component* self)
{
	search_started = FALSE;
	search_start = self;
	search_index = 0;
	return psy_ui_component_rev_search(psy_ui_app_main(psy_ui_app()));
}
