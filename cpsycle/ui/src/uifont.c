...
psy_ui_FontInfo* psy_ui_fontinfo_alloc(void)
{
	return (psy_ui_FontInfo*)malloc(sizeof(psy_ui_FontInfo));
}

psy_ui_FontInfo* psy_ui_fontinfo_alloc_init(void)
{
	psy_ui_FontInfo* rv;

	rv = psy_ui_fontinfo_alloc();
	if (rv) {
		psy_ui_fontinfo_init(rv, "Tahoma", 18);
	}
	return rv;
}
...
psy_ui_Font* psy_ui_font_alloc(void)
{
	return (psy_ui_Font*)malloc(sizeof(psy_ui_Font));
}

psy_ui_Font* psy_ui_font_alloc_init(void)
{
	psy_ui_Font* rv;

	rv = psy_ui_font_alloc();
	if (rv) {
		psy_ui_font_init(rv, NULL);
	}
	return rv;
}
