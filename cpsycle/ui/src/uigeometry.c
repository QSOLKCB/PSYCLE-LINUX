...
{
	return (psy_ui_RealPoint*)malloc(sizeof(psy_ui_RealPoint));
}
...
{
	return (psy_ui_RealSize*)malloc(sizeof(psy_ui_RealSize));
}
...
psy_ui_RealSize* psy_ui_realsize_alloc_init(void)
{
	...
	rv = psy_ui_realsize_alloc();
	if (rv) {
		psy_ui_realsize_init(rv);
	}
	return rv;
}
