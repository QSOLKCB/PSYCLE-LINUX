...
INLINE void psy_ui_realpoint_dispose(psy_ui_RealPoint* self)
{
}
...
psy_ui_RealPoint* psy_ui_realpoint_alloc(void);
psy_ui_RealPoint* psy_ui_realpoint_alloc_init(void);
...
INLINE psy_ui_RealPoint psy_ui_add_realpoints(psy_ui_RealPoint p1, psy_ui_RealPoint p2)
{
	psy_ui_RealPoint rv;
...
psy_ui_RealSize* psy_ui_realsize_alloc(void);
psy_ui_RealSize* psy_ui_realsize_alloc_init(void);

INLINE void psy_ui_realsize_dispose(psy_ui_RealSize* self)
{	
}
...
INLINE void psy_ui_realrectangle_dispose(psy_ui_RealRectangle* self)
{
}
...
INLINE void psy_ui_realrectangle_copy(psy_ui_RealRectangle* self,
...
INLINE psy_ui_Point psy_ui_point_invalid(void)
{
	psy_ui_Point rv;

	rv.x = rv.y = psy_ui_value_zero();
	rv.x.set = rv.y.set = FALSE;	
	return rv;
}
...
INLINE void psy_ui_size_disable(psy_ui_Size* self)
{
	assert(self);

	self->width.set = self->height.set = FALSE;	 
}

INLINE void psy_ui_size_enable(psy_ui_Size* self)
{
	assert(self);

	self->width.set = self->height.set = TRUE;
}

INLINE bool psy_ui_size_enabled(const psy_ui_Size* self)
{
	return (self->width.set && self->height.set);
}
