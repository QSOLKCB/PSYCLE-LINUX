...
static void psy_ui_notebook_on_focus(psy_ui_Notebook*);
...
void psy_ui_notebook_on_align(psy_ui_Notebook* self)
{
	if (!self->split) {		
		...
		for (p = q = psy_ui_component_children(psy_ui_notebook_base(self), 0, FALSE); p != NULL;
				psy_list_next(&p)) {
			psy_ui_Component* component;
...
{	
	...
	q = psy_ui_component_children(psy_ui_notebook_base(self), 0, FALSE);
...
void psy_ui_notebook_on_focus(psy_ui_Notebook* self)
{
	psy_ui_Component* curr;

	assert(self);

	curr = psy_ui_notebook_active_page(self);
	if (curr) {
		psy_ui_component_set_focus(curr);		
	}	
}