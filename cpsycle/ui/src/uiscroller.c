...
/* prototypes */
static void psy_ui_scroller_on_destroyed(psy_ui_Scroller*);
...
static bool psy_ui_scroller_on_input(psy_ui_Scroller*, InputHandler* sender);
...
void psy_ui_scroller_on_destroyed(psy_ui_Scroller* self)
{
	assert(self);

	inputhandler_disconnect(psy_ui_app_input_handler(psy_ui_app()),		
		self, psy_ui_scroller_on_input);
}
...
void psy_ui_scroller_on_pane_size(psy_ui_Scroller* self, psy_ui_Component* sender)
{
	if (self->client) {
		...
		if (psy_ui_component_scroll_step_height_px(self->client) == 0.0) {
			psy_ui_scrollbar_set_thumb_position(self->vscroll, 0.0);
		} else {
			psy_ui_scrollbar_set_thumb_position(self->vscroll,
		...
		if (psy_ui_component_scroll_step_width_px(self->client) == 0.0) {
			psy_ui_scrollbar_set_thumb_position(self->hscroll, 0.0);
		} else {
			psy_ui_scrollbar_set_thumb_position(self->hscroll,
...
void psy_ui_scroller_on_scroll(psy_ui_Scroller* self, psy_ui_Component* sender)
{
	if (!self->thumbmove) {
		...
		pos = floor(psy_ui_component_scroll_top_px(self->client) /
			psy_ui_component_scroll_step_height_px(self->client));
		psy_ui_scrollbar_set_thumb_position(self->vscroll, pos);
		/* horizontal */
		pos = floor(psy_ui_component_scroll_left_px(self->client) /
			psy_ui_component_scroll_step_width_px(self->client));
		psy_ui_scrollbar_set_thumb_position(self->hscroll, pos);
	}
}
...
void psy_ui_scroller_set_input_handler(psy_ui_Scroller* self, InputHandler* input_handler)
{
	assert(self);

	if (!input_handler && !self->client) {
		return;
	}
	inputhandler_connect(input_handler, INPUTHANDLER_FOCUS,
		psy_EVENTDRIVER_CMD, "tracker", psy_INDEX_INVALID,
		self, self->client, (fp_inputhandler_input)
		psy_ui_scroller_on_input);
}


bool psy_ui_scroller_on_input(psy_ui_Scroller* self, InputHandler* sender)
{
	...
	case CMD_NAVUP: {		
		psy_ui_scrollbar_set_thumb_position(self->vscroll,
			psy_ui_scrollbar_position(self->vscroll) - 1);
		psy_signal_emit(&self->vscroll->signal_changed, self->vscroll, 0);
		return TRUE; }
	case CMD_NAVDOWN: {		
		psy_ui_scrollbar_set_thumb_position(self->vscroll,
			psy_ui_scrollbar_position(self->vscroll) + 1);
