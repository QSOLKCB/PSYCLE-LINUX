...
static psy_List* view_dev_children(psy_ui_ViewComponentImp*, int recursive, int visible);
...
void view_dev_destroy(psy_ui_ViewComponentImp* self)
{
	...
	c = psy_ui_component_children(self->component, psy_ui_NONE_RECURSIVE, FALSE);
	for (p = c; p != NULL; p = q) {
		psy_ui_Component* component;
		bool deallocate;
	...
	psy_ui_ViewComponentImp* insertafter)
{
	if ((child->imp.vtable->dev_flags(&child->imp) & psy_ui_COMPONENTIMPFLAGS_HANDLECHILDREN) ==
		...
		if (child->view != self->view) {
			...
			q = p = psy_ui_component_children(child->component, psy_ui_RECURSIVE, FALSE);
			for (; p != NULL; p = p->next) {
				psy_ui_Component* curr;
...
psy_List* view_dev_children(psy_ui_ViewComponentImp* self, int recursive, int only_visible)
{
	psy_List* rv = 0;
	psy_List* p;
	...
	for (p = self->viewcomponents; p != NULL; psy_list_next(&p)) {
		...
		if (only_visible && !psy_ui_component_visible(curr)) {
			continue;
		}
		psy_list_append(&rv, curr);
		if (recursive == psy_ui_RECURSIVE) {
			psy_List* r;
			psy_List* q;

			q = psy_ui_component_children((psy_ui_Component*)psy_list_entry(p),
				recursive, only_visible);
