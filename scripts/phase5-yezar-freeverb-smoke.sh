...
grep -F 'phase5-yezar-freeverb: rate-transition PASS sr=44100->88200 active-before-switch=1117 old-tail=cleared first-left=2232 first-right=2278 network=reinitialized' "$NATIVE_LOG" >/dev/null
...
- Live 44.1 -> 88.2 kHz transition occurs after the first 44.1 kHz comb response has made damping state active; the rebuilt network clears the old tail completely and preserves scaled first-comb timing at 2232 left / 2278 right: PASS
...
its Psycle/Arguru module identity. Sample-rate rebuilds now reset both new delay-line
cursors and comb damping state so the discarded reverb tail cannot leak into the
new network; the Freeverb processing equations are unchanged.
EOF
