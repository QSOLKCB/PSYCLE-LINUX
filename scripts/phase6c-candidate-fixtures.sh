...
# Keep process receipts unchanged. The sidecar separates the scoped successful
# load from the historical player's process-lifetime timeout.
python3 "$ROOT/scripts/phase6c-candidate-parse.py" "$OUT"
python3 "$ROOT/scripts/phase6c-candidate-parse.py" "$OUT" --check
...
- PSY3 parse acceptance: independently derived in `candidate-psy3-parse.json` from the complete hash-bound log and exact fixture; the process receipt and any timeout remain unchanged. Unexpected warnings or incomplete/ambiguous markers keep parse acceptance inconclusive.
