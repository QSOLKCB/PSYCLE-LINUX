    ...
    try {
        ...
        foreach ($window in $windows) {
            ...
            if (-not $closed) {
                ...
                # class, then correlates that HWND back to the verified UIA
                # dialog using strongly overlapping screen bounds and a fresh
                # exact signature check before issuing standard IDOK.
                ...
                try {
                    ...
                    $nativeDialogs = @(
                        [Phase6cNativeButton]::FindTopLevelWindows(
                            [uint32]$Process.Id,
                            "DirectSound Output driver",
                            "#32770"
                        )
                    ...
                    if ($nativeDialogs.Count -ne 1) {
                        throw (
                            "DirectSound bootstrap expected exactly one native process-owned " +
                            "DirectSound Output driver #32770 dialog: count=$($nativeDialogs.Count)"
                        )
                    }
                    $dialogHandle = [IntPtr]$nativeDialogs[0]
                    if (-not [Phase6cNativeButton]::IsWindow($dialogHandle)) {
                        throw "DirectSound bootstrap resolved native dialog HWND is not a live window"
                    }

                    $nativeDialogElement = [System.Windows.Automation.AutomationElement]::FromHandle(
                        $dialogHandle
                    )
                    if ($null -eq $nativeDialogElement) {
                        throw "DirectSound bootstrap could not bind native dialog HWND back to UI Automation"
                    }
                    if ($nativeDialogElement.Current.ProcessId -ne $Process.Id -or
                        [string]$nativeDialogElement.Current.Name -cne
                            "DirectSound Output driver") {
                        throw "DirectSound bootstrap native/UIA dialog identity correlation failed"
                    }

                    $verifiedBounds = $window.Current.BoundingRectangle
                    $nativeBounds = $nativeDialogElement.Current.BoundingRectangle
                    if ($verifiedBounds.IsEmpty -or $nativeBounds.IsEmpty -or
                        $verifiedBounds.Width -le 0 -or $verifiedBounds.Height -le 0 -or
                        $nativeBounds.Width -le 0 -or $nativeBounds.Height -le 0) {
                        throw "DirectSound bootstrap native/UIA dialog correlation lacks usable screen bounds"
                    }
                    $intersection = [System.Windows.Rect]::Intersect(
                        $verifiedBounds,
                        $nativeBounds
                    )
                    if ($intersection.IsEmpty -or
                        $intersection.Width -le 0 -or
                        $intersection.Height -le 0) {
                        throw "DirectSound bootstrap native HWND does not overlap the verified UIA dialog"
                    }
                    $intersectionArea = $intersection.Width * $intersection.Height
                    $verifiedArea = $verifiedBounds.Width * $verifiedBounds.Height
                    $nativeArea = $nativeBounds.Width * $nativeBounds.Height
                    $verifiedCoverage = $intersectionArea / $verifiedArea
                    $nativeCoverage = $intersectionArea / $nativeArea
                    if ($verifiedCoverage -lt 0.90 -or $nativeCoverage -lt 0.90) {
                        throw (
                            "DirectSound bootstrap native/UIA dialog geometry mismatch: " +
                            "verified_coverage=$verifiedCoverage native_coverage=$nativeCoverage"
                        )
                    }

                    $correlatedNodes = $nativeDialogElement.FindAll(
                        [System.Windows.Automation.TreeScope]::Descendants,
                        [System.Windows.Automation.Condition]::TrueCondition
                    )
