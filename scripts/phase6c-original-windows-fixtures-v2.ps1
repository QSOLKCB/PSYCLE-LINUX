...
try {
    ...
    foreach ($spec in $fixtureSpecs) {
        ...
        while ((Get-Date) -lt $deadline) {
            ...
            if ($sequenceOrderExpected.Count -gt 0 -and $uiDiagnostics.Count -eq 0) {
        ...
        if ($sequenceOrderExpected.Count -gt 0) {
            ...
            if ($sequenceOrderStablePolls -ge 4 -and
        ...
        $receipt = [ordered]@{
            ...
            procedure = if ($ObserveSequenceOrder -and $spec.name -eq "sequence-order") {
                "$Procedure; for the sequencer-pattern-order contract enumerate the complete visible UI Automation order-label set matching NN: NN on every poll and require a stable complete set before interpreting order"
...
- Sequence/order observation: when enabled, the project-authored single-sequence fixture is inspected without UI input by collecting the complete visible order-label set matching NN: NN on every poll. A sequence observation is conclusive only after the same complete four-label set is stable for at least four polls; the actual labels are recorded even when they differ from the fixture expectation.
