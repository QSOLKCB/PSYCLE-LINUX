/*
** PSYCLE-LINUX Phase 5C Alk Muter production persistence test.
**
** Exercises the retained effect through PluginCatcher, MachineFactory,
** version-1 preset I/O and PSY3 save/reload using the legal non-default
** public Mute state.
*/
...
#define SLOT 0u
#define CATCHER_NAME "alk-muter:0"
#define PRESET_NAME "Phase 5 Alk Muter"
#define EXPECTED_VALUE 1

static int fail(const char* message)
{
	fprintf(stderr, "phase5-alk-muter-state: FAIL: %s\n", message);
	return 1;
}
	...
	const char* module_path)
{
	...
	if (!psy_audio_plugin_psycle_test(module_path, catcher->native_root_, &info)) {
		rc = fail("production native-plugin probe failed");
			...
			strcmp(info.name, "Alk Muter") != 0 ||
			!info.shortname || strcmp(info.shortname, "Muter") != 0 ||
			!info.author || strcmp(info.author, "Alk") != 0) {
		rc = fail("production probe returned unexpected Alk Muter identity");
	} else {
		machineinfo_catchername(&info, catcher_name);
		if (strcmp(catcher_name, CATCHER_NAME) != 0) {
			fprintf(stderr,
				"phase5-alk-muter-state: FAIL: catcher expected %s got %s\n",
...
static int verify_identity(psy_audio_Machine* machine)
{
	...
	if (!machine || psy_audio_machine_type(machine) != psy_audio_PLUGIN) {
		return fail("Alk Muter missing or no longer a native Psycle plugin");
	}
	info = psy_audio_machine_info(machine);
	if (!info || !info->name || strcmp(info->name, "Alk Muter") != 0 ||
			!info->shortname || strcmp(info->shortname, "Muter") != 0 ||
			!info->author || strcmp(info->author, "Alk") != 0 ||
			!info->modulepath || !strstr(info->modulepath, "alk-muter")) {
		return fail("Alk Muter production identity/module path changed");
	}
	if (psy_audio_machine_num_parameters(machine) != PARAMETER_COUNT) {
		return fail("Alk Muter production parameter count changed");
	}
	if (psy_audio_machine_data_size(machine) != 0) {
		return fail("Alk Muter unexpectedly acquired opaque plugin state");
	}
	return 0;
}

static int seed_parameter(psy_audio_Machine* machine)
{
	psy_audio_MachineParam* param = psy_audio_machine_parameter(machine, 0);
	intptr_t minval;
	intptr_t maxval;
	if (!param) return fail("Mute parameter is missing");
	psy_audio_machine_parameter_range(machine, param, &minval, &maxval);
	if (EXPECTED_VALUE < minval || EXPECTED_VALUE > maxval) {
		return fail("Mute preservation seed is outside public range");
	}
	psy_audio_machine_parameter_tweak_scaled(machine, param, EXPECTED_VALUE);
	if (psy_audio_machine_parameter_scaled_value(machine, param) != EXPECTED_VALUE) {
		return fail("Mute parameter did not retain requested value");
	}
	return 0;
}

static int verify_value(psy_audio_Machine* machine)
{
	psy_audio_MachineParam* param;
	if (verify_identity(machine) != 0) return 1;
	param = psy_audio_machine_parameter(machine, 0);
	if (!param) return fail("Mute parameter missing during verification");
	if (psy_audio_machine_parameter_scaled_value(machine, param) != EXPECTED_VALUE) {
		return fail("Mute parameter did not survive persistence");
	}
	return 0;
}
	...
	psy_audio_Machine* source, const char* preset_path)
{
	...
	if (psy_audio_preset_num_parameters(captured) != PARAMETER_COUNT ||
		...
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("captured Alk Muter preset geometry changed");
	}
	...
	if (status != psy_audio_PRESETIO_OK) {
		rc = fail("Alk Muter preset save failed");
		goto cleanup;
	}
	...
	if (status != psy_audio_PRESETIO_OK || psy_audio_presets_size(&loaded) != 1) {
		rc = fail("Alk Muter preset reload failed");
		goto cleanup;
	}
	...
	if (!reloaded || strcmp(psy_audio_preset_name(reloaded), PRESET_NAME) != 0 ||
			...
			reloaded->datasize != 0 ||
			psy_audio_preset_value(reloaded, 0) != EXPECTED_VALUE) {
		rc = fail("reloaded Alk Muter preset changed");
		goto cleanup;
	}
	fresh = make_machine(factory);
	if (!fresh) {
		rc = fail("fresh Alk Muter preset-restore machine creation failed");
		goto cleanup;
	}
	psy_audio_machine_tweak_preset(fresh, reloaded);
	rc = verify_value(fresh);
...
int main(int argc, char** argv)
{
	...
	if (argc != 3) {
		fprintf(stderr, "usage: %s OUTPUT_DIRECTORY PATH_TO_ALK_MUTER_SO\n", argv[0]);
		return 2;
	}
	if (snprintf(song_path, sizeof(song_path), "%s/phase5-alk-muter.psy", argv[1]) >=
			(int)sizeof(song_path) ||
			snprintf(preset_path, sizeof(preset_path), "%s/phase5-alk-muter.prs", argv[1]) >=
	...
	if (!song) {
		rc = fail("could not allocate Alk Muter song");
		goto initial_cleanup;
	}
	psy_audio_machinecallback_set_song(&callback, song);
	machine = make_machine(&factory);
	if (!machine || verify_identity(machine) != 0 ||
			seed_parameter(machine) != 0 || verify_value(machine) != 0 ||
			preset_roundtrip(&factory, machine, preset_path) != 0) {
		rc = 1;
	...
	if (save_song(song, song_path) != 0) {
		rc = fail("Alk Muter PSY3 save failed");
		goto song_cleanup;
	}
	...
	if (!loaded) {
		rc = fail("fresh Alk Muter PSY3 reopen failed");
	} else {
		...
		if (verify_value(reopened) != 0) {
			rc = 1;
		} else if (!psy_audio_machines_connected(psy_audio_song_machines(loaded),
				psy_audio_wire_make(SLOT, psy_audio_MASTER_INDEX))) {
			rc = fail("Alk Muter wire to Master did not survive PSY3 reload");
		}
...
initial_cleanup:
	...
	printf("phase5-alk-muter-state: PASS\n");
	printf("catcher: alk-muter:0\n");
	printf("state: 1/1 public parameter seeded non-default, 0 opaque bytes\n");
	printf("topology: Alk Muter -> Master\n");
	printf("preset: %s\n", preset_path);
	printf("song: %s\n", song_path);
	return 0;
}
