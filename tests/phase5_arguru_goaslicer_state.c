/*
** PSYCLE-LINUX Phase 5 Arguru Goaslicer state/persistence regression.
**
...
#define GOASLICER_SLOT 0u
#define PARAM_COUNT 2u
#define CATCHER_NAME "arguru-goaslicer:0"
#define PRESET_NAME "Phase 5 Arguru Goaslicer state"
...
{
	fprintf(stderr, "phase5-arguru-goaslicer-state: FAIL: %s\n", message);
	return 1;
}
	...
	const char* module_path)
{
	...
	if (!psy_audio_plugin_psycle_test(module_path, catcher->native_root_, &info)) {
		rc = fail("production native-plugin probe could not read Arguru Goaslicer");
	} else if (info.type != psy_audio_PLUGIN || !info.name ||
			strcmp(info.name, "Arguru Goaslicer") != 0) {
		...
		if (strcmp(catcher_name, CATCHER_NAME) != 0) {
			rc = fail("Arguru Goaslicer catcher identity changed");
		} else {
			psy_audio_plugins_add(&catcher->plugins_, &info);
			if (!psy_audio_plugins_at_id_const(&catcher->plugins_, catcher_name)) {
				rc = fail("Arguru Goaslicer was not registered in PluginCatcher");
			}
		}
	}
...
static psy_audio_Machine* make_goaslicer(psy_audio_MachineFactory* factory)
{
...
static int verify_identity(psy_audio_Machine* machine)
{
	...
	if (!machine || psy_audio_machine_type(machine) != psy_audio_PLUGIN) {
		return fail("Arguru Goaslicer is missing or no longer a Psycle native plugin");
	}
	info = psy_audio_machine_info(machine);
	if (!info || !info->name || strcmp(info->name, "Arguru Goaslicer") != 0 ||
			!info->shortname || strcmp(info->shortname, "Goaslicer") != 0 ||
			!info->author || strcmp(info->author, "J. Arguelles") != 0 ||
			!info->modulepath || !strstr(info->modulepath, "arguru-goaslicer")) {
		return fail("Arguru Goaslicer identity/module path changed");
	}
	...
	if (psy_audio_machine_data_size(machine) != 0) {
		return fail("Arguru Goaslicer unexpectedly acquired opaque plugin state");
	}
	return 0;
}
...
static int set_expected_parameters(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAM_COUNT; ++i) {
		...
		if (!param) {
			return fail("Arguru Goaslicer parameter surface is incomplete");
		}
...
static int verify_expected_parameters(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAM_COUNT; ++i) {
		...
		param = psy_audio_machine_parameter(machine, i);
		if (!param) {
			return fail("Arguru Goaslicer parameter missing during verification");
		}
		actual = psy_audio_machine_parameter_scaled_value(machine, param);
		if (actual != EXPECTED_VALUES[i]) {
			fprintf(stderr,
				"phase5-arguru-goaslicer-state: FAIL: parameter %lu expected %ld got %ld\n",
	...
	psy_audio_Machine* source, const char* preset_path)
{
	...
	if (!captured) {
		...
		return fail("could not allocate captured Arguru Goaslicer preset");
	}
	...
	if (psy_audio_preset_num_parameters(captured) != PARAM_COUNT ||
		...
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("captured Arguru Goaslicer preset has unexpected state geometry");
	}
	...
	if (status != psy_audio_PRESETIO_OK) {
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("Arguru Goaslicer preset save failed");
	}
	...
	if (status != psy_audio_PRESETIO_OK || psy_audio_presets_size(&loaded) != 1) {
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("Arguru Goaslicer preset reload failed");
	}
	...
	if (!reloaded || strcmp(psy_audio_preset_name(reloaded), PRESET_NAME) != 0 ||
		...
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("Arguru Goaslicer preset metadata changed");
	}
	for (i = 0; i < PARAM_COUNT; ++i) {
		if (psy_audio_preset_value(reloaded, i) != EXPECTED_VALUES[i]) {
			psy_audio_presets_dispose(&saved);
			psy_audio_presets_dispose(&loaded);
			return fail("Arguru Goaslicer preset parameter changed");
		}
	}

	fresh = make_goaslicer(factory);
	if (!fresh) {
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("could not create fresh Arguru Goaslicer for preset restore");
	}
...
static int save_song(psy_audio_Song* song, const char* path)
{
	...
	if (status != PSY_OK) {
		fprintf(stderr, "phase5-arguru-goaslicer-state: song save failed (%d)\n", status);
		return 1;
	}
	return 0;
}
...
static psy_audio_Song* load_song(psy_audio_MachineFactory* factory, const char* path)
{
	...
	if (status != PSY_OK) {
		fprintf(stderr, "phase5-arguru-goaslicer-state: song load failed (%d)\n", status);
...
int main(int argc, char** argv)
{
	...
	psy_audio_Machine* loaded_goaslicer;
	int rc;

	if (argc != 3) {
		fprintf(stderr, "usage: %s OUTPUT_DIRECTORY PATH_TO_ARGURU_GOASLICER_SO\n", argv[0]);
		return 2;
	}
	if (snprintf(song_path, sizeof(song_path), "%s/phase5-arguru-goaslicer.psy", argv[1]) >=
			(int)sizeof(song_path) ||
			snprintf(preset_path, sizeof(preset_path), "%s/phase5-arguru-goaslicer.prs", argv[1]) >=
	...
	song = psy_audio_song_alloc_init(&factory);
	if (!song) {
		rc = fail("could not allocate Arguru Goaslicer song");
		goto initial_cleanup;
	}
	psy_audio_machinecallback_set_song(&callback, song);
	goaslicer = make_goaslicer(&factory);
	if (!goaslicer) {
		rc = fail("production MachineFactory could not create Arguru Goaslicer");
		goto song_cleanup;
	}
	psy_audio_machines_insert(psy_audio_song_machines(song), GOASLICER_SLOT, goaslicer);
	psy_audio_machines_connect(psy_audio_song_machines(song),
		psy_audio_wire_make(GOASLICER_SLOT, psy_audio_MASTER_INDEX));

	if ((rc = verify_identity(goaslicer)) != 0 ||
			(rc = set_expected_parameters(goaslicer)) != 0 ||
			(rc = verify_expected_parameters(goaslicer)) != 0 ||
			(rc = exercise_preset_roundtrip(&factory, goaslicer, preset_path)) != 0 ||
	...
	if (!loaded) {
		rc = fail("PSY3 reload could not reconstruct Arguru Goaslicer");
	} else {
		psy_audio_machinecallback_set_song(&loaded_callback, loaded);
		loaded_goaslicer = psy_audio_machines_at(psy_audio_song_machines(loaded),
			GOASLICER_SLOT);
		if ((rc = verify_expected_parameters(loaded_goaslicer)) == 0 &&
				!psy_audio_machines_connected(psy_audio_song_machines(loaded),
					psy_audio_wire_make(GOASLICER_SLOT, psy_audio_MASTER_INDEX))) {
			rc = fail("Arguru Goaslicer wire to Master did not survive PSY3 reload");
		}
...
initial_cleanup:
	...
	printf("phase5-arguru-goaslicer-state: PASS\n");
	printf("preset: %s\n", preset_path);
	printf("song: %s\n", song_path);
	printf("state: 2 parameters, 0 opaque bytes\n");
	return 0;
}
