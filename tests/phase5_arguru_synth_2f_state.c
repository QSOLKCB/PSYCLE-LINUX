/*
** PSYCLE-LINUX Phase 5 Arguru Synth 2f state/persistence regression.
**
** Exercises the retained native generator through Psycle's production Plugin,
...
#define SYNTH_SLOT 0u
#define PARAM_COUNT 28u
#define CATCHER_NAME "arguru-synth-2f:0"
#define PRESET_NAME "Phase 5 Arguru Synth 2f state"

/* Use exact range endpoints wherever practical so Psycle's normalized public
** parameter API round-trips each integer exactly instead of testing floating
** reconstruction artefacts. */
static const intptr_t EXPECTED_VALUES[PARAM_COUNT] = {
	0, 6, -36, 256, 1,
	32, 220500, 256, 32,
	220500, 32, 256, 220500,
	1, 240, 240, 1, 19, -240,
	256, 256, 16, 1024, 16,
	-36, 256, 255, 1
};
...
{
	fprintf(stderr, "phase5-arguru-synth-2f-state: FAIL: %s\n", message);
	return 1;
}
	...
	const char* module_path)
{
	...
	if (!psy_audio_plugin_psycle_test(module_path, catcher->native_root_, &info)) {
		rc = fail("production native-plugin probe could not read Arguru Synth 2f");
	} else if (info.type != psy_audio_PLUGIN || !info.name ||
			strcmp(info.name, "Arguru Synth 2") != 0) {
		rc = fail("production native-plugin probe returned unexpected synth metadata");
	} else {
		machineinfo_catchername(&info, catcher_name);
		if (strcmp(catcher_name, CATCHER_NAME) != 0) {
			fprintf(stderr,
				"phase5-arguru-synth-2f-state: FAIL: catcher identity expected %s got %s\n",
			...
			if (!psy_audio_plugins_at_id_const(&catcher->plugins_, catcher_name)) {
				rc = fail("Arguru Synth 2f was not registered in PluginCatcher");
			}
		}
	}
...
static psy_audio_Machine* make_synth(psy_audio_MachineFactory* factory)
{
...
static int verify_identity(psy_audio_Machine* machine)
{
	...
	if (!machine || psy_audio_machine_type(machine) != psy_audio_PLUGIN) {
		return fail("Arguru Synth 2f is missing or no longer a Psycle native plugin");
	}
	info = psy_audio_machine_info(machine);
	if (!info || !info->name || strcmp(info->name, "Arguru Synth 2") != 0 ||
			!info->shortname || strcmp(info->shortname, "Arguru Synth") != 0 ||
			!info->author || strcmp(info->author,
				"J. Arguelles (arguru) and psycledelics") != 0 ||
			!info->modulepath || !strstr(info->modulepath, "arguru-synth-2f")) {
		return fail("Arguru Synth 2f identity/module path changed");
	}
	...
	if (psy_audio_machine_data_size(machine) != 0) {
		return fail("Arguru Synth 2f unexpectedly acquired opaque plugin state");
	}
	return 0;
}
...
static int set_expected_parameters(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAM_COUNT; ++i) {
		...
		if (!param) {
			return fail("Arguru Synth 2f parameter surface is incomplete");
		}
...
static int verify_expected_parameters(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAM_COUNT; ++i) {
		...
		param = psy_audio_machine_parameter(machine, i);
		if (!param) {
			return fail("Arguru Synth 2f parameter missing during verification");
		}
		actual = psy_audio_machine_parameter_scaled_value(machine, param);
		if (actual != EXPECTED_VALUES[i]) {
			fprintf(stderr,
				"phase5-arguru-synth-2f-state: FAIL: parameter %lu expected %ld got %ld\n",
	...
	psy_audio_Machine* source, const char* preset_path)
{
	...
	if (!captured) {
		...
		return fail("could not allocate captured Arguru Synth 2f preset");
	}
	...
	if (psy_audio_preset_num_parameters(captured) != PARAM_COUNT ||
		...
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("captured Arguru Synth 2f preset has unexpected state geometry");
	}
	...
	if (status != psy_audio_PRESETIO_OK) {
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("Arguru Synth 2f preset save failed");
	}
	...
	if (status != psy_audio_PRESETIO_OK || psy_audio_presets_size(&loaded) != 1) {
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("Arguru Synth 2f preset reload failed");
	}
	...
	if (!reloaded || strcmp(psy_audio_preset_name(reloaded), PRESET_NAME) != 0 ||
		...
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("Arguru Synth 2f preset metadata changed");
	}
	for (i = 0; i < PARAM_COUNT; ++i) {
		if (psy_audio_preset_value(reloaded, i) != EXPECTED_VALUES[i]) {
			fprintf(stderr,
				"phase5-arguru-synth-2f-state: FAIL: preset parameter %lu changed\n",
				(unsigned long)i);
			psy_audio_presets_dispose(&saved);
			psy_audio_presets_dispose(&loaded);
			return 1;
	...
	fresh = make_synth(factory);
	if (!fresh) {
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("could not create fresh Arguru Synth 2f for preset restore");
	}
...
static int save_song(psy_audio_Song* song, const char* path)
{
	...
	if (status != PSY_OK) {
		fprintf(stderr, "phase5-arguru-synth-2f-state: song save failed (%d)\n", status);
		return 1;
	}
	return 0;
}
...
static psy_audio_Song* load_song(psy_audio_MachineFactory* factory, const char* path)
{
	...
	if (status != PSY_OK) {
		fprintf(stderr, "phase5-arguru-synth-2f-state: song load failed (%d)\n", status);
...
int main(int argc, char** argv)
{
	...
	psy_audio_Machine* loaded_synth;
	int rc;

	if (argc != 3) {
		fprintf(stderr, "usage: %s OUTPUT_DIRECTORY PATH_TO_ARGURU_SYNTH_2F_SO\n", argv[0]);
		return 2;
	}
	if (snprintf(song_path, sizeof(song_path), "%s/phase5-arguru-synth-2f.psy", argv[1]) >=
			(int)sizeof(song_path) ||
			snprintf(preset_path, sizeof(preset_path), "%s/phase5-arguru-synth-2f.prs", argv[1]) >=
	...
	song = psy_audio_song_alloc_init(&factory);
	if (!song) {
		rc = fail("could not allocate Arguru Synth 2f song");
		goto initial_cleanup;
	}
	psy_audio_machinecallback_set_song(&callback, song);
	synth = make_synth(&factory);
	if (!synth) {
		rc = fail("production MachineFactory could not create Arguru Synth 2f");
		goto song_cleanup;
	}
	psy_audio_machines_insert(psy_audio_song_machines(song), SYNTH_SLOT, synth);
	psy_audio_machines_connect(psy_audio_song_machines(song),
		psy_audio_wire_make(SYNTH_SLOT, psy_audio_MASTER_INDEX));

	if ((rc = verify_identity(synth)) != 0 ||
			(rc = set_expected_parameters(synth)) != 0 ||
			(rc = verify_expected_parameters(synth)) != 0 ||
			(rc = exercise_preset_roundtrip(&factory, synth, preset_path)) != 0 ||
	...
	if (!loaded) {
		rc = fail("PSY3 reload could not reconstruct Arguru Synth 2f");
	} else {
		psy_audio_machinecallback_set_song(&loaded_callback, loaded);
		loaded_synth = psy_audio_machines_at(psy_audio_song_machines(loaded),
			SYNTH_SLOT);
		if ((rc = verify_expected_parameters(loaded_synth)) == 0 &&
				!psy_audio_machines_connected(psy_audio_song_machines(loaded),
					psy_audio_wire_make(SYNTH_SLOT, psy_audio_MASTER_INDEX))) {
			rc = fail("Arguru Synth 2f wire to Master did not survive PSY3 reload");
		}
...
initial_cleanup:
	...
	printf("phase5-arguru-synth-2f-state: PASS\n");
	printf("preset: %s\n", preset_path);
	printf("song: %s\n", song_path);
	printf("state: 28 parameters, 0 opaque bytes\n");
	return 0;
}
