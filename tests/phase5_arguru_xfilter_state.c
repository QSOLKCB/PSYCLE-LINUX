/*
** PSYCLE-LINUX Phase 5 Arguru XFilter / CrossDelay state regression.
**
...
#define XFILTER_SLOT 0u
#define PARAM_COUNT 6u
#define CATCHER_NAME "arguru-xfilter:0"
#define PRESET_NAME "Phase 5 Arguru XFilter state"

/* Use exact public-range endpoints so normalized/scaled reconstruction is
** exact while still exercising delay, mix, Lines mode and Lines state. */
...
{
	fprintf(stderr, "phase5-arguru-xfilter-state: FAIL: %s\n", message);
	return 1;
}
	...
	const char* module_path)
{
	...
	if (!psy_audio_plugin_psycle_test(module_path, catcher->native_root_, &info)) {
		rc = fail("production native-plugin probe could not read Arguru CrossDelay");
	} else if (info.type != psy_audio_PLUGIN || !info.name ||
			strcmp(info.name, "Arguru CrossDelay") != 0) {
		...
		if (strcmp(catcher_name, CATCHER_NAME) != 0) {
			fprintf(stderr,
				"phase5-arguru-xfilter-state: FAIL: catcher identity expected %s got %s\n",
			...
			if (!psy_audio_plugins_at_id_const(&catcher->plugins_, catcher_name)) {
				rc = fail("Arguru CrossDelay was not registered in PluginCatcher");
			}
		}
	}
...
static psy_audio_Machine* make_xfilter(psy_audio_MachineFactory* factory)
{
...
static int verify_identity(psy_audio_Machine* machine)
{
	...
	if (!machine || psy_audio_machine_type(machine) != psy_audio_PLUGIN) {
		return fail("Arguru CrossDelay is missing or no longer a Psycle native plugin");
	}
	info = psy_audio_machine_info(machine);
	if (!info || !info->name || strcmp(info->name, "Arguru CrossDelay") != 0 ||
			!info->shortname || strcmp(info->shortname, "CrossDelay") != 0 ||
			!info->author || strcmp(info->author, "J. Arguelles") != 0 ||
			!info->modulepath || !strstr(info->modulepath, "arguru-xfilter")) {
		return fail("Arguru CrossDelay identity/module path changed");
	}
	...
	if (psy_audio_machine_data_size(machine) != 0) {
		return fail("Arguru CrossDelay unexpectedly acquired opaque plugin state");
	}
	return 0;
}
...
static int set_expected_parameters(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAM_COUNT; ++i) {
		...
		if (!param) {
			return fail("Arguru CrossDelay parameter surface is incomplete");
		}
...
static int verify_expected_parameters(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAM_COUNT; ++i) {
		...
		if (!param) {
			return fail("Arguru CrossDelay parameter missing during verification");
		}
		actual = psy_audio_machine_parameter_scaled_value(machine, param);
		if (actual != EXPECTED_VALUES[i]) {
			fprintf(stderr,
				"phase5-arguru-xfilter-state: FAIL: parameter %lu expected %ld got %ld\n",
	...
	psy_audio_Machine* source, const char* preset_path)
{
	...
	if (!captured) {
		...
		return fail("could not allocate captured Arguru XFilter preset");
	}
	...
	if (psy_audio_preset_num_parameters(captured) != PARAM_COUNT ||
		...
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("captured Arguru XFilter preset has unexpected state geometry");
	}
	...
	if (status != psy_audio_PRESETIO_OK) {
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("Arguru XFilter preset save failed");
	}
	...
	if (status != psy_audio_PRESETIO_OK || psy_audio_presets_size(&loaded) != 1) {
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("Arguru XFilter preset reload failed");
	}
	...
	if (!reloaded || strcmp(psy_audio_preset_name(reloaded), PRESET_NAME) != 0 ||
		...
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("Arguru XFilter preset metadata changed");
	}
	for (i = 0; i < PARAM_COUNT; ++i) {
		if (psy_audio_preset_value(reloaded, i) != EXPECTED_VALUES[i]) {
			psy_audio_presets_dispose(&saved);
			psy_audio_presets_dispose(&loaded);
			return fail("Arguru XFilter preset parameter changed");
		}
	}

	fresh = make_xfilter(factory);
	if (!fresh) {
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("could not create fresh Arguru XFilter for preset restore");
	}
...
static int save_song(psy_audio_Song* song, const char* path)
{
	...
	if (status != PSY_OK) {
		fprintf(stderr, "phase5-arguru-xfilter-state: song save failed (%d)\n", status);
		return 1;
	}
	return 0;
}
...
static psy_audio_Song* load_song(psy_audio_MachineFactory* factory, const char* path)
{
	...
	if (status != PSY_OK) {
		fprintf(stderr, "phase5-arguru-xfilter-state: song load failed (%d)\n", status);
...
int main(int argc, char** argv)
{
	...
	psy_audio_Machine* loaded_xfilter;
	int rc;

	if (argc != 3) {
		fprintf(stderr, "usage: %s OUTPUT_DIRECTORY PATH_TO_ARGURU_XFILTER_SO\n", argv[0]);
		return 2;
	}
	if (snprintf(song_path, sizeof(song_path), "%s/phase5-arguru-xfilter.psy", argv[1]) >=
			(int)sizeof(song_path) ||
			snprintf(preset_path, sizeof(preset_path), "%s/phase5-arguru-xfilter.prs", argv[1]) >=
	...
	song = psy_audio_song_alloc_init(&factory);
	if (!song) {
		rc = fail("could not allocate Arguru XFilter song");
		goto initial_cleanup;
	}
	psy_audio_machinecallback_set_song(&callback, song);
	xfilter = make_xfilter(&factory);
	if (!xfilter) {
		rc = fail("production MachineFactory could not create Arguru XFilter");
		goto song_cleanup;
	}
	psy_audio_machines_insert(psy_audio_song_machines(song), XFILTER_SLOT, xfilter);
	psy_audio_machines_connect(psy_audio_song_machines(song),
		psy_audio_wire_make(XFILTER_SLOT, psy_audio_MASTER_INDEX));

	if ((rc = verify_identity(xfilter)) != 0 ||
			(rc = set_expected_parameters(xfilter)) != 0 ||
			(rc = verify_expected_parameters(xfilter)) != 0 ||
			(rc = exercise_preset_roundtrip(&factory, xfilter, preset_path)) != 0 ||
	...
	if (!loaded) {
		rc = fail("PSY3 reload could not reconstruct Arguru XFilter");
	} else {
		psy_audio_machinecallback_set_song(&loaded_callback, loaded);
		loaded_xfilter = psy_audio_machines_at(psy_audio_song_machines(loaded),
			XFILTER_SLOT);
		if ((rc = verify_expected_parameters(loaded_xfilter)) == 0 &&
				!psy_audio_machines_connected(psy_audio_song_machines(loaded),
					psy_audio_wire_make(XFILTER_SLOT, psy_audio_MASTER_INDEX))) {
			rc = fail("Arguru XFilter wire to Master did not survive PSY3 reload");
		}
...
initial_cleanup:
	...
	printf("phase5-arguru-xfilter-state: PASS\n");
	printf("preset: %s\n", preset_path);
	printf("song: %s\n", song_path);
