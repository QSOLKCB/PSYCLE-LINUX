/*
** PSYCLE-LINUX Phase 5C Audacity Compressor production persistence test.
**
...
#define SLOT 0u
#define CATCHER_NAME "audacity-compressor:0"
#define PRESET_NAME "Phase 5 Audacity Compressor"

static const intptr_t EXPECTED_VALUES[PARAMETER_COUNT] = {
	-18, 400, 350, 1500, 1, -50, 1
};

static int fail(const char* message)
{
	fprintf(stderr, "phase5-audacity-compressor-state: FAIL: %s\n", message);
	return 1;
}
	...
	const char* module_path)
{
	...
	if (!psy_audio_plugin_psycle_test(module_path, catcher->native_root_, &info)) {
		rc = fail("production native-plugin probe failed");
			...
			strcmp(info.name, "Audacity Compressor") != 0 ||
			!info.shortname || strcmp(info.shortname, "ACompressor") != 0 ||
			!info.author ||
			strcmp(info.author, "Dominic Mazzoni/Sartorius/JosepMa") != 0) {
		rc = fail("production probe returned unexpected Audacity Compressor identity");
	} else {
		machineinfo_catchername(&info, catcher_name);
		if (strcmp(catcher_name, CATCHER_NAME) != 0) {
			fprintf(stderr,
				"phase5-audacity-compressor-state: FAIL: catcher expected %s got %s\n",
...
static int verify_identity(psy_audio_Machine* machine)
{
	...
	if (!machine || psy_audio_machine_type(machine) != psy_audio_PLUGIN)
		return fail("Audacity Compressor missing or no longer a native Psycle plugin");
	info = psy_audio_machine_info(machine);
	if (!info || !info->name || strcmp(info->name, "Audacity Compressor") != 0 ||
			!info->shortname || strcmp(info->shortname, "ACompressor") != 0 ||
			!info->author ||
			strcmp(info->author, "Dominic Mazzoni/Sartorius/JosepMa") != 0 ||
			!info->modulepath || !strstr(info->modulepath, "audacity-compressor"))
	...
	if (psy_audio_machine_data_size(machine) != 0)
		return fail("Audacity Compressor unexpectedly acquired opaque plugin state");
	return 0;
}
...
static int seed_parameters(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		...
		if (psy_audio_machine_parameter_scaled_value(machine, param) != EXPECTED_VALUES[i])
			return fail("seed parameter did not retain requested value");
	}
	return 0;
}
...
static int verify_values(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		...
		if (actual != EXPECTED_VALUES[i]) {
			fprintf(stderr,
				"phase5-audacity-compressor-state: FAIL: parameter %lu expected %ld got %ld\n",
	...
	const char* module_path)
{
	...
	if (!reloaded || strcmp(psy_audio_preset_name(reloaded), PRESET_NAME) != 0 ||
			...
			reloaded->datasize != 0) {
		rc = fail("reloaded preset metadata changed");
		goto preset_cleanup;
	}
	for (i = 0; i < PARAMETER_COUNT; ++i) {
...
int main(int argc, char** argv)
{
	...
	if (argc != 3) {
		fprintf(stderr, "usage: %s OUTPUT_DIRECTORY PATH_TO_AUDACITY_COMPRESSOR_SO\n", argv[0]);
		return 2;
	}
	if (snprintf(song_path, sizeof(song_path), "%s/phase5-audacity-compressor.psy", argv[1]) >=
			(int)sizeof(song_path) ||
			snprintf(preset_path, sizeof(preset_path), "%s/phase5-audacity-compressor.prs", argv[1]) >=
	...
	if (!song) {
		rc = fail("could not allocate Audacity Compressor song");
		goto initial_cleanup;
	}
...
initial_cleanup:
	...
	printf("phase5-audacity-compressor-state: PASS\n");
	printf("catcher: audacity-compressor:0\n");
	printf("state: 7/7 public parameters seeded non-default, 0 opaque bytes\n");
	printf("preset-factory: independent\n");
	printf("topology: Audacity Compressor -> Master\n");
	printf("preset: %s\n", preset_path);
	printf("song: %s\n", song_path);
	return 0;
}
