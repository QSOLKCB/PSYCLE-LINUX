/*
** PSYCLE-LINUX Phase 5C Audacity WahWah production persistence test.
*/
...
#define SLOT 0u
#define CATCHER_NAME "wahwah:0"
#define PRESET_NAME "Phase 5 Audacity WahWah"
...
{
	fprintf(stderr, "phase5-audacity-wahwah-state: FAIL: %s\n", message);
	return 1;
}
	...
	const char* module_path)
{
	...
	if (!psy_audio_plugin_psycle_test(module_path, catcher->native_root_, &info)) {
		rc = fail("production native-plugin probe failed");
			...
			strcmp(info.name, "Audacity WahWah") != 0 ||
			!info.shortname || strcmp(info.shortname, "WahWah") != 0 ||
			!info.author ||
			strcmp(info.author, "Nasca Octavian Paul/Sartorius") != 0) {
		rc = fail("production probe returned unexpected Audacity WahWah identity");
	} else {
		machineinfo_catchername(&info, catcher_name);
		if (strcmp(catcher_name, CATCHER_NAME) != 0) {
			fprintf(stderr,
				"phase5-audacity-wahwah-state: FAIL: catcher expected %s got %s\n",
...
static int verify_identity(psy_audio_Machine* machine)
{
	...
	if (!machine || psy_audio_machine_type(machine) != psy_audio_PLUGIN)
		return fail("Audacity WahWah missing or no longer a native Psycle plugin");
	info = psy_audio_machine_info(machine);
	if (!info || !info->name || strcmp(info->name, "Audacity WahWah") != 0 ||
			!info->shortname || strcmp(info->shortname, "WahWah") != 0 ||
			!info->author ||
			strcmp(info->author, "Nasca Octavian Paul/Sartorius") != 0 ||
			!info->modulepath || !strstr(info->modulepath, "wahwah"))
	...
	if (psy_audio_machine_data_size(machine) != 0)
		return fail("Audacity WahWah unexpectedly acquired opaque plugin state");
	return 0;
}
...
static int verify_values(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		...
		if (actual != EXPECTED_VALUES[i]) {
			fprintf(stderr,
				"phase5-audacity-wahwah-state: FAIL: parameter %lu expected %ld got %ld\n",
...
int main(int argc, char** argv)
{
	...
	if (argc != 3) {
		fprintf(stderr, "usage: %s OUTPUT_DIRECTORY PATH_TO_WAHWAH_SO\n", argv[0]);
		return 2;
	}
	if (snprintf(song_path, sizeof(song_path), "%s/phase5-audacity-wahwah.psy", argv[1]) >=
			(int)sizeof(song_path) ||
			snprintf(preset_path, sizeof(preset_path), "%s/phase5-audacity-wahwah.prs", argv[1]) >=
	...
	if (!song) {
		rc = fail("could not allocate Audacity WahWah song");
		goto initial_cleanup;
	}
...
initial_cleanup:
	...
	printf("phase5-audacity-wahwah-state: PASS\n");
	printf("catcher: wahwah:0\n");
	printf("state: 5/5 public parameters seeded non-default, 0 opaque bytes\n");
	printf("preset-factory: independent\n");
	printf("topology: Audacity WahWah -> Master\n");
	printf("preset: %s\n", preset_path);
	printf("song: %s\n", song_path);
	return 0;
}
