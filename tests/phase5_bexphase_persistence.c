/*
** PSYCLE-LINUX Phase 5C BexPhase production persistence regression.
**
** Exercises the retained BexPhase effect through PluginCatcher/MachineFactory,
** version-1 preset I/O and an independently reopened PSY3 song.
*/
...
#define PARAMETER_COUNT 6u
#define SLOT 0u
#define CATCHER_NAME "bexphase:0"
#define PRESET_NAME "Phase 5 BexPhase"
...
{
	fprintf(stderr, "phase5-bexphase-state: FAIL: %s\n", message);
	return 1;
}
	...
	const char* module_path)
{
	...
	if (!psy_audio_plugin_psycle_test(module_path, catcher->native_root_, &info)) {
		rc = fail("production native-plugin probe failed");
			...
			strcmp(info.name, "DocBexter'S PhaZaR") != 0 ||
			!info.shortname || strcmp(info.shortname, "BexPhase!") != 0 ||
			!info.author || strcmp(info.author, "Simon Bucher") != 0) {
		rc = fail("production probe returned unexpected BexPhase identity");
	} else {
		machineinfo_catchername(&info, catcher_name);
		if (strcmp(catcher_name, CATCHER_NAME) != 0) {
			fprintf(stderr,
				"phase5-bexphase-state: FAIL: catcher expected %s got %s\n",
...
static int verify_identity(psy_audio_Machine* machine)
{
	...
	if (!machine || psy_audio_machine_type(machine) != psy_audio_PLUGIN)
		return fail("BexPhase missing or no longer a native Psycle plugin");
	info = psy_audio_machine_info(machine);
	if (!info || !info->name || strcmp(info->name, "DocBexter'S PhaZaR") != 0 ||
			!info->shortname || strcmp(info->shortname, "BexPhase!") != 0 ||
			!info->author || strcmp(info->author, "Simon Bucher") != 0 ||
			!info->modulepath || !strstr(info->modulepath, "bexphase"))
		return fail("BexPhase production identity/module path changed");
	if (psy_audio_machine_num_parameters(machine) != PARAMETER_COUNT)
		return fail("BexPhase production parameter count changed");
	if (psy_audio_machine_data_size(machine) != 0)
		return fail("BexPhase unexpectedly acquired opaque plugin state");
	return 0;
}
...
static int seed_parameters(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		...
		intptr_t minval;
		intptr_t maxval;
		if (!param) return fail("BexPhase parameter surface is incomplete");
		psy_audio_machine_parameter_range(machine, param, &minval, &maxval);
		if (EXPECTED_VALUES[i] < minval || EXPECTED_VALUES[i] > maxval)
			return fail("BexPhase preservation seed is outside public range");
		...
		if (psy_audio_machine_parameter_scaled_value(machine, param) != EXPECTED_VALUES[i])
			return fail("BexPhase seed parameter did not retain requested value");
	}
	return 0;
}
...
static int verify_values(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		psy_audio_MachineParam* param = psy_audio_machine_parameter(machine, i);
		intptr_t actual;
		if (!param) return fail("BexPhase parameter missing during verification");
		actual = psy_audio_machine_parameter_scaled_value(machine, param);
		if (actual != EXPECTED_VALUES[i]) {
			fprintf(stderr,
				"phase5-bexphase-state: FAIL: parameter %lu expected %ld got %ld\n",
	...
	const char* module_path)
{
	...
	if (psy_audio_preset_num_parameters(captured) != PARAMETER_COUNT ||
		...
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("captured BexPhase preset geometry changed");
	}
	...
	if (status != psy_audio_PRESETIO_OK) {
		rc = fail("BexPhase preset save failed");
		goto preset_cleanup;
	}
	...
	if (status != psy_audio_PRESETIO_OK || psy_audio_presets_size(&loaded) != 1) {
		rc = fail("BexPhase preset reload failed");
		goto preset_cleanup;
	}
	...
	if (!reloaded || strcmp(psy_audio_preset_name(reloaded), PRESET_NAME) != 0 ||
			...
			reloaded->datasize != 0) {
		rc = fail("reloaded BexPhase preset metadata changed");
		goto preset_cleanup;
	}
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		if (psy_audio_preset_value(reloaded, i) != EXPECTED_VALUES[i]) {
			rc = fail("reloaded BexPhase preset parameter changed");
			goto preset_cleanup;
		}
	}
...
int main(int argc, char** argv)
{
	...
	if (argc != 3) {
		fprintf(stderr, "usage: %s OUTPUT_DIRECTORY PATH_TO_BEXPHASE_SO\n", argv[0]);
		return 2;
	}
	if (snprintf(song_path, sizeof(song_path), "%s/phase5-bexphase.psy", argv[1]) >=
			(int)sizeof(song_path) ||
			snprintf(preset_path, sizeof(preset_path), "%s/phase5-bexphase.prs", argv[1]) >=
	...
	if (!song) {
		rc = fail("could not allocate BexPhase song");
		goto initial_cleanup;
	}
	...
	if (save_song(song, song_path) != 0) {
		rc = fail("BexPhase PSY3 save failed");
		goto song_cleanup;
	}
	...
	if (!loaded) {
		rc = fail("fresh BexPhase PSY3 reopen failed");
	} else {
		...
		if (verify_values(reopened) != 0) rc = 1;
				...
				psy_audio_wire_make(SLOT, psy_audio_MASTER_INDEX)))
			rc = fail("BexPhase wire to Master did not survive PSY3 reload");
...
initial_cleanup:
	...
	printf("phase5-bexphase-state: PASS\n");
	printf("catcher: bexphase:0\n");
	printf("state: 6/6 public parameters seeded non-default, 0 opaque bytes\n");
	printf("preset-factory: independent\n");
	printf("topology: DocBexter'S PhaZaR -> Master\n");
	printf("preset: %s\n", preset_path);
	printf("song: %s\n", song_path);
	return 0;
}
