/*
** PSYCLE-LINUX Phase 5C Crasher production persistence test.
**
** Crasher has no public parameters and no opaque payload.  This gate still
** exercises production discovery, a zero-parameter version-1 preset, and a
** fresh PSY3 reopen so the historical diagnostic identity remains loadable.
*/
...
#define SLOT 0u
#define CATCHER_NAME "crasher:0"
#define PRESET_NAME "Phase 5 Crasher"

static int fail(const char* message)
{
	fprintf(stderr, "phase5-crasher-state: FAIL: %s\n", message);
	return 1;
}
	...
	const char* module_path)
{
	...
	if (!psy_audio_plugin_psycle_test(module_path, catcher->native_root_, &info)) {
		rc = fail("production native-plugin probe failed");
			...
			strcmp(info.name, "crasher") != 0 ||
			!info.shortname || strcmp(info.shortname, "crasher") != 0 ||
			!info.author || strcmp(info.author, "bohan") != 0) {
		rc = fail("production probe returned unexpected Crasher identity");
	} else {
		machineinfo_catchername(&info, catcher_name);
		if (strcmp(catcher_name, CATCHER_NAME) != 0) {
			fprintf(stderr,
				"phase5-crasher-state: FAIL: catcher expected %s got %s\n",
...
static int verify_identity(psy_audio_Machine* machine)
{
	...
	if (!machine || psy_audio_machine_type(machine) != psy_audio_PLUGIN)
		return fail("Crasher missing or no longer a native Psycle plugin");
	info = psy_audio_machine_info(machine);
	if (!info || !info->name || strcmp(info->name, "crasher") != 0 ||
			!info->shortname || strcmp(info->shortname, "crasher") != 0 ||
			!info->author || strcmp(info->author, "bohan") != 0 ||
			!info->modulepath || !strstr(info->modulepath, "crasher"))
	...
	if (psy_audio_machine_data_size(machine) != 0)
		return fail("Crasher unexpectedly acquired opaque plugin state");
	return 0;
}
	...
	const char* module_path)
{
	...
	if (status != psy_audio_PRESETIO_OK) {
		rc = fail("zero-parameter preset save failed");
		goto preset_cleanup;
	}
	...
	if (status != psy_audio_PRESETIO_OK || psy_audio_presets_size(&loaded) != 1) {
		rc = fail("zero-parameter preset reload failed");
		goto preset_cleanup;
	}
	...
	if (!reloaded || strcmp(psy_audio_preset_name(reloaded), PRESET_NAME) != 0 ||
		...
		rc = fail("reloaded zero-parameter preset metadata changed");
		goto preset_cleanup;
	}
...
int main(int argc, char** argv)
{
	...
	if (argc != 3) {
		fprintf(stderr, "usage: %s OUTPUT_DIRECTORY PATH_TO_CRASHER_SO\n", argv[0]);
		return 2;
	}
	if (snprintf(song_path, sizeof(song_path), "%s/phase5-crasher.psy", argv[1]) >=
			(int)sizeof(song_path) ||
			snprintf(preset_path, sizeof(preset_path), "%s/phase5-crasher.prs", argv[1]) >=
	...
	if (!song) {
		rc = fail("could not allocate Crasher song");
		goto initial_cleanup;
	}
	...
	if (save_song(song, song_path) != 0) {
		rc = fail("Crasher PSY3 save failed");
		goto song_cleanup;
	}
	...
	if (!loaded) {
		rc = fail("fresh Crasher PSY3 reopen failed");
	} else {
		...
		if (verify_identity(reopened) != 0) rc = 1;
				...
				psy_audio_wire_make(SLOT, psy_audio_MASTER_INDEX)))
			rc = fail("Crasher wire to Master did not survive PSY3 reload");
...
initial_cleanup:
	...
	printf("phase5-crasher-state: PASS\n");
	printf("catcher: crasher:0\n");
	printf("state: 0 public parameters, 0 opaque bytes\n");
	printf("preset-factory: independent\n");
	printf("topology: crasher -> Master\n");
	printf("preset: %s\n", preset_path);
	printf("song: %s\n", song_path);
	return 0;
}
