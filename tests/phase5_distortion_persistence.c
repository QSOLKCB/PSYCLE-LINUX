/*
** PSYCLE-LINUX Phase 5C ayeternal Dist! Distortion production persistence.
**
** Exercises the retained effect through PluginCatcher, MachineFactory,
** version-1 preset I/O and fresh PSY3 save/reopen with all seven public state
...
#define SLOT 0u
#define CATCHER_NAME "distortion:0"
#define PRESET_NAME "Phase 5 Dist! Distortion"

static const intptr_t EXPECTED_VALUES[PARAMETER_COUNT] = {
	50000, 20000, 45000, 10000, 35000, 15000, 1
};

static int fail(const char* message)
{
	fprintf(stderr, "phase5-distortion-state: FAIL: %s\n", message);
	return 1;
}
	...
	const char* module_path)
{
	...
	if (!psy_audio_plugin_psycle_test(module_path, catcher->native_root_, &info)) {
		rc = fail("production native-plugin probe failed");
			...
			strcmp(info.name, "ayeternal Dist! Distortion") != 0 ||
			!info.shortname || strcmp(info.shortname, "Dist!ortion") != 0 ||
			!info.author || strcmp(info.author, "bohan") != 0) {
		rc = fail("production probe returned unexpected Dist! identity");
	} else {
		machineinfo_catchername(&info, catcher_name);
		if (strcmp(catcher_name, CATCHER_NAME) != 0) {
			fprintf(stderr,
				"phase5-distortion-state: FAIL: catcher expected %s got %s\n",
...
static int verify_identity(psy_audio_Machine* machine)
{
	...
	if (!machine || psy_audio_machine_type(machine) != psy_audio_PLUGIN)
		return fail("Dist!ortion missing or no longer a native Psycle plugin");
	info = psy_audio_machine_info(machine);
	if (!info || !info->name ||
			strcmp(info->name, "ayeternal Dist! Distortion") != 0 ||
			!info->shortname || strcmp(info->shortname, "Dist!ortion") != 0 ||
			!info->author || strcmp(info->author, "bohan") != 0 ||
			!info->modulepath || !strstr(info->modulepath, "distortion"))
		return fail("Dist!ortion production identity/module path changed");
	if (psy_audio_machine_num_parameters(machine) != PARAMETER_COUNT)
		return fail("Dist!ortion production parameter count changed");
	if (psy_audio_machine_data_size(machine) != 0)
		return fail("Dist!ortion unexpectedly acquired opaque plugin state");
	return 0;
}
...
static int verify_values(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		...
		if (actual != EXPECTED_VALUES[i]) {
			fprintf(stderr,
				"phase5-distortion-state: FAIL: parameter %lu expected %ld got %ld\n",
	...
	const char* module_path)
{
	...
	if (psy_audio_preset_num_parameters(captured) != PARAMETER_COUNT ||
		...
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("captured Dist! preset geometry changed");
	}
	...
	if (!reloaded || strcmp(psy_audio_preset_name(reloaded), PRESET_NAME) != 0 ||
		...
		rc = fail("reloaded Dist! preset metadata changed");
		goto preset_cleanup;
	}
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		if (psy_audio_preset_value(reloaded, i) != EXPECTED_VALUES[i]) {
			rc = fail("reloaded Dist! preset parameter changed");
			goto preset_cleanup;
		}
	}
...
int main(int argc, char** argv)
{
	...
	if (argc != 3) {
		fprintf(stderr, "usage: %s OUTPUT_DIRECTORY PATH_TO_DISTORTION_SO\n", argv[0]);
		return 2;
	}
	if (snprintf(song_path, sizeof(song_path), "%s/phase5-distortion.psy", argv[1]) >=
			(int)sizeof(song_path) ||
			snprintf(preset_path, sizeof(preset_path), "%s/phase5-distortion.prs", argv[1]) >=
	...
	if (!song) {
		rc = fail("could not allocate Dist! song");
		goto initial_cleanup;
	}
	...
	if (!loaded) {
		rc = fail("fresh Dist! PSY3 reopen failed");
	} else {
		...
		if (verify_values(reopened) != 0) rc = 1;
				...
				psy_audio_wire_make(SLOT, psy_audio_MASTER_INDEX)))
			rc = fail("Dist! wire to Master did not survive PSY3 reload");
...
initial_cleanup:
	...
	printf("phase5-distortion-state: PASS\n");
	printf("catcher: distortion:0\n");
	printf("state: 7/7 public parameters seeded non-default, symmetric=1, 0 opaque bytes\n");
	printf("preset-factory: independent\n");
	printf("topology: ayeternal Dist! Distortion -> Master\n");
	printf("preset: %s\n", preset_path);
	printf("song: %s\n", song_path);
	return 0;
}
