...
/* Hashes intentionally start at zero for the observation run. The smoke gate
** prints them; a follow-up commit freezes all seven before 5C/Druttis closes. */
const Spec SPECS[] = {
    {"EQ-3", "EQ-3", "EQ-3", 0x0110, psycle::plugin_interface::EFFECT,
        12, 4, Kind::Effect, UINT64_C(0)},
    {"FeedMe", "FeedMe 1.2", "FeedMe", 0x0120, psycle::plugin_interface::GENERATOR,
        24, 5, Kind::Generator, UINT64_C(0)},
    {"Koruz", "Koruz", "Koruz", 0x0110, psycle::plugin_interface::EFFECT,
        14, 2, Kind::Effect, UINT64_C(0)},
    {"Phantom", "Phantom 1.2", "Phantom", 0x0120, psycle::plugin_interface::GENERATOR,
        55, 5, Kind::Generator, UINT64_C(0)},
    {"Plucked String", "Plucked String 1.2", "Plucked String", 0x0120,
        psycle::plugin_interface::GENERATOR, 7, 1, Kind::Plucked, UINT64_C(0)},
    {"Slicit", "Slicit", "Slicit", 0x0100, psycle::plugin_interface::EFFECT,
        68, 4, Kind::Slicit, UINT64_C(0)},
    {"Sublime", "Sublime 1.1", "Sublime", 0x0110,
        psycle::plugin_interface::GENERATOR, 60, 4, Kind::Generator, UINT64_C(0)},
};
...
int verify_metadata(const Spec& spec, const CMachineInfo* info)
{
    ...
    if (!info->Name || std::strcmp(info->Name, spec.name) != 0 ||
            ...
            !author_is_druttis(info->Author) || info->numCols != spec.columns)
    ...
    CMachineInterface* (*create_machine)(), void (*delete_machine)(CMachineInterface&))
{
    ...
    /* EQ-3 and Koruz explicitly rebuild sample-rate dependent coefficients in
    ** SequencerTick. Compare a live transitioned machine with a fresh target-rate
    ** instance. Slicit derives step timing directly from callback values in Work. */
    if (spec.kind != Kind::Effect) return 0;
