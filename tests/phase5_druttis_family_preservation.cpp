...
void hash_int(std::uint64_t& hash, std::int32_t value)
{
    ...
    for (unsigned int i = 0; i < 4; ++i) hash_byte(hash, static_cast<unsigned char>((u >> (i * 8)) & 0xffu));
}
...
std::uint64_t parameter_hash(const CMachineInfo* info)
{
    ...
    for (int i = 0; i < info->numParameters; ++i) {
        ...
        if (p) { hash_int(hash, p->MinValue); hash_int(hash, p->MaxValue); hash_int(hash, p->Flags); hash_int(hash, p->DefValue); }
    }
    return hash;
}
...
int verify_metadata(const Spec& spec, const CMachineInfo* info)
{
    ...
    if (info->APIVersion != psycle::plugin_interface::MI_VERSION) return fail(spec, "native-machine API version changed");
    if (info->PlugVersion != spec.plug_version) return fail(spec, "plugin version changed");
    if (info->Flags != spec.flags) return fail(spec, "generator/effect classification changed");
    if (info->numParameters != spec.parameter_count) return fail(spec, "parameter count changed");
    if (!info->Name || std::strcmp(info->Name, spec.name) != 0 || !info->ShortName ||
            ...
            info->numCols != spec.columns) return fail(spec, "machine identity metadata changed");
    if (!info->Parameters) return fail(spec, "parameter table is missing");
    for (int i = 0; i < info->numParameters; ++i) {
        const CMachineParameter* p = info->Parameters[i];
        if (!p || !p->Name || !p->Description) return fail(spec, "parameter metadata contains a null entry");
        if (p->MinValue > p->MaxValue) return fail(spec, "parameter range is inverted");
        if (p->Flags == psycle::plugin_interface::MPF_STATE && (p->DefValue < p->MinValue || p->DefValue > p->MaxValue))
            return fail(spec, "state parameter default is outside its range");
    }
    const std::uint64_t actual = parameter_hash(info);
    std::printf("druttis-metadata-hash[%s]=0x%016llx\n", spec.label, static_cast<unsigned long long>(actual));
    if (spec.metadata_hash != 0 && actual != spec.metadata_hash) return fail(spec, "frozen parameter metadata hash changed");
    return 0;
}
...
void apply_defaults(CMachineInterface* machine, const CMachineInfo* info, TestCallback& callback)
{
    ...
    for (int i = 0; i < info->numParameters; ++i) machine->Vals[i] = info->Parameters[i]->DefValue;
    machine->Init();
    for (int i = 0; i < info->numParameters; ++i)
        if (info->Parameters[i]->Flags == psycle::plugin_interface::MPF_STATE) machine->ParameterTweak(i, info->Parameters[i]->DefValue);
}

bool finite_signal(const std::vector<float>& left, const std::vector<float>& right)
{
    if (left.size() != right.size()) return false;
    for (std::size_t i = 0; i < left.size(); ++i) if (!std::isfinite(left[i]) || !std::isfinite(right[i])) return false;
    return true;
}

bool nonzero_signal(const std::vector<float>& left, const std::vector<float>& right)
{
    for (std::size_t i = 0; i < left.size(); ++i) if (std::fabs(left[i]) > 1.0e-8f || std::fabs(right[i]) > 1.0e-8f) return true;
    return false;
}

bool same_signal(const std::vector<float>& a_l, const std::vector<float>& a_r,
    const std::vector<float>& b_l, const std::vector<float>& b_r, float tolerance = 1.0e-5f)
{
    if (a_l.size() != b_l.size() || a_r.size() != b_r.size() || !finite_signal(a_l, a_r) || !finite_signal(b_l, b_r)) return false;
    for (std::size_t i = 0; i < a_l.size(); ++i)
        if (std::fabs(a_l[i] - b_l[i]) > tolerance || std::fabs(a_r[i] - b_r[i]) > tolerance) return false;
    return true;
}

bool different_signal(const std::vector<float>& a_l, const std::vector<float>& a_r,
    const std::vector<float>& b_l, const std::vector<float>& b_r, float tolerance = 1.0e-4f)
{
    if (a_l.size() != b_l.size() || a_r.size() != b_r.size() || !finite_signal(a_l, a_r) || !finite_signal(b_l, b_r)) return false;
    bool different = false;
    for (std::size_t i = 0; i < a_l.size(); ++i)
        if (std::fabs(a_l[i] - b_l[i]) > tolerance || std::fabs(a_r[i] - b_r[i]) > tolerance) different = true;
    return different;
}

void make_probe(std::vector<float>& left, std::vector<float>& right, int count)
{
    left.resize(count); right.resize(count);
...
{
    machine->ParameterTweak(0, 180); machine->ParameterTweak(1, 180);
    machine->ParameterTweak(3, 96); machine->ParameterTweak(4, 96);
    machine->ParameterTweak(6, 160); machine->ParameterTweak(7, 160);
}
...
{
    TestCallback callback_a, callback_b;
    ...
    if (!a || !a->Vals || !b || !b->Vals) {
        if (a) delete_machine(*a); if (b) delete_machine(*b);
        return fail(spec, "CreateMachine failed for effect regression");
    }
    apply_defaults(a, info, callback_a); apply_defaults(b, info, callback_b);
    make_probe(input_l, input_r, 2048);
    a_l = input_l; a_r = input_r; b_l = input_l; b_r = input_r;
    a->Work(a_l.data(), a_r.data(), static_cast<int>(a_l.size()), 1);
    b->Work(b_l.data(), b_r.data(), static_cast<int>(b_l.size()), 1);
    if (!finite_signal(a_l, a_r) || !finite_signal(b_l, b_r)) rc = fail(spec, "effect produced a non-finite sample");
    else if (!same_signal(a_l, a_r, b_l, b_r, 1.0e-5f)) rc = fail(spec, "fresh effect instances are not deterministic");
    else std::printf("phase5-druttis-family: deterministic effect PASS [%s]\n", spec.label);
    delete_machine(*b); delete_machine(*a);
    if (rc != 0 || spec.kind != Kind::Effect) return rc;

    TestCallback live_cb, stale_cb;
    CMachineInterface* live = create_machine();
    CMachineInterface* stale = (std::strcmp(spec.label, "EQ-3") == 0) ? create_machine() : nullptr;
    if (!live || !live->Vals || (std::strcmp(spec.label, "EQ-3") == 0 && (!stale || !stale->Vals))) {
        if (stale) delete_machine(*stale); if (live) delete_machine(*live);
    ...
    if (stale) { configure_eq3_active(live); configure_eq3_active(stale); }
    live_cb.set_sample_rate(88200);
    live->SequencerTick();
    make_probe(input_l, input_r, 2048);
    a_l = input_l; a_r = input_r;
    live->Work(a_l.data(), a_r.data(), static_cast<int>(a_l.size()), 1);
    if (!finite_signal(a_l, a_r)) { if (stale) delete_machine(*stale); delete_machine(*live); return fail(spec, "live sample-rate transition produced non-finite audio"); }

    if (stale) {
        b_l = input_l; b_r = input_r;
        stale->Work(b_l.data(), b_r.data(), static_cast<int>(b_l.size()), 1);
        if (!different_signal(a_l, a_r, b_l, b_r, 1.0e-4f)) rc = fail(spec, "EQ-3 active response did not change after sample-rate transition");
        else std::printf("phase5-druttis-family: live rate response changed PASS [%s]\n", spec.label);
        delete_machine(*stale); delete_machine(*live); return rc;
    }

    TestCallback ref_cb; ref_cb.set_sample_rate(88200);
    CMachineInterface* ref = create_machine();
    if (!ref || !ref->Vals) { if (ref) delete_machine(*ref); delete_machine(*live); return fail(spec, "CreateMachine failed for target-rate reference"); }
    apply_defaults(ref, info, ref_cb);
    b_l = input_l; b_r = input_r;
    ref->Work(b_l.data(), b_r.data(), static_cast<int>(b_l.size()), 1);
    if (!same_signal(a_l, a_r, b_l, b_r, 1.0e-4f)) rc = fail(spec, "live sample-rate transition diverged from fresh target rate");
    else std::printf("phase5-druttis-family: live 44.1->88.2 effect transition PASS [%s]\n", spec.label);
    delete_machine(*ref); delete_machine(*live); return rc;
}

int render_note(const Spec& spec, CMachineInterface* machine, int count, std::vector<float>& left, std::vector<float>& right)
{
    machine->SeqTick(0, 69, 0, 0, 0);
    left.assign(count, 0.0f); right.assign(count, 0.0f);
...
{
    TestCallback callback_a, callback_b;
    CMachineInterface* a = create_machine(); CMachineInterface* b = create_machine();
    ...
    if (rc == 0 && spec.kind != Kind::Plucked && !same_signal(a_l, a_r, b_l, b_r, 1.0e-5f)) rc = fail(spec, "fresh generator instances are not deterministic");
    if (rc == 0) std::printf("phase5-druttis-family: active generator PASS [%s]\n", spec.label);
    delete_machine(*b);
    if (rc != 0) { delete_machine(*a); return rc; }
    a->Stop(); callback_a.set_sample_rate(88200); a->SequencerTick();
    rc = render_note(spec, a, 16384, a_l, a_r);
    if (rc == 0) std::printf("phase5-druttis-family: live 44.1->88.2 generator transition PASS [%s]\n", spec.label);
    delete_machine(*a); return rc;
}

int test_plugin(const Spec& spec, const char* path)
{
    using GetInfoFn = const CMachineInfo* (*)(); using CreateMachineFn = CMachineInterface* (*)(); using DeleteMachineFn = void (*)(CMachineInterface&);
    void* library = dlopen(path, RTLD_LAZY | RTLD_LOCAL);
    if (!library) { std::fprintf(stderr, "phase5-druttis-family: FAIL [%s]: dlopen: %s\n", spec.label, dlerror()); return 1; }
    dlerror();
    ...
    if (error || !get_info || !create_machine || !delete_machine) {
        std::fprintf(stderr, "phase5-druttis-family: FAIL [%s]: ABI exports missing: %s\n", spec.label, error ? error : "unknown symbol error"); dlclose(library); return 1;
    }
    const CMachineInfo* info = get_info();
    int rc = verify_metadata(spec, info);
    if (rc == 0) rc = (spec.flags == psycle::plugin_interface::GENERATOR) ? verify_generator(spec, info, create_machine, delete_machine) : verify_effect(spec, info, create_machine, delete_machine);
...
int main(int argc, char** argv)
{
    ...
    if (argc != count + 1) {
        std::fprintf(stderr, "usage: %s EQ3.so FEEDME.so KORUZ.so PHANTOM.so PLUCKED.so SLICIT.so SUBLIME.so\n", argv[0]); return 2;
    }
    for (int i = 0; i < count; ++i) if (test_plugin(SPECS[i], argv[i + 1]) != 0) return 1;
