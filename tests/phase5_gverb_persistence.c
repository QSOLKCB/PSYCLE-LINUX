/*
** PSYCLE-LINUX Phase 5C LADSPA GVerb production persistence.
**
** Exercises the retained effect through PluginCatcher, MachineFactory,
** version-1 preset I/O and fresh PSY3 save/reopen with all eight public state
...
#define SLOT 0u
#define CATCHER_NAME "ladspa-gverb:0"
#define PRESET_NAME "Phase 5 LADSPA GVerb"

static const intptr_t EXPECTED_VALUES[PARAMETER_COUNT] = {
	200, 1200, 700, 800, -6000, -9000, -12000, 1
};

static int fail(const char* message)
{
	fprintf(stderr, "phase5-gverb-state: FAIL: %s\n", message);
	return 1;
}
	...
	const char* module_path)
{
	...
	if (!psy_audio_plugin_psycle_test(module_path, catcher->native_root_, &info)) {
		rc = fail("production native-plugin probe failed");
			...
			strcmp(info.name, "LADSPA GVerb") != 0 ||
			!info.shortname || strcmp(info.shortname, "GVerb") != 0 ||
			!info.author ||
			strcmp(info.author, "Juhana Sadeharju/Steve Harris/Sartorius") != 0) {
		rc = fail("production probe returned unexpected GVerb identity");
	} else {
		machineinfo_catchername(&info, catcher_name);
		if (strcmp(catcher_name, CATCHER_NAME) != 0) {
			fprintf(stderr,
				"phase5-gverb-state: FAIL: catcher expected %s got %s\n",
...
static int verify_identity(psy_audio_Machine* machine)
{
	...
	if (!machine || psy_audio_machine_type(machine) != psy_audio_PLUGIN)
		return fail("GVerb missing or no longer a native Psycle plugin");
	info = psy_audio_machine_info(machine);
	if (!info || !info->name || strcmp(info->name, "LADSPA GVerb") != 0 ||
			!info->shortname || strcmp(info->shortname, "GVerb") != 0 ||
			!info->author ||
			strcmp(info->author, "Juhana Sadeharju/Steve Harris/Sartorius") != 0 ||
			!info->modulepath || !strstr(info->modulepath, "gverb"))
		return fail("GVerb production identity/module path changed");
	if (psy_audio_machine_num_parameters(machine) != PARAMETER_COUNT)
		return fail("GVerb production parameter count changed");
	if (psy_audio_machine_data_size(machine) != 0)
		return fail("GVerb unexpectedly acquired opaque plugin state");
	return 0;
}
...
static int verify_values(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		...
		if (actual != EXPECTED_VALUES[i]) {
			fprintf(stderr,
				"phase5-gverb-state: FAIL: parameter %lu expected %ld got %ld\n",
	...
	const char* module_path)
{
	...
	if (psy_audio_preset_num_parameters(captured) != PARAMETER_COUNT ||
		...
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("captured GVerb preset geometry changed");
	}
	...
	if (status != psy_audio_PRESETIO_OK) {
		rc = fail("GVerb preset save failed");
		goto preset_cleanup;
	}
	...
	if (status != psy_audio_PRESETIO_OK || psy_audio_presets_size(&loaded) != 1) {
		rc = fail("GVerb preset reload failed");
		goto preset_cleanup;
	}
	...
	if (!reloaded || strcmp(psy_audio_preset_name(reloaded), PRESET_NAME) != 0 ||
		...
		rc = fail("reloaded GVerb preset metadata changed");
		goto preset_cleanup;
	}
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		if (psy_audio_preset_value(reloaded, i) != EXPECTED_VALUES[i]) {
			rc = fail("reloaded GVerb preset parameter changed");
			goto preset_cleanup;
		}
	}
...
int main(int argc, char** argv)
{
	...
	if (argc != 3) {
		fprintf(stderr, "usage: %s OUTPUT_DIRECTORY PATH_TO_GVERB_SO\n", argv[0]);
		return 2;
	}
	if (snprintf(song_path, sizeof(song_path), "%s/phase5-gverb.psy", argv[1]) >=
			(int)sizeof(song_path) ||
			snprintf(preset_path, sizeof(preset_path), "%s/phase5-gverb.prs", argv[1]) >=
	...
	if (!song) {
		rc = fail("could not allocate GVerb song");
		goto initial_cleanup;
	}
	...
	if (save_song(song, song_path) != 0) {
		rc = fail("GVerb PSY3 save failed");
		goto song_cleanup;
	}
	...
	if (!loaded) {
		rc = fail("fresh GVerb PSY3 reopen failed");
	} else {
		...
		if (verify_values(reopened) != 0) rc = 1;
				...
				psy_audio_wire_make(SLOT, psy_audio_MASTER_INDEX)))
			rc = fail("GVerb wire to Master did not survive PSY3 reload");
...
initial_cleanup:
	...
	printf("phase5-gverb-state: PASS\n");
	printf("catcher: ladspa-gverb:0\n");
	printf("state: 8/8 public parameters seeded non-default, input=stereo, 0 opaque bytes\n");
	printf("preset-factory: independent\n");
	printf("topology: LADSPA GVerb -> Master\n");
	printf("preset: %s\n", preset_path);
	printf("song: %s\n", song_path);
	return 0;
}
