/*
** PSYCLE-LINUX Phase 5C LegaSynth production persistence regression.
*/
...
#define PARAMETER_COUNT 28u
#define STATE_COUNT 25u
#define SLOT 0u
#define CATCHER_NAME "legasynth-303:0"
#define PRESET_NAME "Phase 5 LegaSynth"

static const intptr_t EXPECTED_VALUES[PARAMETER_COUNT] = {
	5, -25, 9000, 7, 100, 30, 0,
	40000, -20, 12, 1000, 20000, 30000, 40000, 0,
	20, 30, 2, 40, 50, -30, 0,
	1, 20, 24, 10, 50, 64
};

static const int EXPECTED_TYPES[PARAMETER_COUNT] = {
	MPF_STATE, MPF_STATE, MPF_STATE, MPF_STATE, MPF_STATE, MPF_STATE, MPF_HEADER,
	MPF_STATE, MPF_STATE, MPF_STATE, MPF_STATE, MPF_STATE, MPF_STATE, MPF_STATE, MPF_HEADER,
	MPF_STATE, MPF_STATE, MPF_STATE, MPF_STATE, MPF_STATE, MPF_STATE, MPF_HEADER,
	MPF_STATE, MPF_STATE, MPF_STATE, MPF_STATE, MPF_STATE, MPF_STATE
};

static int fail(const char* message)
{
	fprintf(stderr, "phase5-legasynth-state: FAIL: %s\n", message);
	return 1;
}
	...
	const char* module_path)
{
	...
	if (!psy_audio_plugin_psycle_test(module_path, catcher->native_root_, &info)) {
		rc = fail("production native-plugin probe failed");
			...
			strcmp(info.name, "LegaSynth TB303") != 0 ||
			!info.shortname || strcmp(info.shortname, "TB303") != 0 ||
			!info.author || strcmp(info.author,
				"Juan Linietsky, ported by Sartorius") != 0) {
		rc = fail("production probe returned unexpected LegaSynth identity");
	} else {
		machineinfo_catchername(&info, catcher_name);
		if (strcmp(catcher_name, CATCHER_NAME) != 0) {
			fprintf(stderr,
				"phase5-legasynth-state: FAIL: catcher expected %s got %s\n",
...
static int verify_identity(psy_audio_Machine* machine)
{
	...
	if (!machine || psy_audio_machine_type(machine) != psy_audio_PLUGIN)
		return fail("LegaSynth missing or no longer a native Psycle plugin");
	info = psy_audio_machine_info(machine);
	if (!info || !info->name || strcmp(info->name, "LegaSynth TB303") != 0 ||
			!info->shortname || strcmp(info->shortname, "TB303") != 0 ||
			!info->author || strcmp(info->author,
				"Juan Linietsky, ported by Sartorius") != 0 ||
			!info->modulepath || !strstr(info->modulepath, "legasynth-303"))
		return fail("LegaSynth production identity/module path changed");
	if (psy_audio_machine_num_parameters(machine) != PARAMETER_COUNT)
		return fail("LegaSynth 28-slot production parameter count changed");
	if (psy_audio_machine_data_size(machine) != 0)
		return fail("LegaSynth unexpectedly acquired opaque plugin state");

	for (i = 0; i < PARAMETER_COUNT; ++i) {
		psy_audio_MachineParam* param = psy_audio_machine_parameter(machine, i);
		int type;
		if (!param) return fail("LegaSynth production parameter surface is incomplete");
		type = psy_audio_machine_parameter_type(machine, param) & 0x1FF;
		if (type != EXPECTED_TYPES[i]) {
			fprintf(stderr,
				"phase5-legasynth-state: FAIL: slot %lu expected type %d got %d\n",
	...
	if (state_count != STATE_COUNT)
		return fail("LegaSynth 25-state/three-header partition changed");
	return 0;
}
...
static int seed_parameters(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		...
		if (EXPECTED_VALUES[i] < minval || EXPECTED_VALUES[i] > maxval)
			return fail("LegaSynth preservation seed is outside public range");
...
static int verify_values(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		...
		if (actual != EXPECTED_VALUES[i]) {
			fprintf(stderr,
				"phase5-legasynth-state: FAIL: slot %lu expected %ld got %ld\n",
	...
	const char* module_path)
{
	...
	if (psy_audio_preset_num_parameters(captured) != PARAMETER_COUNT ||
		...
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("captured LegaSynth preset geometry changed");
	}
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		if (psy_audio_preset_value(captured, i) != EXPECTED_VALUES[i]) {
			...
			psy_audio_presets_dispose(&saved);
			psy_audio_presets_dispose(&loaded);
			return fail("captured LegaSynth preset slot changed");
		}
	}
	...
	if (status != psy_audio_PRESETIO_OK) {
		rc = fail("LegaSynth preset save failed");
		goto cleanup;
	}
	...
	if (status != psy_audio_PRESETIO_OK || psy_audio_presets_size(&loaded) != 1) {
		rc = fail("LegaSynth preset reload failed");
		goto cleanup;
	}
	...
	if (!reloaded || strcmp(psy_audio_preset_name(reloaded), PRESET_NAME) != 0 ||
		...
		rc = fail("reloaded LegaSynth preset metadata changed");
		goto cleanup;
	}
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		if (psy_audio_preset_value(reloaded, i) != EXPECTED_VALUES[i]) {
			rc = fail("reloaded LegaSynth preset parameter changed");
			goto cleanup;
		}
	}
...
int main(int argc, char** argv)
{
	...
	if (argc != 3) {
		fprintf(stderr, "usage: %s OUTPUT_DIRECTORY PATH_TO_LEGASYNTH_SO\n", argv[0]);
		return 2;
	}
	if (snprintf(song_path, sizeof(song_path), "%s/phase5-legasynth.psy", argv[1]) >=
			(int)sizeof(song_path) ||
			snprintf(preset_path, sizeof(preset_path), "%s/phase5-legasynth.prs", argv[1]) >=
	...
	if (!song) {
		rc = fail("could not allocate LegaSynth song");
		goto initial_cleanup;
	}
	...
	if (save_song(song, song_path) != 0) {
		rc = fail("LegaSynth PSY3 save failed");
		goto song_cleanup;
	}
	...
	if (!loaded) {
		rc = fail("fresh LegaSynth PSY3 reopen failed");
	} else {
		...
		if (verify_values(reopened) != 0) rc = 1;
				...
				psy_audio_wire_make(SLOT, psy_audio_MASTER_INDEX)))
			rc = fail("LegaSynth wire to Master did not survive PSY3 reload");
...
initial_cleanup:
	...
	printf("phase5-legasynth-state: PASS\n");
	printf("catcher: legasynth-303:0\n");
	printf("state: 25 MPF_STATE controls seeded non-default across 28 ABI slots, separators=3-header, chorus=on, 0 opaque bytes\n");
	printf("preset-factory: independent\n");
	printf("topology: LegaSynth TB303 -> Master\n");
	printf("preset: %s\n", preset_path);
	printf("song: %s\n", song_path);
	return 0;
}
