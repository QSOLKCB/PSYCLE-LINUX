/*
** PSYCLE-LINUX Phase 5C Ninereeds Fractal 7900s production persistence.
*/
...
#define PARAMETER_COUNT 2u
#define SLOT 0u
#define CATCHER_NAME "nrs-7900-fractal:0"
#define PRESET_NAME "Phase 5 Ninereeds Fractal"
#define EXPECTED_EFFECT 65534
#define EXPECTED_DEPTH 2

static int fail(const char* message)
{
	fprintf(stderr, "phase5-ninereeds-fractal-state: FAIL: %s\n", message);
	return 1;
}
	...
	const char* module_path)
{
	...
	if (!psy_audio_plugin_psycle_test(module_path, catcher->native_root_, &info)) {
		rc = fail("production native-plugin probe failed");
			...
			strcmp(info.name, "Ninereeds Fractal 7900s Port") != 0 ||
			!info.shortname || strcmp(info.shortname, "Fractal Dist") != 0 ||
			!info.author || strcmp(info.author, "Ninereeds & 7900") != 0 ||
			info.plugversion != 0x0002) {
		rc = fail("production probe returned unexpected Fractal identity");
	} else {
		machineinfo_catchername(&info, catcher_name);
		if (strcmp(catcher_name, CATCHER_NAME) != 0) {
			fprintf(stderr, "phase5-ninereeds-fractal-state: FAIL: catcher expected %s got %s\n",
...
static int verify_identity(psy_audio_Machine* machine)
{
	...
	if (!machine || psy_audio_machine_type(machine) != psy_audio_PLUGIN)
		return fail("Fractal missing or no longer a native Psycle plugin");
	info = psy_audio_machine_info(machine);
	if (!info || !info->name || strcmp(info->name, "Ninereeds Fractal 7900s Port") != 0 ||
			!info->shortname || strcmp(info->shortname, "Fractal Dist") != 0 ||
			!info->author || strcmp(info->author, "Ninereeds & 7900") != 0 ||
			info->plugversion != 0x0002 || !info->modulepath ||
			!strstr(info->modulepath, "nrs-7900-fractal"))
		return fail("Fractal production identity/module path changed");
	if (psy_audio_machine_num_parameters(machine) != PARAMETER_COUNT)
		return fail("Fractal production parameter count changed");
	if (psy_audio_machine_data_size(machine) != 0)
		return fail("Fractal unexpectedly acquired opaque plugin state");
	return 0;
}

static int seed_parameters(psy_audio_Machine* machine)
{
	psy_audio_MachineParam* effect = psy_audio_machine_parameter(machine, 0);
	psy_audio_MachineParam* depth = psy_audio_machine_parameter(machine, 1);
	intptr_t minval, maxval;
	if (!effect || !depth) return fail("production parameters missing");
	psy_audio_machine_parameter_range(machine, effect, &minval, &maxval);
	if (minval != 0 || maxval != 65534) return fail("effect production range changed");
	psy_audio_machine_parameter_range(machine, depth, &minval, &maxval);
	if (minval != 0 || maxval != 32) return fail("depth production range changed");
	psy_audio_machine_parameter_tweak_scaled(machine, effect, EXPECTED_EFFECT);
	psy_audio_machine_parameter_tweak_scaled(machine, depth, EXPECTED_DEPTH);
	return 0;
}

static int verify_values(psy_audio_Machine* machine)
{
	psy_audio_MachineParam* effect;
	psy_audio_MachineParam* depth;
	if (verify_identity(machine) != 0) return 1;
	effect = psy_audio_machine_parameter(machine, 0);
	depth = psy_audio_machine_parameter(machine, 1);
	if (!effect || !depth) return fail("parameters missing during verification");
	if (psy_audio_machine_parameter_scaled_value(machine, effect) != EXPECTED_EFFECT ||
			psy_audio_machine_parameter_scaled_value(machine, depth) != EXPECTED_DEPTH)
		return fail("persisted Fractal parameter values changed");
	return 0;
}
...
static int verify_production_audio(psy_audio_Machine* machine)
{
	...
	float* left;
	float* right;
	const float expected_left[] = {27333.1406f, -8888.6250f, 8888.5313f, -27333.0938f};
	const float expected_right[] = {-27333.0938f, 8888.5313f, -8888.6250f, 27333.1406f};
	int i;
	int rc = 0;
	...
	left[0] = -30000.0f; left[1] = -1000.0f; left[2] = 1000.0f; left[3] = 30000.0f; left[4] = 12345.0f;
	right[0] = 30000.0f; right[1] = 1000.0f; right[2] = -1000.0f; right[3] = -30000.0f; right[4] = -12345.0f;
	psy_audio_buffercontext_init(&bc, NULL, &buffer, &buffer, 4, 1);
	psy_audio_machine_work(machine, &bc);
	for (i = 0; i < 4; ++i) {
		if (fabsf(left[i] - expected_left[i]) > 0.08f ||
				fabsf(right[i] - expected_right[i]) > 0.08f) {
			rc = fail("production Fractal source-derived audio marker changed");
			break;
		}
	}
	if (rc == 0 && (left[4] != 12345.0f || right[4] != -12345.0f))
		rc = fail("production Fractal work crossed requested block boundary");
	...
	const char* module_path)
{
	...
	if (psy_audio_preset_num_parameters(captured) != PARAMETER_COUNT ||
			...
			psy_audio_preset_value(captured, 0) != EXPECTED_EFFECT ||
			psy_audio_preset_value(captured, 1) != EXPECTED_DEPTH) {
		psy_audio_preset_dispose(captured); free(captured);
		psy_audio_presets_dispose(&saved); psy_audio_presets_dispose(&loaded);
		return fail("captured Fractal preset state changed");
	}
	psy_audio_presets_insert(&saved, 0, captured);
	status = psy_audio_presetsio_save(preset_path, &saved);
	if (status != psy_audio_PRESETIO_OK) { rc = fail("Fractal preset save failed"); goto cleanup; }
	...
	if (status != psy_audio_PRESETIO_OK || psy_audio_presets_size(&loaded) != 1) {
		rc = fail("Fractal preset reload failed"); goto cleanup;
	}
	...
	if (!reloaded || strcmp(psy_audio_preset_name(reloaded), PRESET_NAME) != 0 ||
			...
			psy_audio_preset_value(reloaded, 0) != EXPECTED_EFFECT ||
			psy_audio_preset_value(reloaded, 1) != EXPECTED_DEPTH) {
		rc = fail("reloaded Fractal preset state changed"); goto cleanup;
	}
	...
	if (register_native_plugin(&catcher, module_path) != 0) rc = 1;
		...
		else { psy_audio_machine_tweak_preset(fresh, reloaded); rc = verify_values(fresh); }
	}
...
{
	char song_path[4096], preset_path[4096];
	...
	int rc = 0;
	if (argc != 3) {
		fprintf(stderr, "usage: %s OUTPUT_DIRECTORY PATH_TO_FRACTAL_SO\n", argv[0]);
		return 2;
	}
	if (snprintf(song_path, sizeof(song_path), "%s/phase5-ninereeds-fractal.psy", argv[1]) >= (int)sizeof(song_path) ||
			snprintf(preset_path, sizeof(preset_path), "%s/phase5-ninereeds-fractal.prs", argv[1]) >= (int)sizeof(preset_path))
	...
	psy_audio_machinefactory_init(&factory, &callback, &catcher, NULL);
	psy_audio_machinefactory_createwithoutproxy(&factory);
	if (register_native_plugin(&catcher, argv[2]) != 0) { rc = 1; goto initial_cleanup; }
	song = psy_audio_song_alloc_init(&factory);
	if (!song) { rc = fail("could not allocate Fractal song"); goto initial_cleanup; }
	psy_audio_machinecallback_set_song(&callback, song);
	machine = make_machine(&factory);
	if (!machine || verify_identity(machine) || seed_parameters(machine) || verify_values(machine) ||
			verify_production_audio(machine) || preset_roundtrip(machine, preset_path, argv[2])) {
		rc = 1;
	...
	machine = NULL;
	psy_audio_machines_connect(psy_audio_song_machines(song), psy_audio_wire_make(SLOT, psy_audio_MASTER_INDEX));
	if (save_song(song, song_path) != 0) { rc = fail("Fractal PSY3 save failed"); goto song_cleanup; }
	...
	if (!loaded) rc = fail("fresh Fractal PSY3 reopen failed");
	else {
		...
		if (verify_values(reopened) != 0) rc = 1;
			...
			rc = fail("Fractal wire to Master did not survive PSY3 reload");
		psy_audio_song_deallocate(loaded); loaded = NULL;
	}
...
initial_cleanup:
	...
	psy_audio_dispose();
	if (rc != 0) return rc;
	printf("phase5-ninereeds-fractal-state: PASS\n");
	printf("catcher: nrs-7900-fractal:0\n");
	printf("state: 2/2 MPF_STATE controls seeded non-default effect=65534 depth=2, 0 opaque bytes\n");
	printf("audio: production cubic recurrence markers=8 bounded=yes zero-callback=noop\n");
	printf("preset-factory: independent\n");
	printf("topology: Ninereeds Fractal 7900s Port -> Master\n");
	printf("preset: %s\n", preset_path);
	printf("song: %s\n", song_path);
	return 0;
}
