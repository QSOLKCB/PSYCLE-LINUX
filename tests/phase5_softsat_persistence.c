/*
** PSYCLE-LINUX Phase 5C ThunderPalace SoftSat production persistence.
**
** Exercises the retained effect through PluginCatcher, MachineFactory,
** version-1 preset I/O and fresh PSY3 save/reopen with both public state
** parameters seeded to legal non-default values.
*/
...
#define SLOT 0u
#define CATCHER_NAME "thunderpalace-softsat:0"
#define PRESET_NAME "Phase 5 ThunderPalace SoftSat"

static const intptr_t EXPECTED_VALUES[PARAMETER_COUNT] = { 12000, 1536 };

static int fail(const char* message)
{
	fprintf(stderr, "phase5-softsat-state: FAIL: %s\n", message);
	return 1;
}
	...
	const char* module_path)
{
	...
	if (!psy_audio_plugin_psycle_test(module_path, catcher->native_root_, &info)) {
		rc = fail("production native-plugin probe failed");
			...
			strcmp(info.name, "ThunderPalace SoftSat") != 0 ||
			!info.shortname || strcmp(info.shortname, "SoftSat") != 0 ||
			!info.author || strcmp(info.author, "Catatonic Porpoise") != 0) {
		rc = fail("production probe returned unexpected SoftSat identity");
	} else {
		machineinfo_catchername(&info, catcher_name);
		if (strcmp(catcher_name, CATCHER_NAME) != 0) {
			fprintf(stderr,
				"phase5-softsat-state: FAIL: catcher expected %s got %s\n",
...
static int verify_identity(psy_audio_Machine* machine)
{
	...
	if (!machine || psy_audio_machine_type(machine) != psy_audio_PLUGIN)
		return fail("SoftSat missing or no longer a native Psycle plugin");
	info = psy_audio_machine_info(machine);
	if (!info || !info->name || strcmp(info->name, "ThunderPalace SoftSat") != 0 ||
			!info->shortname || strcmp(info->shortname, "SoftSat") != 0 ||
			!info->author || strcmp(info->author, "Catatonic Porpoise") != 0 ||
			!info->modulepath || !strstr(info->modulepath, "thunderpalace-softsat"))
		return fail("SoftSat production identity/module path changed");
	if (psy_audio_machine_num_parameters(machine) != PARAMETER_COUNT)
		return fail("SoftSat production parameter count changed");
	if (psy_audio_machine_data_size(machine) != 0)
		return fail("SoftSat unexpectedly acquired opaque plugin state");
	return 0;
}
...
static int verify_values(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		...
		if (actual != EXPECTED_VALUES[i]) {
			fprintf(stderr,
				"phase5-softsat-state: FAIL: parameter %lu expected %ld got %ld\n",
	...
	const char* module_path)
{
	...
	if (psy_audio_preset_num_parameters(captured) != PARAMETER_COUNT ||
		...
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("captured SoftSat preset geometry changed");
	}
	...
	if (!reloaded || strcmp(psy_audio_preset_name(reloaded), PRESET_NAME) != 0 ||
		...
		rc = fail("reloaded SoftSat preset metadata changed");
		goto preset_cleanup;
	}
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		if (psy_audio_preset_value(reloaded, i) != EXPECTED_VALUES[i]) {
			rc = fail("reloaded SoftSat preset parameter changed");
			goto preset_cleanup;
		}
	}
...
int main(int argc, char** argv)
{
	...
	if (argc != 3) {
		fprintf(stderr, "usage: %s OUTPUT_DIRECTORY PATH_TO_SOFTSAT_SO\n", argv[0]);
		return 2;
	}
	if (snprintf(song_path, sizeof(song_path), "%s/phase5-softsat.psy", argv[1]) >=
			(int)sizeof(song_path) ||
			snprintf(preset_path, sizeof(preset_path), "%s/phase5-softsat.prs", argv[1]) >=
	...
	if (!song) {
		rc = fail("could not allocate SoftSat song");
		goto initial_cleanup;
	}
	...
	if (save_song(song, song_path) != 0) {
		rc = fail("SoftSat PSY3 save failed");
		goto song_cleanup;
	}
	...
	if (!loaded) {
		rc = fail("fresh SoftSat PSY3 reopen failed");
	} else {
		...
		if (verify_values(reopened) != 0) rc = 1;
				...
				psy_audio_wire_make(SLOT, psy_audio_MASTER_INDEX)))
			rc = fail("SoftSat wire to Master did not survive PSY3 reload");
...
initial_cleanup:
	...
	printf("phase5-softsat-state: PASS\n");
	printf("catcher: thunderpalace-softsat:0\n");
	printf("state: 2/2 public parameters seeded non-default, threshold=12000 hardness=1536, 0 opaque bytes\n");
	printf("preset-factory: independent\n");
	printf("topology: ThunderPalace SoftSat -> Master\n");
	printf("preset: %s\n", preset_path);
	printf("song: %s\n", song_path);
	return 0;
}
