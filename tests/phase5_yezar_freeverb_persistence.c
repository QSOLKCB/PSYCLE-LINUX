/*
** PSYCLE-LINUX Phase 5C Yezar/Jezar Freeverb production persistence test.
**
** Exercises the retained Freeverb effect through PluginCatcher, MachineFactory,
** preset I/O and PSY3 save/reload using five legal non-default public values.
*/
...
{
	fprintf(stderr, "phase5-yezar-freeverb-state: FAIL: %s\n", message);
	return 1;
}
	...
	const char* module_path)
{
	...
	if (!psy_audio_plugin_psycle_test(module_path, catcher->native_root_, &info)) {
		rc = fail("production native-plugin probe failed");
			...
			strcmp(info.name, "Jezar Freeverb") != 0 ||
			!info.shortname || strcmp(info.shortname, "Freeverb") != 0 ||
			!info.author || strcmp(info.author, "Jezar") != 0) {
		rc = fail("production probe returned unexpected Freeverb identity");
	} else {
		machineinfo_catchername(&info, catcher_name);
		if (strcmp(catcher_name, CATCHER_NAME) != 0) {
			fprintf(stderr,
				"phase5-yezar-freeverb-state: FAIL: catcher expected %s got %s\n",
...
static int verify_identity(psy_audio_Machine* machine)
{
	...
	if (!machine || psy_audio_machine_type(machine) != psy_audio_PLUGIN) {
		return fail("Freeverb missing or no longer a native Psycle plugin");
	}
	info = psy_audio_machine_info(machine);
	if (!info || !info->name || strcmp(info->name, "Jezar Freeverb") != 0 ||
			!info->shortname || strcmp(info->shortname, "Freeverb") != 0 ||
			!info->author || strcmp(info->author, "Jezar") != 0 ||
			!info->modulepath || !strstr(info->modulepath, "arguru-freeverb")) {
		return fail("Freeverb production identity/module path changed");
	}
	if (psy_audio_machine_num_parameters(machine) != PARAMETER_COUNT) {
		return fail("Freeverb production parameter count changed");
	}
	if (psy_audio_machine_data_size(machine) != 0) {
		return fail("Freeverb unexpectedly acquired opaque plugin state");
	}
	return 0;
}
...
static int seed_parameters(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		...
		intptr_t minval;
		intptr_t maxval;
		if (!param) return fail("Freeverb parameter surface is incomplete");
		psy_audio_machine_parameter_range(machine, param, &minval, &maxval);
		if (EXPECTED_VALUES[i] < minval || EXPECTED_VALUES[i] > maxval) {
			return fail("Freeverb preservation seed is outside public range");
		}
		psy_audio_machine_parameter_tweak_scaled(machine, param, EXPECTED_VALUES[i]);
		if (psy_audio_machine_parameter_scaled_value(machine, param) != EXPECTED_VALUES[i]) {
			return fail("Freeverb seed parameter did not retain requested value");
		}
	}
	return 0;
}
...
static int verify_values(psy_audio_Machine* machine)
{
	...
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		psy_audio_MachineParam* param = psy_audio_machine_parameter(machine, i);
		intptr_t actual;
		if (!param) return fail("Freeverb parameter missing during state verification");
		actual = psy_audio_machine_parameter_scaled_value(machine, param);
		if (actual != EXPECTED_VALUES[i]) {
			fprintf(stderr,
				"phase5-yezar-freeverb-state: FAIL: parameter %lu expected %ld got %ld\n",
	...
	psy_audio_Machine* source, const char* preset_path)
{
	...
	if (psy_audio_preset_num_parameters(captured) != PARAMETER_COUNT ||
		...
		psy_audio_presets_dispose(&saved);
		psy_audio_presets_dispose(&loaded);
		return fail("captured Freeverb preset geometry changed");
	}
	...
	if (status != psy_audio_PRESETIO_OK) {
		rc = fail("Freeverb preset save failed");
		goto cleanup;
	}
	...
	if (status != psy_audio_PRESETIO_OK || psy_audio_presets_size(&loaded) != 1) {
		rc = fail("Freeverb preset reload failed");
		goto cleanup;
	}
	...
	if (!reloaded || strcmp(psy_audio_preset_name(reloaded), PRESET_NAME) != 0 ||
			...
			reloaded->datasize != 0) {
		rc = fail("reloaded Freeverb preset metadata changed");
		goto cleanup;
	}
	for (i = 0; i < PARAMETER_COUNT; ++i) {
		if (psy_audio_preset_value(reloaded, i) != EXPECTED_VALUES[i]) {
			rc = fail("reloaded Freeverb preset parameter changed");
			goto cleanup;
		}
	}
	fresh = make_machine(factory);
	if (!fresh) {
		rc = fail("fresh Freeverb preset-restore machine creation failed");
		goto cleanup;
	}
...
int main(int argc, char** argv)
{
	...
	if (argc != 3) {
		fprintf(stderr, "usage: %s OUTPUT_DIRECTORY PATH_TO_ARGURU_FREEVERB_SO\n", argv[0]);
		return 2;
	}
	if (snprintf(song_path, sizeof(song_path), "%s/phase5-yezar-freeverb.psy", argv[1]) >=
			(int)sizeof(song_path) ||
			snprintf(preset_path, sizeof(preset_path), "%s/phase5-yezar-freeverb.prs", argv[1]) >=
	...
	if (!song) {
		rc = fail("could not allocate Freeverb song");
		goto initial_cleanup;
	}
	...
	if (save_song(song, song_path) != 0) {
		rc = fail("Freeverb PSY3 save failed");
		goto song_cleanup;
	}
	...
	if (!loaded) {
		rc = fail("fresh Freeverb PSY3 reopen failed");
	} else {
		...
		if (verify_values(reopened) != 0) {
			rc = 1;
		} else if (!psy_audio_machines_connected(psy_audio_song_machines(loaded),
				psy_audio_wire_make(SLOT, psy_audio_MASTER_INDEX))) {
			rc = fail("Freeverb wire to Master did not survive PSY3 reload");
		}
...
initial_cleanup:
	...
	printf("phase5-yezar-freeverb-state: PASS\n");
	printf("catcher: arguru-freeverb:0\n");
	printf("state: 5/5 public parameters seeded non-default, 0 opaque bytes\n");
	printf("topology: Jezar Freeverb -> Master\n");
	printf("preset: %s\n", preset_path);
	printf("song: %s\n", song_path);
	return 0;
}
