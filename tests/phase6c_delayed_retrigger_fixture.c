...
** Project-authored Phase 6C delayed/retrigger/extended-command fixture.
**
** C-Psycle constructs and freshly reloads these bytes only as fixture evidence.
** It is not the original-Psycle behavioural authority.
*/
#include <math.h>
#include <stdint.h>
#include <stdio.h>
...
#define TITLE "PSYCLE-LINUX Phase 6C delayed/retrigger fixture"
#define BPM 137.0
#define LPB 8
#define TPB 24
...
#define MARKER_NOTE 55

static int fail(const char* message)
{
    fprintf(stderr, "phase6c-delayed-retrigger-fixture: FAIL: %s\n", message);
    return 1;
}
    ...
    uint8_t parameter)
{
    ...
    return psy_audio_pattern_set_event_at_cursor(pattern, cursor, event)
        ? 0 : fail("could not insert command event");
}
    ...
    uint8_t parameter)
{
    ...
    if (event.note != note || event.mach != MACHINE ||
        ...
        return fail("command event changed");
    }
    return 0;
}
...
static int verify_song(psy_audio_Song* song)
{
    ...
    if (!sampler || psy_audio_machine_type(sampler) != psy_audio_SAMPLER)
        return fail("built-in sampler missing");
...
int main(int argc, char** argv)
{
    ...
    if (snprintf(path, sizeof(path),
            "%s/phase6c-delayed-retrigger.psy", argv[1]) >= (int)sizeof(path))
    ...
    if (!sampler) return fail("could not create built-in sampler");
    ...
    psy_audio_pattern_set_name(pattern, "Command Semantics");
    psy_audio_pattern_set_length(
        pattern, psy_dsp_beatpos_make_real(1.0, psy_dsp_DEFAULT_PPQ));

    if (put_event(song, pattern, 0, 0.0, 48,
            psy_audio_PATTERNCMD_NOTE_DELAY, NOTE_DELAY_PARAM) != 0 ||
        put_event(song, pattern, 1, 0.0, 50,
            psy_audio_PATTERNCMD_RETRIGGER, RETRIGGER_PARAM) != 0 ||
        put_event(song, pattern, 2, 0.0, 52,
            psy_audio_PATTERNCMD_RETR_CONT, RETR_CONT_PARAM) != 0 ||
        put_event(song, pattern, 3, 0.0, psy_audio_NOTECOMMANDS_EMPTY,
            psy_audio_PATTERNCMD_EXTENDED, EXTENDED_LPB_PARAM) != 0 ||
        put_event(song, pattern, 4, 1.0 / LPB, MARKER_NOTE, 0, 0) != 0)
        return 1;
    ...
    if (!loaded || verify_song(loaded) != 0)
        return fail("fresh C-Psycle load did not preserve command fixture");
    ...
    puts("phase6c-delayed-retrigger-fixture: PASS fd=7f fb=3f fa=42 fe=04 marker-row=1");
    return 0;
}
