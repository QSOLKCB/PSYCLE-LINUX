...
** Project-authored Phase 6C Sampler voice-startup isolation fixtures.
**
** PR #73 established that the first ordinary note is the first cumulative
** substrate rung that crashes pinned Psycle 1.12.0 x86 during offline render.
** These fixtures advance through the source-pinned Sampler::Tick gates:
**
**   note-no-previous-inst
**     note reaches Sampler with instrument FF; original returns because there
**     is no previous instrument on the track.
**
**   note-missing-sample
**     explicit instrument 1, while sample slot 1 is absent; original returns
**     at samples.IsEnabled().
**
**   note-sample-default-inst
**     sample 0 exists, no serialized C-Psycle instrument state; original
**     constructor-provided legacy Instrument remains the only instrument state.
**
**   note-sample-serialized-inst
**     sample 0 plus serialized instrument state; this mirrors the crashing
**     ordinary-note substrate while keeping this diagnostic lane independent.
**
** These are diagnostic evidence only and do not classify delayed/retrigger
** parity.
*/
#include <math.h>
#include <stdint.h>
...
typedef struct VoiceVariant {
    const char* name;
    const char* title;
    uintptr_t note_instrument;
    int serialize_instrument;
} VoiceVariant;

static const VoiceVariant VARIANTS[] = {
    {
        "note-no-previous-inst",
        "PSYCLE-LINUX Phase 6C Sampler startup no previous instrument",
        psy_audio_NOTECOMMANDS_INST_EMPTY,
        1
    },
    {
        "note-missing-sample",
        "PSYCLE-LINUX Phase 6C Sampler startup missing sample",
        1,
        1
    },
    {
        "note-sample-default-inst",
        "PSYCLE-LINUX Phase 6C Sampler startup default instrument",
        0,
        0
    },
    {
        "note-sample-serialized-inst",
        "PSYCLE-LINUX Phase 6C Sampler startup serialized instrument",
        0,
        1
    }
};
...
{
    fprintf(stderr,
        "phase6c-sampler-voice-startup[%s]: FAIL: %s\n",
        variant, message);
    return 1;
}
...
static int install_sample(
    psy_audio_Song* song, const VoiceVariant* variant)
{
    ...
    psy_audio_sample_set_name(sample, "Phase 6C startup impulse");
...
static int install_serialized_instrument(
    psy_audio_Song* song, const VoiceVariant* variant)
{
    psy_audio_Instrument* instrument = psy_audio_instrument_allocinit();
    ...
    psy_audio_instrument_set_name(
        instrument, "Phase 6C startup instrument");
    ...
    const VoiceVariant* variant)
{
    ...
    event.note = NOTE;
    event.inst = variant->note_instrument;
    event.mach = MACHINE;
    event.cmd = 0;
    event.parameter = 0;
    return psy_audio_pattern_set_event_at_cursor(pattern, cursor, event)
        ? 0 : fail(variant->name, "could not insert startup note");
}
    ...
    psy_audio_Song* song, const VoiceVariant* variant)
{
    ...
    if (!sample || psy_audio_sample_num_frames(sample) != CLICK_FRAMES)
        return fail(variant->name, "deterministic sample 0 missing");
    sample_one = psy_audio_samples_at(
        psy_audio_song_samples(song), psy_audio_sampleindex_make(1, 0));
    if (sample_one)
        return fail(variant->name, "sample slot 1 unexpectedly populated");

    instrument = psy_audio_instruments_at(
        psy_audio_song_instruments(song), psy_audio_instrumentindex_make(0, 0));
    if (variant->serialize_instrument) {
        if (!instrument || !psy_audio_instrument_entries(instrument))
            return fail(variant->name, "serialized instrument state missing");
    } else if (instrument) {
        return fail(variant->name, "default-instrument fixture gained serialized state");
    }
    ...
    if (event.note != NOTE ||
        ...
        event.cmd != 0 ||
        event.parameter != 0)
        return fail(variant->name, "startup note changed");

    return 0;
}

static int save_song(
    psy_audio_Song* song, const VoiceVariant* variant, const char* path)
{
...
static int build_variant(
    const VoiceVariant* variant, const char* output_directory)
{
    char path[4096];
    ...
    if (snprintf(path, sizeof(path),
            "%s/phase6c-sampler-voice-startup-%s.psy",
    ...
    if (install_sampler(song, &factory, variant) != 0 ||
        install_sample(song, variant) != 0)
        return 1;
    if (variant->serialize_instrument &&
        install_serialized_instrument(song, variant) != 0)
        return 1;

    pattern = psy_audio_patterns_at(psy_audio_song_patterns(song), 0);
    if (!pattern)
        return fail(variant->name, "default pattern missing");
    psy_audio_pattern_set_name(pattern, "Sampler Voice Startup Isolation");
    psy_audio_pattern_set_length(
        pattern, psy_dsp_beatpos_make_real(PATTERN_BEATS, psy_dsp_DEFAULT_PPQ));
    if (install_note(song, pattern, variant) != 0)
        return 1;
    ...
    if (!loaded || verify_song(loaded, variant) != 0)
        return fail(
            variant->name,
            "fresh C-Psycle load did not preserve startup fixture");
...
int main(int argc, char** argv)
{
    ...
    puts("phase6c-sampler-voice-startup: PASS");
    return 0;
}
