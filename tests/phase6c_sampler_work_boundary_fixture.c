...
** Project-authored Phase 6C Sampler Voice::Tick / Voice::Work boundary fixtures.
**
** PR #74 established that an enabled sample advances into original Psycle's
** Sampler voice-startup path and reproduces the pinned 0xC0000005 render exit.
** This ladder isolates the next source-pinned boundary without modifying the
** original executable:
**
**   release-no-active-voice
**     enabled sample + release event on an empty track; original Sampler::Tick
**     passes the sample-enabled gate but returns before Voice::Tick.
**
**   delayed-note-short
**     one-row song with note 60 / E-DF. Original Voice::Tick initializes the
**     voice and sets a 15/6-row (2.5-row) delay. The song ends after one row,
**     so Voice::Work remains in its pre-controller.Work delayed return path.
**
**   delayed-note-long
**     same E-DF event in a four-beat song. The delay expires and sample work
**     becomes reachable.
**
**   ordinary-note-short
**     one-row ordinary note. Sample work becomes reachable immediately.
**
** Diagnostic only. No delayed/retrigger parity classification is made here.
*/
#include <math.h>
#include <stdint.h>
#include <stdio.h>
...
#define MACHINE 0
#define NOTE 60
#define RELEASE_NOTE psy_audio_NOTECOMMANDS_RELEASE
#define SHORT_PATTERN_BEATS (1.0 / LPB)
#define LONG_PATTERN_BEATS 4.0
#define CLICK_FRAMES 512
#define CLICK_IMPULSE_FRAME 256
#define SAMPLER_CMD_EXTENDED 0x0E
#define SAMPLER_CMD_EXT_NOTEDELAY_MAX 0xDF

typedef struct WorkBoundaryVariant {
    const char* name;
    const char* title;
    uintptr_t note;
    uintptr_t command;
...
static const WorkBoundaryVariant VARIANTS[] = {
    {
        "release-no-active-voice",
        "PSYCLE-LINUX Phase 6C Sampler work boundary release no active voice",
        RELEASE_NOTE, 0, 0, SHORT_PATTERN_BEATS
    },
    {
        "delayed-note-short",
        "PSYCLE-LINUX Phase 6C Sampler work boundary delayed note short",
        NOTE, SAMPLER_CMD_EXTENDED, SAMPLER_CMD_EXT_NOTEDELAY_MAX,
        SHORT_PATTERN_BEATS
    },
    {
        "delayed-note-long",
        "PSYCLE-LINUX Phase 6C Sampler work boundary delayed note long",
        NOTE, SAMPLER_CMD_EXTENDED, SAMPLER_CMD_EXT_NOTEDELAY_MAX,
        LONG_PATTERN_BEATS
    },
    {
        "ordinary-note-short",
        "PSYCLE-LINUX Phase 6C Sampler work boundary ordinary note short",
        NOTE, 0, 0, SHORT_PATTERN_BEATS
    }
};
...
{
    fprintf(stderr,
        "phase6c-sampler-work-boundary[%s]: FAIL: %s\n",
        variant, message);
    return 1;
}
...
static int install_sampler(
    ...
    const WorkBoundaryVariant* variant)
{
...
static int install_sample_and_instrument(
    psy_audio_Song* song, const WorkBoundaryVariant* variant)
{
    ...
    psy_audio_sample_set_name(sample, "Phase 6C work-boundary impulse");
    ...
    psy_audio_instrument_set_name(
        instrument, "Phase 6C work-boundary instrument");
    ...
    const WorkBoundaryVariant* variant)
{
    ...
    event.note = variant->note;
    event.inst = 0;
    event.mach = MACHINE;
    event.cmd = variant->command;
    event.parameter = variant->parameter;
    return psy_audio_pattern_set_event_at_cursor(pattern, cursor, event)
        ? 0 : fail(variant->name, "could not insert boundary event");
}
    ...
    psy_audio_Song* song, const WorkBoundaryVariant* variant)
{
    ...
    if (event.note != variant->note ||
        event.inst != 0 ||
        event.mach != MACHINE ||
        event.cmd != variant->command ||
        event.parameter != variant->parameter)
        return fail(variant->name, "boundary event changed");

    return 0;
}

static int save_song(
    psy_audio_Song* song, const WorkBoundaryVariant* variant, const char* path)
{
...
static int build_variant(
    const WorkBoundaryVariant* variant, const char* output_directory)
{
    char path[4096];
    ...
    if (snprintf(path, sizeof(path),
            "%s/phase6c-sampler-work-boundary-%s.psy",
    ...
    psy_audio_pattern_set_name(pattern, "Sampler Voice Work Boundary Isolation");
    psy_audio_pattern_set_length(
        pattern,
        psy_dsp_beatpos_make_real(
            variant->pattern_beats, psy_dsp_DEFAULT_PPQ));
    if (install_event(song, pattern, variant) != 0)
        return 1;
    ...
    if (!loaded || verify_song(loaded, variant) != 0)
        return fail(
            variant->name,
            "fresh C-Psycle load did not preserve work-boundary fixture");
...
int main(int argc, char** argv)
{
    ...
    puts("phase6c-sampler-work-boundary: PASS");
    return 0;
}
